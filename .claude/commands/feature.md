...
**요청이 `docs/specs/` 컴포넌트 스펙 경로 하나뿐이면** 요청 파일을 새로 쓰지 않고
그 파일을 `--request-file` 로 그대로 준다 — 스펙 본문이 동결 요청 원문이 된다
(ADR-H079). slug 는 파일명에서 `.md` 를 뺀 것을 소문자로 쓴다. 경로에 문장이
붙어 있으면 위의 흐름대로 문장 전체를 동결한다 — 추가 조건은 스펙에 적는 자리다.
