---
description: 요청 하나를 레인 예측 → 계획 → 구현 → 게이트 → 코드리뷰 → PR → PR리뷰 → 보고서(00~08)까지 끌고 간다. 머지는 하지 않는다.
---

> **이 커맨드는 트리아지와 일곱 페이즈를 전부 돈다** — 레인 예측 · 계획 ·
> 구현 · 게이트 · 코드리뷰 · PR · PR리뷰 · 보고서. 레인이 `docs`
> 면 계획 리뷰어 · 역할이 빠진다 — 그것은 기계가 정하고 네가
> 정하지 않는다.
...
01 의 리뷰어는 `plan-reviewer` 하나다 (ADR-H045). **2라운드부터는
