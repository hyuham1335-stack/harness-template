...
어휘 — **상태**: `수용됨`(1·2차 파일럿 전, 상태 칸이 없던 시절의 기록) · `채택됨`(main 에 머지) · `근거 기록`(결정이 아니라 실측 표). **지금**: `구현` · `부분 승계`(일부만 코드에 남음) · `실행기 소멸`(`scripts/execute.py` 와 함께 템플릿 밖, [[ADR-H037]]) · `기록`(코드 대상 없음). 열린 항목은 [미구현 백로그](#미구현-백로그-2026-09-19)의 번호로 가리킨다.
...
| [[ADR-H035]] | 통제 어휘를 리뷰어까지 | 수용됨 | 구현 | `CONTRACT_MISMATCH`([[ADR-H043]])·`false_positive`([[ADR-H050]]) 추가. **한계 4 `OTHER` 26관측(C6) 미착수** → 백로그 8 |
| [[ADR-H036]] | 세션·런 귀속은 구간 겹침 | 수용됨 | 구현 | — |
...
**파동 2 — 사람 결정이 곧 작업** — 13 닫힘 · 8 은 파동 3 으로 이동 (2026-09-19)
...
8(클론의 `OTHER` 관측 대기, 파동 2 에서 이동) · 7(오탐 원인 규명 → `required_tests` 1줄 + `BASELINE_CHECKS` 제외, [[ADR-H058]] 결정 7 을 뒤집으므로 새 ADR) · 11(코드 완비 — 아래 정정) · 15 · 19 표(지표 9개 **전부 기록 배선이 있다**, 원인은 런 0) · 파동 1 의 6 등급값.
