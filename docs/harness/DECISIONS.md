...
어휘 — **상태**: `수용됨`(1·2차 파일럿 전, 상태 칸이 없던 시절의 기록) · `채택됨`(main 에 머지) · `근거 기록`(결정이 아니라 실측 표). **지금**: `구현` · `개정됨`(뒤 결정이 바꿔 그쪽이 정본) · `부분 승계`(일부만 코드에 남음) · `해당 없음`(근거 기록이라 코드 대상이 없음) · `실행기 소멸`(`scripts/execute.py` 와 함께 템플릿 밖, [[ADR-H037]]) · `대상 소멸`(덜어내기로 코드 밖 — [[ADR-H075]]·[[ADR-H076]]) · `기록`(코드 대상 없음). 열린 항목은 [미구현 백로그](#미구현-백로그-2026-09-19)의 번호로 가리킨다.

**실행기 시대 (H001~H012)** — 순차 실행기에 대한 결정이다. 원칙은 페이즈 파이프라인이 물려받았고 코드는 없다.
...
| [[ADR-H007]] | `MAX_RETRIES` 는 바닥값 | 수용됨 | 대상 소멸 | `RETRY_RECORD_MIN`·`RETRY_BUDGET_FLOOR`·캘리브레이션은 [[ADR-H075]] Wave 1 이 지웠다. 원칙("값이 아니라 출처")만 남아 최다 인용된다 |
| [[ADR-H008]] | 가드레일은 step 이 고른다 | 수용됨 | 실행기 소멸 | `_load_guardrails` 가 없어 `CLAUDE.md` 자동 주입도 없다(ROADMAP §4). "읽었는가" 를 묻던 [[ADR-H055]] 의 게이트는 [[ADR-H075]] 가 지웠다 |
| [[ADR-H009]] | step 이 지목한 소스는 실행기가 싣는다 | 수용됨 | 실행기 소멸 | 부수 결정(`scoped.select` 삭제)은 `tests_from` 경로 조립으로 승계 |
| [[ADR-H010]] | 접두부는 분해해서 잰다 | 수용됨 | 실행기 소멸 | [[ADR-H012]] 가 축을 재해석. 옛 ROADMAP §7-1 "예산 단위 미정" 은 덜어내기로 소멸(백로그 14 닫힘) |
| [[ADR-H011]] | 끌어온 양은 트랜스크립트에서 사후로 | 수용됨 | 대상 소멸 | `read_session_metrics`·`read_cost_state`·`cli cost` 는 [[ADR-H075]] 가 지웠다(`runtime.py` 에는 시각·인코딩만 남았다). `backfill_reads.py` 도 소멸 |
| [[ADR-H012]] | 통독 여부 × 크기 | 수용됨 | 기록 | n=2 그대로. 상한은 걸지 않았다 |

**페이즈 파이프라인 이식 (H013~H037)**

| ADR | 한 줄 | 상태 | 지금 | 개정·대체 · 비고 |
|---|---|---|---|---|
| [[ADR-H013]] | 레이아웃 재조정 · G3 다섯 줄 | 수용됨 | 부분 승계 | 소멸: 어댑터 `verified`·`verify-adapter`·`gate --replay` ([[ADR-H075]]). 레이아웃과 다섯 줄 통과(2026-09-03)는 그대로다 |
| [[ADR-H014]] | 재기 · 형식 규칙 · xv 선언 | 수용됨 | 부분 승계 | 소멸: xv 선언 ([[ADR-H075]]). 모델 호출 계수·계약 파서 `dropped`·형식 규칙은 산다. (a) 계수 기준은 [[ADR-H020]] 이 개정 |
...
| [[ADR-H018]] | 원장 신원 `(run, phase, key)` | 수용됨 | 대상 소멸 | 원장(`findings.jsonl`·`repaired_by`)은 [[ADR-H075]] Wave 2 가 지웠다 |
| [[ADR-H019]] | 계약 삭제는 push 뒤 | 수용됨 | 구현 | — |
| [[ADR-H020]] | 관측 단위는 지시 | 수용됨 | 구현 | [[ADR-H042]] 결정 2 가 "낸 자리에서 전부" 로 개정. `PreToolUse(Task)` 실측은 미착수 → 백로그 16 |
| [[ADR-H021]] | 승격 베이스라인은 기계 측정 | 수용됨 | 대상 소멸 | 승격 계층은 [[ADR-H075]] Wave 2 가 지웠다 |
| [[ADR-H022]] | xv 부재와 일시 실패를 가른다 | 수용됨 | 대상 소멸 | `cross_verify` 와 02 는 [[ADR-H075]] Wave 3 가 지웠다 |
| [[ADR-H023]] | 정체 단위는 (소유자, 시그니처) | 수용됨 | 대상 소멸 | `sig_chain`·`flip`·귀속 모듈은 [[ADR-H075]] Wave 4 가 지웠다 |
| [[ADR-H024]] | 왕복은 지급 | 수용됨 | 대상 소멸 | 02→01 왕복과 지급(`grants`)은 [[ADR-H075]] 가 지웠다. 01 상한 값은 `loop.max_by_profile` 이 정본이다 |
| [[ADR-H025]] | 선언은 읽히거나 거부된다 | 수용됨 | 구현 | [[ADR-H061]] 이 예외를 명시(프론트매터 `effort` 는 Claude Code 가 읽는 선언이라 exit 2 대상 아님) |
| [[ADR-H026]] | 승격의 축은 제목 | 수용됨 | 대상 소멸 | [[ADR-H034]] 가 축을 바꾼 뒤 승격 계층 자체를 [[ADR-H075]] 가 지웠다 |
| [[ADR-H027]] | 프로브 면제는 gap | 수용됨 | 구현 | [[ADR-H051]] 결정 2 가 같은 모양 |
| [[ADR-H028]] | `--scope pr` 은 합쳐 잰다 | 수용됨 | 구현 | 라우팅 scope `worktree` 유지는 [[ADR-H046]] 이 재확인 |
| [[ADR-H029]] | 예산의 사유는 원장이 | 수용됨 | 부분 승계 | 소멸: 원장 ([[ADR-H075]]). `counter_inc` 사유는 상태·이벤트에 산다. `REPAIR_REASONS` 는 [[ADR-H059]] 가 삭제 |
...
| [[ADR-H032]] | 소요·비용은 유도값 | 수용됨 | 부분 승계 | 소멸: 비용 계측·귀속 ([[ADR-H075]]). 소요는 `events.jsonl` 유도값으로 산다 |
| [[ADR-H033]] | 승격 판정 시한 | 수용됨 | 대상 소멸 | 승격 계층을 [[ADR-H075]] 가 지웠다 |
| [[ADR-H034]] | 승격의 축은 `rule_key` | 수용됨 | 대상 소멸 | `rule_key`·원장·승격을 [[ADR-H075]] 가 지웠다 |
| [[ADR-H035]] | 통제 어휘를 리뷰어까지 | 수용됨 | 대상 소멸 | 통제 어휘(`taxonomy`·`OTHER`)는 [[ADR-H075]] 가 지웠다 — category 는 리뷰어의 자유 태그다. `false_positive`([[ADR-H050]])는 01 에 남았다 |
| [[ADR-H036]] | 세션·런 귀속은 구간 겹침 | 수용됨 | 대상 소멸 | 세션 원장(`session_log`·`run_cost`)은 [[ADR-H075]] T1-9 가 지웠다 |
...
| [[ADR-H040]] | Stop 훅 제거 | 수용됨 | 부분 승계 | 소멸: SessionStart·SessionEnd 훅 ([[ADR-H075]]). 훅은 PreToolUse(Bash) 하나 — 위험 명령 차단([[ADR-H076]] A5) |

**1차 파일럿 회고 (H041~H045)**

| ADR | 한 줄 | 상태 | 지금 | 개정·대체 · 비고 |
|---|---|---|---|---|
| [[ADR-H041]] | 01 은 Critical 만 강제 · 상한 3 | 수용됨 | 부분 승계 | 소멸: 01 `stuck_after_identical` ([[ADR-H075]]). 상한은 같은 결정이 `{fix: 1, normal: 2}` 로 개정(small 레인 삭제). Critical 만 강제는 산다. `fix: 1` 은 [[ADR-H053]] |
| [[ADR-H042]] | 02 미편집 생략 · 지시는 낸 자리에서 | 수용됨 | 부분 승계 | 소멸: 02 미편집 생략(`plan_unedited`·`skip_policy`) — 02 와 함께 ([[ADR-H075]]). 지시 단위 계수는 산다. 05·07 이중 리뷰 문단은 [[ADR-H043]] 이 대체 |
| [[ADR-H043]] | gen 은 05 로 · 07 은 신호 있을 때만 | 수용됨 | 부분 승계 | 소멸: 07 트리거(`review07.decide`) — 07 은 `/code-review` 1회 ([[ADR-H075]]). gen 이 05 리뷰어인 것은 산다. 트리거 둘은 [[ADR-H059]] 가 먼저 삭제 |
| [[ADR-H044]] | 00 이 레인을 나눈다 | 수용됨 | 대상 소멸 | 00-triage 는 [[ADR-H075]] Wave 3 가 지웠다 — 레인은 `init --profile` 선언이고 빗나가면 `lane_miss` |
| [[ADR-H045]] | xv 는 02 한 곳 | 수용됨 | 대상 소멸 | 02 교차검증은 [[ADR-H075]] Wave 3 가 지웠다 |
...
| [[ADR-H047]] | 하한·캘리브레이션은 런이 갱신 | 채택됨 | 부분 승계 | 소멸: 캘리브레이션·`verify-adapter` ([[ADR-H075]]). 테스트 수 하한(`completed_runs`)·`stage_na` 는 산다 |
| [[ADR-H048]] | 카운터 상한 강제 · 재상정은 지급 | 채택됨 | 부분 승계 | 소멸: 재상정 지급(`xverify_return`) ([[ADR-H075]]). 카운터 상한 강제는 산다 |
| [[ADR-H049]] | contract-trace 는 어댑터 관례를 읽는다 | 채택됨 | 부분 승계 | 소멸: `out_of_contract` 검사·원장 출처 비율 ([[ADR-H075]]). `entrypoint_resolver` 관례 읽기는 산다 |
| [[ADR-H050]] | 0 findings 는 신호 | 채택됨 | 부분 승계 | 소멸: 결정 5(b)(02) · `clean_05` ([[ADR-H075]]). gap 어휘(`GAP_REASONS`)·`false_positive` 는 산다. 결정 4 는 [[ADR-H062]] 가 닫음 |
| [[ADR-H051]] | 미룸은 이월 · 시한은 등급이 치른다 | 채택됨 | 대상 소멸 | 미룸 이월·`promotion_overdue` 는 원장·승격과 함께 [[ADR-H075]] 가 지웠다 |
| [[ADR-H052]] | 08 은 대기·형식 왕복·비용을 센다 | 채택됨 | 부분 승계 | 소멸: 비용 칸 ([[ADR-H075]]). 사람 대기 분리·형식 반려 계수는 산다 |
| [[ADR-H053]] | `fix` 레인 | 채택됨 | 구현 | 실물 `fix` 런 0(ROADMAP §7 의 1). 우회 측정 → 백로그 10 |
| [[ADR-H054]] | 예산·상한 값의 15런 근거 | 근거 기록 | 해당 없음 | `files_max` 는 [[ADR-H066]] 이 (b) 로 닫음 |
| [[ADR-H055]] | 규칙 읽기를 게이트가 묻는다 | 채택됨 | 대상 소멸 | `rules_read` 해시 게이트는 오탐 반려만 내어 [[ADR-H075]] Wave 4 가 지웠다(`CLAUDE.md` ⚠ 문단) |
| [[ADR-H056]] | 승격은 기계 강제만 · `retire` | 채택됨 | 대상 소멸 | 승격·`retire`·슬롯 예산은 [[ADR-H075]] Wave 2 가 지웠다 |
| [[ADR-H057]] | 화면은 `ui` 역할 | 채택됨 | 대상 소멸 | `ui` 역할은 [[ADR-H075]] Wave 4 가 지웠다 — 작성자는 `impl` 하나 |
| [[ADR-H058]] | 계약·인가 테스트를 contract-trace 가 센다 | 채택됨 | 부분 승계 | 소멸: `authz_untested`·`missing_journey_spec`·03 계약 사전검사 ([[ADR-H075]]). `untested_entrypoint`·`untested_error_symbol`·`06_pr_notes` 는 산다 |
| [[ADR-H059]] | 07 트리거 삭제 · 05 범위는 레인이 | 채택됨 | 부분 승계 | 소멸: 07 트리거 판정 — 07 은 `/code-review` 1회 ([[ADR-H075]]). 05 범위(`review.depth`, 값 하나)는 산다. audit 문구는 [[ADR-H061]] 이 대체 |
| [[ADR-H060]] | 02 는 최대 1회 · `risk` 가 돌린다 | 채택됨 | 대상 소멸 | 02 와 `risk` 는 [[ADR-H075]] Wave 3 가 지웠다 |
| [[ADR-H061]] | 작성자는 싸게, 검사자는 비싸게 | 채택됨 | 부분 승계 | 소멸: `config.models` 등급표 ([[ADR-H075]]). 모델·effort 는 에이전트 프론트매터가 정한다(작성자 sonnet · 리뷰어 opus). 모델은 [[ADR-H077]] 이 셋 다 `claude-opus-5-5` 로 개정 — 「작성자는 싸게」가 뒤집혔다 |
| [[ADR-H062]] | `test` 리뷰어가 `arch` 보다 먼저 | 채택됨 | 대상 소멸 | 리뷰어가 gen 하나라([[ADR-H075]] Wave 4) 순서를 정할 대상이 없다 |
| [[ADR-H063]] | 프로필 시드는 템플릿 키를 전부 갖는다 | 채택됨 | 구현 | 백로그 12 닫음 |
| [[ADR-H064]] | 05 수리 작성자도 지시 키를 받는다 | 채택됨 | 부분 승계 | 소멸: 모델 슬롯(`_slot_of`) ([[ADR-H075]]). 05 수리 봉투의 작성자 지시 키는 산다 |
| [[ADR-H065]] | 승격 자체 게이트는 실행기가 | 채택됨 | 대상 소멸 | 승격 자체 게이트는 승격 계층과 함께 [[ADR-H075]] 가 지웠다 |
| [[ADR-H066]] | 파일 수 예산은 소스만 센다 | 채택됨 | 구현 | 백로그 13 닫음. 줄 수는 전체 그대로 |
| [[ADR-H067]] | `risk_undeclared` 는 관측만 | 채택됨 | 대상 소멸 | `risk_undeclared` 는 02 와 함께 [[ADR-H075]] 가 지웠다 |
| [[ADR-H068]] | 원장은 세션이 말한 worktree 를 따로 적는다 | 채택됨 | 대상 소멸 | 세션 원장을 [[ADR-H075]] T1-9 가 지웠다 |
| [[ADR-H069]] | 어댑터 `verified` 는 규칙이 판정을 낸 관측으로 올린다 | 채택됨 | 대상 소멸 | 어댑터 `verified`·`verify-adapter` 는 [[ADR-H075]] 가 지웠다 |
| [[ADR-H070]] | 클론 4런이 연 결함 셋 — 델타 재리뷰 · 02 인용 대조 · 규칙 집합 | 채택됨 | 부분 승계 | 소멸: 결정 3·4(02 인용 대조) · 결정 5(`rules_exclude`) ([[ADR-H075]]). 델타 재리뷰(결정 1·2)는 산다. 백로그 20 · 21 · 23 닫음 |
| [[ADR-H071]] | 클론 4런이 연 마찰 셋 — 관례 파일 면제 · precheck 재질문 · 에스컬레이션 메뉴 | 채택됨 | 대상 소멸 | `convention_globs`·`--ack-policy`·`REVIEW_ESCALATION_OPTIONS` 는 [[ADR-H075]] Wave 4 가 지웠다 |
| [[ADR-H072]] | 결정 대기 넷 — 등급 신호 · 07 빈손 호출 · 귀속 기본값 · 타입 전용 export | 채택됨 | 부분 승계 | 소멸: 결정 2·3·4(07 빈손 호출 · 귀속 기본값 · 타입 전용 키워드) ([[ADR-H075]]). 결정 1(`not_applicable`)은 어댑터에 산다 |
...
| [[ADR-H076]] | 덜어내기 사후 검증 — 실행기 결함과 집행 지점 | 채택됨 | 구현(PR 1~4 · fix 1·2 · PR 4 잔여) | 초록이 정합을 뜻하지 않았다. PR 1 = 실행기 결함 A1~A10 + 훅. PR 2 = 읽히지 않는 선언·잔재 + gen 에이전트(추기). PR 3 = 문서·색인·PILOT-LOG(추기). fix = PR 3 이 남긴 결함 다섯(추기). fix 2 = fix 가 남긴 셋 — 미읽음 선언 · 「미구현」 행 · 재게이트 소요(추기). PR 4 = 비용·시간 — 픽스처 복사 · 원자 쓰기 · 어댑터 재로드 · kind cmd 삭제(추기). PR 4 잔여 = 정확성 넷 + `receipt_stale` 이벤트 + 예측 7·8 기준선 재셈, 성능 셋은 백로그 36~38(추기) |
| [[ADR-H077]] | 서브에이전트 셋을 Opus 5.5 로 — 작성자도 | 채택됨 | 구현 | [[ADR-H061]] 결정 1 의 「작성자는 싸게」를 개정. 별칭 대신 전체 ID. effort 는 `high` 그대로. [[ADR-H075]] 되돌림 조건 3 을 반대 방향으로 |
| [[ADR-H078]] | 06 승인을 설정 스위치로 — `vcs.pr_approval` | 채택됨 | 구현 | [[ADR-H015]] 의 「사람이 미리 켜 두는 `--auto`」에 켤 자리를 줬다. 템플릿 `auto` · 키 없으면 `user`. 영수증 검사와 철회는 그대로 멈춘다 |
| [[ADR-H079]] | 컴포넌트 스펙 — PRD 는 Must 목록, US 하나 = `docs/specs/` 파일 하나 | 채택됨 | 구현 | 스펙 경로 하나뿐인 요청은 그 파일을 그대로 동결한다 — 실행기 0줄. 03 「읽을 곳」에 요청 원문([[ADR-H075]] 의 세 줄 → 넷) · 05 입력에 요청 원문 경로 |
...
**1~19 는 본문이 미룬 것이고, 20~30 은 다르다 — 실물 런이 연 것이다.** 근거는
클론 리포 `banana-island-ops` 의 4런(`20260919-2342-9258` · `-2343-d42d` · `-2343-1c04` ·
`20260920-0107-4265`, 2026-09-20)이고, 아래에서 **「클론 4런」** 으로 줄여 부른다.
숫자의 정본은 **그 리포의** `docs/harness/PILOT-LOG.md` 「4런 교차 분석」 절이다 — 여기에
값을 옮겨 적지 않는다 ([[ADR-H039]]). 그 문서는 같은 것을 `D-1`~`D-11` 로 부르고
**자기 절에 이 번호들과의 대응표를 갖고 있다.** **20 · 21 · 23 은 미룬 결정이 아니라 결함이다.**

우선순위 — **P0** 결함이 게이트를 지나갈 수 있는 공백 · **P1** 재지 못해 판단을 못 하는 관측 공백 · **P2** 사람이 값이나 방향을 정해야 진행되는 것 · **P3** 클론한 프로젝트의 몫이거나 표본이 쌓여야 하는 것.

### 열린 항목 (2026-09-23)

**지금 열린 번호는 이것뿐이다** — 나머지는 전부 `닫힘`(대상 소멸 포함)이다. 아래 파동 표와 티어 목록은
기록이고, 무엇이 열려 있는지는 이 표가 말한다. 번호를 닫거나 더하면 이 표부터 고친다.

| # | 무엇 | 성격 | 닫힘 조건 |
|---|---|---|---|
| 10 | 파이프라인 우회 측정 | 밖 | 클론 `git log --merges` 대비 `_workspace/runs/` 수를 첫 실물런 뒤 1회 잰다 |
| 16 | `PreToolUse(Task)` 훅으로 모델 호출 실측 | 밖 | 코어 기준이 될 수 없다 — 상위 기준이 필요해질 때 |
| 17 | 어댑터 `e2e.cmd` 도입 | 밖 | 프로젝트가 러너를 들인다 |
| 18 | ROADMAP §5 보류 여섯 | 밖 | §5 의 이유가 무효가 될 때 |
| 19 | 재검토 시점 대기 표 | 표본 | 행별 — 첫 실물런이 대부분을 연다 |
| 36 | 어댑터·config 재로드 셋 | 표본 | 아래 36 |
| 37 | `pr` 1회에 `secret_values` 2번 | 표본 | 아래 37 |
| 38 | 유닛×파일 전수 읽기 넷 | 표본 | 아래 38 |

### 실행 순서표 (2026-09-19 · 파동 5 추가 2026-09-20 · 파동 6 추가 2026-09-21)
...
8(클론의 `OTHER` 관측 대기, 파동 2 에서 이동) · 7(오탐 원인 규명 → `required_tests` 1줄 + `BASELINE_CHECKS` 제외, [[ADR-H058]] 결정 7 을 뒤집으므로 새 ADR) · 11(코드 완비 — 아래 정정) · 15 · 19 표(지표 14개 **전부 기록 배선이 있다**, 원인은 런 0) · 파동 1 의 6 등급값 · **28 본체**(리뷰어별 수확 편차 — 곁가지는 [[ADR-H073]] 이 닫았다) · **29**(`nothing_locked` 기계화) · **34**(`instruction_slot_budget` 값 — **33 이 먼저다**, 세는 범위와 방식을 고치기 전 값은 무의미하다). 셋 다 2026-09-21 배치. **2026-09-23: 7·8·11·15 는 대상 소멸로 닫힘**([[ADR-H076]]). **28 본체·29·34 는 2026-09-22 [[ADR-H075]] 가, 6 등급값은 2026-09-23 [[ADR-H076]] PR 4 잔여가 대상 소멸로 닫았다** — 이 파동에 남은 것은 19 표뿐이다.

**파동 4 — 밖 · 보류**

10(클론 리포에서 `git log --merges` 대조, 하네스에 넣어도 `cmd_status` 한 칸) · 14(`secondary` 읽는 코드 0, 공급자 미정) · 16(`.claude/settings.json` 의 `PreToolUse` matcher 는 `Bash` 뿐) · 17(아래 정정) · 18 · **32**(무엇이 「제출 검사」인가 — 결정이 앞선다) · **35**(워커별 규칙 파일 선별 — 매핑 축과 누수 측정법 둘 다 미정. 2026-09-21 배치) · **2(b)** — 02 시점에 계약 파일이 없다(03 이 처음 `produces`). "02 에 계약을 넘긴다" 는 봉투 한 줄이 아니라 계약 작성 시점을 01/02 로 옮기는 변경이고 페이즈 파일 3개 + `_record_02` 대조 대상 + `lint-phases` 의 `produces_key` 유일성까지 60~120줄급이다. **2026-09-23: 14·2(b) 는 대상 소멸로 닫힘, 17 은 정정**([[ADR-H076]]). **32·35 는 2026-09-22 [[ADR-H075]] 가 대상 소멸로 닫았다** — 남은 것은 10 · 16 · 17 · 18.

**파동 5 — 클론 4런이 연 것 (2026-09-20). 앞의 셋은 결정이 아니라 결함이다**

| # | 성격 | 규모 | 닫힘 조건 |
|---|---|---|---|
| 20 | **닫힘 2026-09-20 · [[ADR-H070]] 결정 1·2** | 두 줄 이동만으로는 닫지 못했다 — `_review_render` 가 `routing` 전원을 이름 지어 **제출 셋이 exit 8** 이 되는 새 결함이 생긴다. 봉투도 같이 좁혔다 | 테스트 3: `rounds_planned["3"] == [델타]` · 델타 라운드 패킷이 한 명만 · 1라운드는 전원 |
| 21 | **닫힘 2026-09-20 · [[ADR-H070]] 결정 3·4** | 1줄이 아니었다 — 페이즈 파일 세 곳과 **01 과 공유인 `plan-reviewer.md`** 까지다. 어휘는 `check_review` 를 통째로 부르면 헤딩 대조가 전 제출을 exit 8 로 만들어 `check_vocabulary` 로 떼어냈다 | 테스트 6 |
| 23 | **닫힘 2026-09-20 · [[ADR-H070]] 결정 5** | **스냅샷 안은 버렸다** — 반려 창을 닫는 게 아니라 `[init, 워커읽기]` 로 늘린다. 남은 것은 제외 목록(`config.project.rules_exclude`)이고 그것이 닫힘 조건을 그대로 지킨다 | 테스트 3 |
| 24 | **범위만 닫힘 2026-09-20 · [[ADR-H071]] 결정 1·2** | 어댑터 `convention_globs` + 코어 `is_convention_file`. **진입점 맵에 넣지 않은 것이 요점**이다 — 넣으면 `missing_entrypoint` 가 계약에 없는 화면을 요구한다 | 픽스처로 잠갔다(테스트 5). **타입 전용 export 는 열린 채다** — 결정이 앞선다 |
| 26 | **닫힘 2026-09-20 · [[ADR-H071]] 결정 3·4·5** | 지문은 내용 해시가 아니라 **실패 사유 집합 + 값**이다 — 05·06 사이 수리가 내용 해시를 매번 바꾼다 | 테스트 12. `waiting_human {"reason":"precheck_policy"}` 런당 1회를 테스트가 잠근다 |
| 30 | **닫힘 2026-09-20 · [[ADR-H071]] 결정 6** | 문자열 1개가 맞았다. 다만 **상수로 올려야** 「메뉴에 있다」를 기계가 말한다 | 테스트 2. 「기타」를 안 고르는지는 다음 런이 판정한다 |

나머지 다섯은 이 파동에 없다 — **22 · 27 은 사람 결정이 앞서서(파동 2 성격),
25 는 07 의 존재 이유를 같이 정해야 하고, 28 · 29 는 표본이 더 쌓여야 한다(파동 3).**
순서로는 20 · 21 · 23 을 먼저 한 상자에 묶는 것이 싸다 — 셋 다 코드가 작고,
셋 다 **다음 런 하나로 고쳐졌는지 기계가 말한다.**

**앞의 셋은 닫혔다 (2026-09-20 · [[ADR-H070]]).** 셋 다 규모 추정이 틀렸다는 것이
이 파동의 수확이다 — 20 은 상태와 봉투 둘을 같이 고쳐야 했고, 21 은 1줄이 아니라
네 파일이었으며, 23 은 **설계안 자체가 반려 창을 늘리는 것이어서 버렸다.**
남은 셋(24 · 26 · 30)은 결함이 아니라 마찰 제거이고 닫힘 조건이 「다음 런」에 달려 있다.

**파동 6 — 결정이 앞섰던 넷 (2026-09-21). 사람이 고르자 코드가 작아졌다**

| # | 성격 | 규모 | 닫힘 조건 |
|---|---|---|---|
| 22 | **닫힘 2026-09-21 · [[ADR-H072]] 결정 1** | 재라벨링으로는 안 닫혔다 — `self-python` 은 `cmd:null` 이 다섯이라 e2e 하나를 올려도 등급이 그대로다. **문제를 다시 정의했다**: 등급 고정은 결함이 아니라 어댑터가 비었다는 정확한 신호다 | 테스트 2. 등급은 안 바뀐다 — 그것이 결정의 내용이다 |
| 24 (잔여) | **닫힘 2026-09-21 · [[ADR-H072]] 결정 4** | `enum` 을 빼는 것이 갈린 곳이다 — `export enum` 은 런타임 객체를 내보내므로 「계약은 런타임 심볼을 본다」는 사유로 면제할 수 없다 | 테스트 4. 타입 별칭 0건은 다음 런이 판정한다 |
| 25 | **닫힘 2026-09-21 · [[ADR-H072]] 결정 2** | 분기를 사슬 안에서 옮기지 않고 **사슬 밖**으로 뺐다 — 재배치는 `decide()` 가 경고하는 「구멍 옮기기」다 | 테스트 4(기존 1 갱신 · 신규 3) |
| 27 | **닫힘 2026-09-21 · [[ADR-H072]] 결정 3** | 사다리 재배치만으로는 새 결함이 생긴다 — 위치 인덱스 배정이 뒤집힌 사다리에서 한 역할을 건너뛴다. 배정을 미시도 집합으로 같이 바꿨다 | 테스트 3 |

**넷 다 규모 추정이 컸다** — 22 는 「한 줄」이 아니라 **문제 재정의**였고, 25·27 은 「분기 이동」·
「순서 바꾸기」가 아니라 각각 사슬 밖으로 빼는 것과 배정 방식을 바꾸는 것이었다. ~~**결정 대기가 비었고
남은 것은 표본 대기(28 · 29)와 밖(10 · 17 · 18)뿐이다.**~~ **그 셈이 틀렸다** — [[ADR-H070]] 이
**번호를 주지 않고** 산문으로 남긴 항목 하나가 어느 칸에도 안 세어지고 있었다. 파동 7 이 그것을
닫으면서 **번호 없는 잔여를 만들지 않는다**를 규칙으로 삼는다.

**파동 7 — 번호를 못 받았던 잔여 (2026-09-21). 선언과 실행의 괴리**

| # | 성격 | 규모 | 닫힘 조건 |
|---|---|---|---|
| (번호 없음) | **닫힘 2026-09-21 · [[ADR-H073]]** | `submit_checks[].id` 는 10종이 아니라 **24종**이었고 20종이 grep 0건. `produces[].schema` 는 **가리킬 아티팩트가 없어** 처방이 반대(삭제)였다 | `lint-phases` 가 어휘·`on_fail`·구현 포인터를 잡는다. 테스트 15 |
| 28 (곁가지) | **닫힘 2026-09-21 · [[ADR-H073]] 결정 8** | 값을 맞추는 것이 아니라 **폴백을 없애는 것**이었다 — 값을 맞추면 두 출처가 그대로 남는다 | 테스트 4. 선언을 바꾸면 절단이 따라 바뀌는 변이 테스트 |

**이 파동은 규모 추정이 작았다 — 처음이다.** `~180~260줄` 로 봤는데 실제는 그 두 배 가까이다.
전부 **탐색이 전제를 뒤집어서**다: 24종(추정 10) · `schema` 10종(추정 8) · 그리고 **07 의
`source_quote_substring` 은 구현이 아예 없었다**(추정: 전부 구현돼 있다). 마지막 것이 이 파동의
수확이고, 그것이 없었으면 레지스트리가 가짜 포인터를 박아 **M36 결함 하나를 검증된 것처럼
세탁하고** 끝났을 것이다.

**남은 것은 표본 대기(28 본체 · 29)와 밖(10 · 17 · 18), 그리고 이 파동이 새로 연 코드 둘(31 · 32)이다.**
31 · 32 는 지금 짤 수 있지만 [[ADR-H073]] 이 **범위 밖으로 밀어 번호를 준 것**이라 파동 8 감이다.

**파동 8 — 계기판을 정직하게 (2026-09-21). 33 만 닫혔고 31 · 32 는 미착수다**

**이 표는 닫힌 것과 안 닫힌 것을 같이 담는다** — 다른 파동 표와 다른 점이고, 그래서
행마다 상태를 적는다. 근거는 2026-09-21 의 설계 검토이고 실측은 클론 4런과 이 템플릿
자신이다.

| # | 성격 | 규모 | 착수 조건 · 닫힘 조건 |
|---|---|---|---|
| 33 | **닫힘 2026-09-21 · [[ADR-H074]]** | 「집계 범위 + 정규식」 둘인 줄 알았는데 **gap 을 지울지 남길지**가 같이 걸렸다 — 방출만 멈추면 아무도 안 내는 어휘가 남는다 | 변이 테스트 6. 실물 `CLAUDE.md` 가 13 → **17** |
| 31 | **닫힘 2026-09-22 · [[ADR-H075]]** | 대상 소멸 — 제출 검사 레지스트리가 9종으로 줄었다 | — |
| 32 | **닫힘 2026-09-22 · [[ADR-H075]]** | 대상 소멸 — 05 가 `submit_checks` 4종을 선언한다 | — |

**33 을 31 보다 앞에 두는 것이 이 표의 유일한 순서 주장이었다** — 33 은 **다음 런의 정보량을
늘린다**(파일별 슬롯 분포가 처음 보인다). 31 도 같은 성질이라 둘 다 런 **전에** 하는 것이
싸다. 32 는 결정이 앞서므로 언제든 뒤다.

**33 은 닫혔다 (2026-09-21 · [[ADR-H074]]).** 규모 추정(`~40줄`)은 대체로 맞았고 갈린 곳은
하나다 — **gap 방출을 멈추는 것만으로는 안 끝났다.** `GAP_DESCRIPTIONS`·`NON_DEMOTING_GAPS`
에 이름을 남기면 **아무도 안 내는 어휘**가 되어, [[ADR-H073]] 이 `produces[].schema` 에서
걷어낸 것과 같은 모양이 된다. **31 · 32 는 미착수 그대로다.**

**파동 3 · 4 에 추가로 배치한다** (2026-09-21) — 아래 두 파동은 **닫힌 적이 없고 계속
받는 칸**이다. 28 본체 · 29 · **34** 는 파동 3(표본 대기), **35** 는 파동 4(결정이 앞선다).

**뒤의 셋도 닫혔다 (2026-09-20 · [[ADR-H071]]).** 이번에는 규모 추정이 대체로 맞았고
갈린 곳은 둘이다 — 24 는 「진입점 맵을 넓힌다」가 아니라 **술어를 하나 더 두는 것**이어야
했고(맵을 넓히면 `missing_entrypoint` 가 계약에 없는 화면을 요구한다), 30 의 「문자열 1개」는
**상수로 올려야** 기계가 말할 수 있었다. 24 는 **타입 전용 export 만큼 열린 채로 남는다** —
그쪽은 결정이 앞선다. 남은 것은 결정 대기(22 · 25 · 27)와 표본 대기(28 · 29)뿐이다.
**그 결정 대기 셋과 24 의 잔여를 파동 6 이 닫았다** (2026-09-21 · [[ADR-H072]]).
...
2. **계약 자체의 결함을 보는 눈** — (a) 닫힘 (2026-09-19 · [[ADR-H050]] 추기) · (b) **닫힘 (2026-09-23 · [[ADR-H076]]) — 대상 소멸**(02 가 없다, [[ADR-H075]]). [[ADR-H050]] 결정 5. (a) gen 체크리스트 「계약이 지시한 재사용 심볼의 정의를 열어 전제를 확인한다」— [[ADR-H059]] 가 넣은 「기존 코드와의 상호작용」이 절반이다. (b) 02 에 계약 초안을 함께 넘기기 — 미착수. 둘 다 모델 판단이라 기계 검증이 없다.
...
20. **닫힘 (2026-09-20 · [[ADR-H070]] 결정 1·2)** — **에스컬레이션 뒤 3라운드가 리뷰어 전원을 다시 부른다** — 클론 4런이 연 것. **결함이었다.**
    05 판정이 `escalate` 경로에서 `rounds_planned[round+1]` 을 쓰기 **전에** `return` 한다. 그러면
    `_planned_for_round` 가 키를 못 찾아 `planned`(전원)로 폴백한다. `run_resume` 도 그 키를 세우지 않는다.
    **산문과 렌더는 반대를 약속한다** — 「수리 뒤 델타 재리뷰는 한 명이다. 전원을 다시 부르지 않는다」.
    덮는 테스트가 없다(`rounds_planned` 픽스처는 `{"2": …}` 뿐). 실측: 에스컬레이션 2런 모두
    `round_reviewers = {"1":4, "2":1, "3":4}` 이고 **r3 수확은 major 0 · critical 0**,
    반면 r2(델타 1인)는 둘 다 major 1건을 잡았다. **좁은 재리뷰는 무는데 넓은 재확인은 안 문다.**
    **닫은 방식**: 델타 계산 두 줄을 `if exceeded:` 앞으로 올리고, `_review_render` 도 그 라운드의
    계획으로 좁힌다. **상태만 고치면 새 결함이 생긴다** — 패킷이 넷을 부르고 `_planned_guard` 가
    셋을 exit 8 로 되돌린다.
21. **닫힘 (2026-09-20 · [[ADR-H070]] 결정 3·4)** — **02 의 `quote` 검사가 페이즈 프롬프트와 다른 문서를 가리킨다** — 클론 4런이 연 것. **결함이었다.**
    `02-cross-verify.md` 는 `quote` 를 **리뷰어 자신의 raw** 부분문자열로 요구하는데 `_record_02` 는
    **`01_plan.md`** 와 대조한다. 프롬프트대로 쓰면 findings 수만큼 실패한다. 실측: 02 가 돈 3런 중
    **2런이 `check_fail {"errors": 5}` → `format_reject` → 재제출**. 곁가지로 02 는 산문이 약속한
    기계 검사 여럿을 실제로 하지 않는다 — `.raw.md` 헤딩 수 대조는 `produces` 에 raw 가 없어 불가능하고,
    `verdict.check_review` 가 호출되지 않아 `mode`·`severity` 어휘가 무검증이며,
    `adopted` 항목이 **아예 없는** finding 은 기본 채택으로 처리된다.
    **잔여 닫힘 (2026-09-21 · [[ADR-H073]])** — 아래 「열린 채로 둔다」 던 둘을 닫았다.
    `submit_checks[].id` 는 `cli.SUBMIT_CHECKS` 어휘가 됐고, `produces[].schema` 는
    가리킬 아티팩트가 없어 **살리지 않고 걷어냈다**. 그 조사에서 **07 의
    `source_quote_substring` 은 선언·산문만 있고 구현이 아예 없던 것**이 드러났다.
    **닫은 방식**: 프롬프트를 검사기에 맞췄다 — raw 는 `produces` 에 없고 primary 가 MCP 라 그
    프롬프트를 못 고치는데 그 도구의 quote 는 플랜에서 나온다. 헤딩 수 약속은 걷어냈다. 어휘는
    `verdict.check_vocabulary` 로 떼어 02·05 가 같이 쓴다. `adopted` 기본 채택은 **동작을 바꾸지
    않고 산문만 맞췄고**, 프론트매터의 `"schema"` 와 `submit_checks[].id` 를 읽는 코드가 없는 것은
    **열린 채로 둔다**.
23. **닫힘 (2026-09-20 · [[ADR-H070]] 결정 5)** — **`rules_read` 해시 대조가 병렬 런을 상정하지 않는다** — 클론 4런이 연 것. **결함이었다.**
    기대 해시를 **제출 시점 디스크에서 다시 계산**한다 — 런 시작 스냅샷이 없다(고정되는 건 요청 텍스트뿐).
    설계 의도는 「규칙이 런 중에 바뀌면 재제출」인데, worktree 병렬 런에서 머지가 `docs/*.md` 를 바꾸면
    같은 반려가 난다. 더 나쁜 건 **`docs/PIPELINE-LOG.md` 가 하네스 자신의 쓰기 대상**이라는 것 —
    `/log` 한 번이 모든 역할의 증명을 무효로 만든다. 검사가 「규칙이 바뀌었다」와 「다른 런이 문서를
    건드렸다」를 **구분하지 못한다.** 실측: 4런 중 2런이 이 사유로 `format_reject`.
    **닫은 방식**: `config.project.rules_exclude` 로 하네스 자신이 쓰는 파일을 집합에서 뺀다.
    **스냅샷 안은 버렸다** — 반려는 「워커가 신고한 해시 ≠ 기대값」이고 워커 해시는 워커가 읽은
    시점의 것이라, 기대값을 init 으로 당기면 반려 창이 `[워커읽기, 제출]` 에서 `[init, 워커읽기]` 로
    **늘어난다.** 패킷 시점으로 내리면 이번엔 진짜 규칙 변경 뒤의 제출이 통과해 닫힘 조건을 깎는다.
    **두 사건을 가르는 것은 여전히 못 한다** — 가를 수단(누가 썼는가)이 코드에 없다.
...
6. **배선 닫힘 (2026-09-19 · [[ADR-H064]])** — **05 `review_repair` 작성자의 모델 등급** — [[ADR-H061]]. `_instruct` 호출은 00·01·03·04 넷뿐이라 05 수리 작성자는 `config.models` 밖이다. 닫힘: `state.models.instructed × counters.repair` 5런 뒤 빈도를 보고 정한다.
    **닫힘 (2026-09-23 · [[ADR-H076]] PR 4 잔여) — 대상 소멸.** 등급 **값**을 정할 `config.models` 가 [[ADR-H075]] 로 지워졌다. 작성자 모델은 `impl-writer` 프론트매터가 정하고, 올릴지는 [[ADR-H075]] 되돌림 조건 3 이 정한다(→ [[ADR-H077]] 이 먼저 올리고 조건 3 을 내리는 방향으로 뒤집었다).
7. **`untested_contract_item` 을 03 이 요구하지 않는다** — [[ADR-H058]]. 원인 미규명 오탐 6/6(§E6) 이 남아 `BASELINE_CHECKS` 유예에만 있다. 닫힘: 오탐 원인 규명 뒤 `required_tests` 에 편입.
    **닫힘 (2026-09-23 · [[ADR-H076]]) — 대상 소멸.** `untested_contract_item` 검사와 `BASELINE_CHECKS` 는 계약 대조 축소로 지워졌다([[ADR-H075]]).
8. **표본 대기 (2026-09-19 재분류 — 템플릿에 원본 없음, 클론 관측 대기)** — **`OTHER` 26관측(폴백의 40%)** — [[ADR-H035]] 한계 4(C6). taxonomy 에 `OTHER: unpromotable` 그대로이고 군집 셋(`search_failed`·`facts_failed` 계열)도 어휘에 없다.
    **닫힘 (2026-09-23 · [[ADR-H076]]) — 대상 소멸.** 통제 어휘(`taxonomy`·`OTHER`)가 지워졌다 — category 는 리뷰어의 자유 태그다([[ADR-H075]]).
9. **닫힘 (2026-09-19 · [[ADR-H023]] 추기)** — **`resolve_ambiguous` 가 deferred 실패에도 flip 인덱스를 올린다** — [[ADR-H023]] 이 "별건" 으로 남긴 것.
10. **파이프라인 우회 측정** — [[ADR-H053]]. 클론 리포에서 `git log --merges` 의 PR 수와 `_workspace/runs/` 의 런 수를 대조한다(ROADMAP §7 의 1). 하네스 밖이다.
11. **비용 계측 0건** — [[ADR-H032]]·[[ADR-H052]]. `cli cost` 는 있으나 파일럿이 걷어냈고 08 표는 「미계측」만 찍는다. **코드는 완비다** — `runtime.read_cost_state` 가 트랜스크립트의 마지막 `cost-state` 레코드를 읽고 `run_cost` 가 세션×런 구간 겹침으로 귀속한다. 훅이 못 채우는 것은 결함이 아니라 설계다: `cost-state` 는 런타임이 트랜스크립트 마지막 줄로 쓰므로 그 세션 자신의 `SessionEnd` 훅은 볼 수 없다(`runtime.py` 주석). 닫힘: `cost-state` 가 실재하는 세션에서 `cli cost --run-id` 를 돌려 값이 나는 것을 확인 — 표본 대기.
    **닫힘 (2026-09-23 · [[ADR-H076]]) — 대상 소멸.** `cli cost`·`read_cost_state`·`run_cost` 가 지워졌다([[ADR-H075]]) — 위 「코드는 완비다」는 더 이상 참이 아니다.
...
24. **닫힘 (2026-09-20 · [[ADR-H071]] 결정 1·2 + 2026-09-21 · [[ADR-H072]] 결정 4)** —
    **타입 전용 export 는 [[ADR-H072]] 가 닫았다**: `type`·`interface` 는 `out_of_contract`
    가 아예 보지 않는다. **`enum` 은 면제가 아니다** — TypeScript 의 `export enum` 은 런타임
    객체를 내보내므로 「계약은 런타임 심볼을 본다」는 사유가 그 자리에서 깨진다. 실측 3건도
    전부 타입 별칭이었다.

    **원문 (범위 부분)** —
    **`out_of_contract` 의 면제가 진입점 파일에만 걸린다** — 클론 4런. [[ADR-H049]] 가 연 면제는
    **작동한다.** 근거는 날짜가 아니라 반례다 — 이 4런이 새 라우트 파일 다섯을 만들었고 그 전부가
    `export const maxDuration` 을 갖는데 **한 건도 `out_of_contract` 로 잡히지 않았다**
    (원장의 `maxDuration` 오탐은 [[ADR-H049]] 직전인 2026-09-16 런이 마지막이고 12/17 런에 올라 있었다).
    남은 것은 **적용 범위**다:
    면제는 `is_entrypoint_file()` 이 참일 때만 걸리는데 `nextjs-ts` 의 진입점 맵은 API route 하나뿐이라
    **`page.tsx` 의 `export const dynamic` 은 여전히 major** 로 잡힌다. 별건으로 **타입 전용 export**
    (`type`·`interface`·`enum`)도 면제가 없다 — 탈출구는 계약 `## 데이터 형태` 에 이름을 적는 것뿐이고,
    이는 01 이 내부 타입까지 계약에 열거해야 한다는 뜻이다. **어느 쪽이 맞는지는 결정이 앞선다.**
    실측: 4런의 `contract-trace` findings 6건 중 5건이 `out_of_contract`, **전부 major · 전부 deferred**
    (`dynamic` ×2 · 타입 별칭 ×3).
    **닫은 방식**: 어댑터가 `entrypoint_resolver.convention_globs` 를 선언하고 코어는
    `contract.is_convention_file` 로 읽는다. **진입점 맵을 넓히지 않았다** — 넣으면
    `missing_entrypoint` 가 계약에 없는 화면을 진입점으로 요구한다. 면제와 실재 확인은 다른 질문이다.
    **남은 것은 타입 별칭 ×3** — 면제할 것인지 계약이 내부 타입까지 열거할 것인지는 여전히 결정이 앞선다.
25. **닫힘 (2026-09-21 · [[ADR-H072]] 결정 2)** — **07 이 구조적으로 빈손이고, 트리아지
    오판정이 그 빈손을 한 번 더 부른다** — 클론 4런.
    **닫은 방식**: `review07.decide()` 의 `triage_miss` 분기를 if/elif 사슬에서 **빼고**
    사슬이 끝난 뒤 돌게 된 런에만 effort 를 올린다. 사슬 안에서 순서만 바꾸면 그 함수의
    docstring 이 경고하는 「구멍을 옮기는 것」이 된다 — 사슬 **밖**이라 그 위험이 없고
    `gaps` 는 사슬보다 앞에서 이미 끝난다. `audit` 보다 앞에 둔다 — 뒤면 감사 런의 `high` 를
    `medium` 으로 내린다. **「외부가 꺼져 있으면 07 을 통째로 건너뛴다」는 안은 버렸다** —
    그러면 `escaped_05` 표본이 영영 0 이 되고 생략 정책의 근거가 사라진다.
    `external_pr_review.enabled` 가 `false` 이고 05 가 수렴하면 07 은 `skip_reason: "clean_05"` 로
    건너뛴다 — **둘 다 빈손인 것이 설계상 보장**이다. 그런데 `triage_miss` 가 난 런에서는
    `review07` 이 07 을 `effort: "medium"` 으로 **강제**한다. 실측: 4런 모두 `external.status: "disabled"`,
    3런 `skipped`, 강제 실행된 1런은 findings 0 에 모델 호출 1회 소모. 07 의 wall 은 4런 합 9.3분(4%).
    **결정할 것**: 외부 봇이 꺼져 있고 `escaped_05 == 0` 이면 페이즈를 통째로 건너뛸 것인가,
    그리고 오판정의 벌칙이 「빈손 보장된 호출 추가」인 것이 맞는가.
26. **닫힘 (2026-09-20 · [[ADR-H071]] 결정 3·4·5)** — **같은 precheck 정책을 05·06 에서 두 번 묻는다**
    — 클론 4런. exit 9 는 상태를 잠그지도 카운터를
    쓰지도 않고 오버라이드 플래그도 없다 — **사람이 「그대로 간다」를 고른 사실이 어디에도 남지 않는다.**
    §E13 이 재개마다 재실행을 요구하므로 06 에서 연속 두 번 묻는 일도 생긴다. 실측: 3런이 05·06
    양쪽에서 exit 9, 그중 1런은 06 에서만 2회. 사람 대기 8.5분 = 4런 전체 대기 28.1분의 **30%**.
    **닫은 방식**: `precheck --ack-policy` 가 판단을 `precheck.policy_override` 와 `policy_acked`
    이벤트로 못박고, 같은 지문이면 06 이 exit 0 + `precheck_policy_override` gap 으로 넘어간다.
    **지문은 내용 해시가 아니라 실패 사유 집합 + 값이다** — 05·06 사이 수리가 내용 해시를 매번 바꾼다.
    **§E13 의 재검사는 그대로다** — 바뀐 것은 검사 여부가 아니라 질문 여부다.
30. **닫힘 (2026-09-20 · [[ADR-H071]] 결정 6)** — **에스컬레이션 선택지가 실제 답을 담지 않는다** —
    클론 4런. 제시 옵션은 셋(계약 결함 의심 ·
    이대로 진행 · 중단)인데 **에스컬레이션 2런 모두 사람이 「기타」를 골랐고**, 그 답은 둘 다
    「좁게 보강하고 진행」 계열이었다. 가장 자주 나오는 답이 메뉴에 없다.
    **닫은 방식**: 05 수리 상한 초과의 선택지를 `cli.REVIEW_ESCALATION_OPTIONS` 상수로 올리고 넷째를
    더했다. 인라인 리터럴이면 「메뉴에 있다」를 기계가 말할 수 없다 — **상수로 올린 것이 검사 지점**이다.
    다른 에스컬레이션 자리(01·02·`run_escalate`)는 건드리지 않았다 — 실측이 05 의 것이다.
...
14. **`cross_verify.secondary` 실연동** — [[ADR-H045]] 결정 4. 예약 필드만 있고 읽는 코드가 0 이며 02 페이즈 파일의 「2차 확인」은 "예약 — 아직 미구현" 이다. 공급자·제출 스키마·비교 판정이 미정이다. 접두부 예산 단위(옛 ROADMAP §7-1 — 덜어내기로 소멸)도 같은 부류다.
    **닫힘 (2026-09-23 · [[ADR-H076]]) — 대상 소멸.** 02 교차검증과 `cross_verify` 가 지워졌다([[ADR-H075]]).
15. **승격 임계 여섯 · 트리아지 임계 셋 · 모델 등급표** — 옛 ROADMAP §7-3·5. 템플릿 실측 0 이라 클론의 런이 판정한다. [[ADR-H054]] 가 15런 값을 적어 뒀다.
    **닫힘 (2026-09-23 · [[ADR-H076]]) — 대상 소멸.** 승격·트리아지·`config.models` 가 지워졌다([[ADR-H075]]). 작성자 모델 값은 [[ADR-H075]] 의 되돌림 조건이 다룬다.
16. **`PreToolUse(matcher Task)` 훅으로 모델 호출 실측** — [[ADR-H020]] 이 "열어만 둔 선택지". 코어 기준이 될 수 없고 상위 기준으로 얹는 별건이다.

22. **닫힘 (2026-09-21 · [[ADR-H072]] 결정 1) — 「결함이 아니라 정확한 신호」로 닫았다** —
    **본문의 사실 주장 정정**: 셋 중 `stage_na:docs` 와 `instruction_slot_over_budget` 은
    **관측 시점에도 이미 비강등이었다** ([[ADR-H047]] 추기 · [[ADR-H056]] 추기,
    `report.NON_DEMOTING_GAPS`). 실제 강등 원인은 `stage_absent:e2e` 하나뿐이다.
    그리고 그 하나를 `not_applicable` 로 올려도 **등급은 안 움직인다** — `self-python` 은
    `compile`·`lint`·`check`·`e2e`·`build` 다섯이 `cmd:null` 이고 04 는 여덟 스테이지를 다 돈다.
    **재라벨링으로 닫을 수 있는 문제가 아니었다.**
    **닫은 방식**: 구조적 부재(`compile`·`build`·`e2e`)만 `not_applicable` 로 올려 gap 이 뜻을
    갖게 하고, `lint`·`check` 의 `stage_absent` 는 **그대로 둔다.** 등급이 매 런 같은 것은
    어댑터가 아직 비었다는 정확한 신호이지 등급 산정의 결함이 아니다. 클론(nextjs-ts)의
    `stage_absent:e2e` 도 같은 이유로 남는다 — 거기서는 러너 미설치이고 그 닫힘은 백로그 17 이다.

    **원문 (2026-09-20 관측)** — **런 내용과 무관한 gap 셋이 등급을 매번 깎는다** — 클론 4런. `stage_absent:e2e` ·
    `stage_na:docs` · `instruction_slot_over_budget` 이 **4/4 런에 붙어** 등급이 언제나
    `PASS_WITH_GAPS` 다. 06 승인에서 사람이 보는 `grade_at_grant` 도 4런 모두 같다 —
    **런마다 같은 값은 판단 근거가 아니다.** 같은 병을 [[ADR-H069]] 가 `adapter_unverified` 에
    대해 한 번 고쳤다(`NON_DEMOTING_GAPS` 로 내려 이름만 남기고, 대신 진짜 경보를 새로 달았다).
    선택지 (a) 셋을 비강등 gap 으로 (b) 어댑터가 `e2e` 를 `not_applicable` 로 선언
    (`_note` 는 `stage_state` 가 세지 않는다) (c) `instruction_slot_budget` 값을 고친다(옛 ROADMAP §7-2).
    **(a) 를 쓰면 「스킵은 통과가 아니다」가 흐려지지 않는지**가 결정의 핵심이다.
27. **닫힘 (2026-09-21 · [[ADR-H072]] 결정 3)** — **04 의 `ambiguous → primary_role`
    기본값이 틀린 역할을 먼저 가리킨다** — 클론 4런.
    **닫은 방식**: 실패 프레임이 하나 이상이고 전부 테스트 파일이면 사다리 맨 앞을 테스트
    역할로 둔다. **사다리만 바꾸면 새 결함이 생긴다** — 배정이 `ladder[len(assigned)]` 라
    위치 기반이고, 같은 sig 가 다른 프레임으로 재발하면(`signature()` 는 프레임을 해시에
    넣지 않는다) 뒤집힌 사다리가 한 역할을 통째로 건너뛴다. 배정을 **미시도 집합**에서
    고르도록 같이 바꿨다 — 지금 코드에서는 두 방식이 같은 답을 내므로 동작을 안 바꾸고
    새 규칙만 안전하게 만든다.
    실패 프레임이 전부 테스트 파일이면 어댑터 귀속 규칙이 판정을 못 내고 `ambiguous` 가 되는데,
    그 기본값이 `impl` 이다. **`ambiguous` 는 예외가 아니라 기본값이다** — [[ADR-H069]] 가
    19런 전수조사로 확인한 「규칙 4종 판정 0/4」가 같은 뿌리다. 실측: 1런에서 스텁 픽스처가 원인인
    AssertionError 를 impl 에 배정 → **impl 이 스텁에 맞추려 계약에 없는 특수 분기를 프로덕션에
    넣었고 사람이 되돌렸다.** 같은 시그니처가 재발해 test 로 넘어가서야 원인이 드러났다 —
    게이트 수리 2라운드가 그 대가다. **결정할 것**: 프레임이 전부 테스트 파일일 때 `test` 를
    먼저 볼 것인가. 오배정의 대가가 「라운드 하나」가 아니라 **「프로덕션 코드 오염」** 이라는 것이
    이 표본의 새 정보다.
...
17. **e2e 러너 도입과 스테이지별 리포트 분리** — [[ADR-H058]] 추기의 도입 순서(ADR → 러너 설치 → 어댑터 `e2e.cmd` → `calibrate --stage e2e` → 계약에 여정 1개)는 프로젝트 몫이고, 그중 `calibrate --stage` 옵션과 `untested_screen`→`skipped` 기록은 이미 구현돼 있어 비어 있는 것은 러너 설치와 어댑터 `e2e.cmd`(세 어댑터 모두 `null`) 뿐이다. 스테이지별 리포트 분리는 도입 ADR 의 코어 항목이다. [[ADR-H057]] 의 `untested_screen` → 여정 커버리지 대체도 같은 시점에 본다.
    **정정 (2026-09-23 · [[ADR-H076]])** — 위 「이미 구현」의 `calibrate --stage`·`untested_screen`·여정은 [[ADR-H075]] 가 지웠다. 남은 것은 어댑터 `e2e.cmd` 도입(프로젝트 몫)이다.
...
28. **리뷰어별 수확 편차가 커서 고정 팬아웃이 낭비인가** — 클론 4런, 표본 부족. 4런 누적
    findings 는 `test` 47 · `data` 20 · `gen` 19 · `sec` 7 · `arch` 4 이고, **major 21건 중 15건(71%)이
    `test`** 이며 그 15건이 전부 `TEST_MISSING_FAILURE_PATH` 다. `sec` 이 낸 유일한 critical 은
    `gen`·`data` 와 같은 결함이었다(1런에서 3인이 같은 것을 각자 발견). 다만 **4런으로 라우팅을
    바꾸지 않는다** — 놓친 것을 세지 못하는 표본이라 `sec`·`arch` 를 조건부로 내리는 근거가 못 된다.
    곁가지로 `review.py` 의 `DEFAULT_CAPS["normal"] = 3` 이 `config.json` 의 `4` 와 어긋나 있다
    (지금은 config 가 이겨서 무해하지만 05 페이즈 파일의 예시는 4 를 전제한다).
    **곁가지 닫힘 (2026-09-21 · [[ADR-H073]] 결정 8)** — 값을 맞추지 않고 **폴백을
    없앴다.** 두 출처를 다시 만드는 것은 결함의 재생산이라([[ADR-H025]]), `_cap` 은
    선언만 읽고 없으면 멈추며 `review.validate` 와 `config.schema.json` 의 `required`
    가 기동 전에 잡는다. **본체(고정 팬아웃이 낭비인가)는 표본 대기로 열린 채다** —
    4런으로 라우팅을 바꾸지 않는다.
    **닫힘 (2026-09-22 · [[ADR-H075]]) — 대상 소멸.** 리뷰어가 하나(`gen`)라 팬아웃도 라우팅도 없다. 놓친 것은 07 의 `dup_05=false` 건수가 센다(ADR-H075 예측 7).
29. **`nothing_locked` 를 리뷰어가 아니라 기계가 잡을 수 있는가** — 클론 4런, 표본 대기.
    원장 236행 중 `resolution: deferred` 가 **168(71%)** 이고 4런 93행 중 **70(75%)** 이다.
    그 4런의 단일 최다 `rule_slug` 는 `nothing_locked` **34건(37%)** — 「테스트가 동작을 잠그지
    않는다」가 findings 의 3분의 1이고 대부분 minor 로 남아 버려진다. **같은 규칙이 이 정도로
    반복되면 산문이나 리뷰어가 아니라 기계 검사로 내려야 한다** — 다만 무엇이 기계로 판정
    가능한지(예: 단언이 자리표시자 문자열에만 걸리는 경우)는 표본이 더 필요하다.
    **닫힘 (2026-09-22 · [[ADR-H075]]) — 대상 소멸.** `test` 리뷰어와 원장이 사라져 `nothing_locked` 표본은 더 쌓이지 않는다. 단 ADR-H075 예측 8(gen 의 `TEST_MISSING_FAILURE_PATH` 류)이 기준선 대비 줄지 않으면 이 처방을 되살린다.
31. **선언된 제출 검사가 «그 런에서 실제로 돌았다»를 기계가 말하지 못한다** — 코드.
    [[ADR-H073]] 이 연 잔여다. `cli.SUBMIT_CHECKS` 가 증명하는 것은 **「그 검사의 구현
    자리가 아직 있다」**뿐이고, 표 밖에서 인라인으로 도는 검사는 여전히 가능하다.
    디스패치 표로는 못 닫는다 — 24개 검사의 시그니처가 전부 다르고 `_record_03` 은
    **검사 순서가 비용 오름차순 설계**라 루프가 그 순서를 평평하게 만든다. 답은
    **런타임 영수증**이고 선례가 이미 있다: `trace_contract` 의
    `out["checks_run"].append(...)`. 닫힘: 각 `_record_XX` 가 자기가 돌린 id 를
    `data["checks_run"]` 에 쌓고, 통과 제출에서 `checks_run` 집합 == 페이즈 선언 집합을
    테스트가 잠근다. 핸들러당 1줄 + 페이즈당 테스트 1개.
    **닫힘 (2026-09-22 · [[ADR-H075]]) — 대상 소멸.** 제출 검사 레지스트리가 9종으로 줄어 「표 밖 인라인 검사」를 영수증으로 셀 실익이 없다.
32. **05 와 08 이 `record` 로 닫히는데 `submit_checks` 를 선언하지 않는다** — 결정.
    [[ADR-H073]] 이 관찰로 남긴 것. 05 는 M46 의 2층(`review.check` · `ledger.append`)이
    따로 있어 검사가 없는 것이 아니라 **선언이 없는 것**이다. 04 는 `gate`, 08 은
    `report` 가 닫으므로 해당 없을 수 있다. **선언을 지금 만들지 않았다** — 어휘가 기계를
    앞서지 않게 하는 것이 그 ADR 자신의 규칙이라, 무엇이 「제출 검사」이고 무엇이 「2층
    검사」인지 가르는 결정이 먼저다.
    **닫힘 (2026-09-22 · [[ADR-H075]]) — 대상 소멸.** 05 가 `submit_checks` 4종을 직접 선언한다(Wave 4). 08 은 `report` 가 닫으므로 해당 없음.
33. **닫힘 (2026-09-21 · [[ADR-H074]])** — **지시문 슬롯 계측이 틀렸다 — 세는 범위도
    방식도**. **닫은 방식**: 재는 집합을 `_rules_read_expected` 와 같게 하고(새로 짤 것이
    없었다 — 그 함수가 이미 올바른 집합을 만든다), 번호 목록도 한 칸으로 세고(실물
    `CLAUDE.md` 13 → **17**), 파일별 수를 `per_file` 로 남기고, **초과 gap 을 어휘째
    걷어냈다.** 방출만 멈추면 아무도 안 내는 어휘가 남아 [[ADR-H073]] 이
    `produces[].schema` 에서 지운 것과 같은 모양이 된다. `instruction_slot_unmeasured`
    는 그대로 gap 이다 — 상수가 아니라 「못 잼」의 신호다. **값 12 는 안 건드렸다**(34).

    **원문** — 코드. **근거가 이미 있어 지금
    짤 수 있다**(파동 8). [[ADR-H056]] 이 만든 `instruction_slot_budget` 의 소비자
    `cli._instruction_slots` 가 셋을 잘못한다. 전부 2026-09-21 에 실측했다:

    | 무엇 | 실측 | 왜 결함인가 |
    |---|---|---|
    | **집계 범위** | `instruction_file`(=`CLAUDE.md`) **하나만** 센다 | `rules_dir` 직속 `*.md` 는 **워커가 전원 매번 읽고 sha256 증명까지** 하는데 한 칸도 안 세어진다. **규칙을 `docs/` 로 옮기면 gap 이 사라지는 착시**가 성립한다 — 계기판을 끄는 것이지 압력을 낮추는 것이 아니다 |
    | **세는 방식** | `_SLOT_BULLET = ^[-*+]\s+` | 번호 목록(`1. `)을 안 센다. 이 템플릿 `CLAUDE.md` 의 「작업 원칙」 **4개가 통째로 빠진다** — 파일 자신이 *"넷 다 기계가 안 잡는 산문"* 이라 적은, **가장 비싼 규칙**이다. 세어진 13칸 + 안 세어진 4칸 |
    | **gap 판정** | `used > budget` 이면 `instruction_slot_over_budget` | 클론 **4/4런 `17/12`**, 이 템플릿 `13/12` — **런 내용과 무관한 상수**다. 매 런 울리는 경보는 경보가 아니고 gap 목록 전체에 둔감해지게 만든다 |

    **닫는 방향** (셋 다 *값이 아니라 측정*을 고치는 것이라 [[ADR-H007]] 에 안 걸린다):
    (1) `_instruction_slots` 를 `_rules_read_expected` 와 **같은 집합** 위에서 돌리고
    **파일별로** 재서 합계를 낸다 — 어느 문서가 비대한지 처음 보인다.
    (2) `^([-*+]|\d+\.)\s+`.
    (3) **gap 을 관측으로 내린다** — `risk_undeclared` 의 전례 그대로다([[ADR-H067]]:
    *"게이트가 아니라 관측만 — 등급·gap·exit 는 건드리지 않는다"*). 숫자는 안 사라진다,
    `report` 가 이미 `"%s/%s (최상위 불릿 / instruction_slot_budget)"` 로 따로 렌더한다.
    **값 12 는 건드리지 않는다** — (1)(2)를 하면 숫자가 확 올라가고, 그 상태의 12 는
    재본 적 없는 값이다(백로그 34).
    **닫힘**: 선언을 바꾸면 숫자가 따라 바뀌는 변이 테스트([[ADR-H025]] 의 함정 —
    "읽는지" 만 보는 단언은 되돌려도 초록이다). ~40줄 + 테스트.
    **펜스 제외는 건드리지 않는다** — 이 파일 기준 0칸이고 의도된 것이다(명령어 예시는 규칙이 아니다).
34. **`instruction_slot_budget` 의 값** — 표본. **백로그 33 이 먼저다.** 12 는 원본
    명세에서 온 **미검증 상속값**이고([[ADR-H007]]) 세는 범위·방식이 틀린 상태에서 정하면
    **틀린 숫자에 맞추는 것**이 된다. 33 뒤에 클론 N런의 `instruction_slots` **파일별 분포**를
    보고 정한다 — *"`CLAUDE.md` 는 안정적인데 `PRD.md` 가 계속 자란다"* 같은 것이 그때
    처음 보인다. 선택지는 (a) 값을 올린다 (b) 규칙을 줄인다 (c) **규칙 자체를 폐기한다**.
    옛 ROADMAP §7-2 가 같은 것을 열린 질문으로 들고 있었다.
    **닫힘 (2026-09-22 · [[ADR-H075]]) — 대상 소멸.** `instruction_slot_budget` 자체가 Wave 2 에서 사라졌다 — 값을 정할 대상이 없다.
35. **워커에게 읽을 규칙 파일을 역할별로 지정한다** — 결정 + 표본. 지금은
    `_rules_read_expected(root, config)` 에 **`role` 인자가 없어** 전원이 같은 집합을 읽는다:
    `CLAUDE.md` + `docs/` 직속 `*.md` 6개 = **8,306자를 역할 2~3명이 매번 전문으로**
    (2026-09-21 실측, `docs/` 가 빈 골격인 상태의 숫자다 — 채워지면 커진다).

    **「필요할 때만 읽기」를 워커에게 맡길 수는 없다** — *"필요 없다"* 는 판단을 **규칙을
    모르는 상태에서** 내리게 되고(`impl` 이 폼을 만들며 *"UI 가이드는 내 일이 아니다"* 로
    판단하는 순환), 그 판단은 어디에도 안 남는다. 그것이 이 하네스가 최악으로 치는 실패다.
    **답은 판단 주체를 실행기로 옮기는 것**이다 — 패킷이 역할별로 **결정론으로 정한 목록**을
    주고 워커는 **받은 것만** 읽고 증명한다. 그러면 총량 감소·증명 유지·순환 회피가 함께 선다.

    **하네스는 이미 이 규율을 다른 곳에서 쓴다** — 05 는 리뷰어를 워커가 안 고르고
    `review.route()` 가 글롭으로 정하며, 07 은 *"네가 effort 를 고르면 같은 상황이 런마다
    다른 리뷰를 받는다"* 고 못 박는다. **규칙 문서만 이 규율에서 빠져 있다** — 패킷이
    목록이 아니라 디렉터리 패턴(`{rules_dir}/ 의 직속 .md`)을 준다.

    **그런데 결정이 둘 앞선다**: (a) 매핑 축이 **역할인가 계약 내용인가** — `impl` 이
    `UI_GUIDE.md` 를 읽어야 하는지는 역할이 아니라 **그 계약에 화면이 있는가**에 달렸을 수
    있다. (b) **가른 뒤 놓친 것을 셀 수단이 없다** — 05 는 `routing.dropped` 와 `escaped_05`
    로 재는데 규칙 쪽엔 대응물이 없다. **백로그 33 이 먼저다** — 파일별 숫자가 보여야
    "어느 문서가 어느 역할에 안 필요한가" 를 논할 근거가 생긴다.
    **닫힘 (2026-09-22 · [[ADR-H075]]) — 대상 소멸.** 역할이 하나라 역할별 목록이 없다. 「읽을 곳」은 03 패킷의 세 줄(지시 파일 · `docs/` 직속 · 계약, 화면이면 `UI_GUIDE.md`)이고 `rules_read` 증명은 없다.
36. **어댑터·config 를 한 명령 안에서 다시 읽는다** — 표본. [[ADR-H076]] PR 4 가 「남은 결함」으로 산문에만
    남긴 것이다(번호 없는 잔여 — 이 목록의 규칙 위반이었다). `gate` 는 `check_requires` →
    `_req_adapter_stage` 와 `_run_gate_cmd` 에서 두 번, `pr` 은 승인 없는 경로의 `pc.run` 에서
    한 번 더, `run_pr` 의 `build_context` 는 `config` 를 안 넘겨 `config.json` 을 한 번 더 읽는다.
    **닫힘**: 첫 실물런에서 06 앞 단계·04 게이트 호출의 비모델 소요가 런 wall 의 수 % 이상이면
    고친다(각 3~8줄). 아니면 PR 4 결정 1(`append_event` 재읽기)과 같이 둔다 — 실측 없는 최적화는 하지 않는다.
37. **`pr` 1회에 `secret_values` 가 두 번 돈다** — 표본. [[ADR-H076]] PR 4 잔여. 부재 목록용
    호출과 `build_body` → `mask_text` 의 호출이 비밀 파일을 두 번 읽는다(어댑터는 안 읽는다).
    **닫힘**: 36 과 같이 판정한다.
38. **유닛×파일 전수 읽기** — 표본. [[ADR-H076]] PR 4 잔여. `pr._verified_lines` 는 유닛마다
    테스트 파일을, `contract._tests_for_source`·`trace_contract._tests_for_entrypoint` 는 항목마다
    테스트 파일 전부를, `trace_contract._found_anywhere` 는 오류 이름마다 소스 전부를 읽는다
    (`_tests_for_entrypoint` 는 PR 4 목록에 없던 것 — 2026-09-23 조사). 비용은 리포 크기 × 유닛 수라
    작은 템플릿에서는 안 보인다. **닫힘**: 첫 실물런의 `05_trace.json` 산출(contract-trace)과 06 `pr`
    소요가 체감되면 공유 텍스트 캐시 하나로 닫는다(10~15줄). 아니면 둔다.
...
| ADR | 볼 것 | 표본 |
|---|---|---|
| [[ADR-H041]] | `run.rounds` 가 상한(`{fix: 1, normal: 2}`)에 닿는 빈도 | 첫 3런 |
| [[ADR-H044]] | **대상 소멸** — `00_triage.json` 과 `triage_miss` 이벤트 | 첫 3런 |
| [[ADR-H046]] | `dispatched_fp` exit 3 빈도 — `receipt_stale` 이벤트의 `receipt: dispatch` 로 센다(2026-09-23 전에는 이벤트가 없어 셀 수 없었다) | 다음 파일럿 3런 |
| [[ADR-H047]] | `tests.status: shrank` 가 실제로 잡히는가 | 다음 파일럿 |
| [[ADR-H049]] | **대상 소멸**(원장) — `contract-trace` 출처 비율이 41/140 아래로 내려가는가 | 다음 파일럿 3런 |
| [[ADR-H053]] | `fix` 레인 실물 런 | 아직 0 |
| [[ADR-H059]] | **대상 소멸**(07 은 `dup_05` 로 잰다) — 07 생략 뒤 `escaped_05` | 다음 감사 런 2회 |
| [[ADR-H060]] | **대상 소멸** — 02 생략 사유 분포와 `no_risk` 런의 05·07 지적 | 첫 5런 |
| [[ADR-H061]] | **부분 소멸**(`instructed` 없음, 모델은 자진신고) — `state.models.instructed × counters.repair` | 5런 |
| [[ADR-H062]] | **대상 소멸** — `routing.dropped` 에 `arch` 가 오르는 빈도와 그 런의 `escaped_05` 구조 지적 | 다음 파일럿 3런 |
| [[ADR-H064]] | **부분 소멸**(등급 슬롯 없음) — 05 수리 지시(`05:r{n}:repair:{role}`)의 등급과 `counters.review_repair` | 5런 ([[ADR-H061]] 과 같이) |
| [[ADR-H065]] | **대상 소멸** — 승격 자체 게이트의 `failed` 가 규칙 결함인지, 기능 브랜치에서 부른 탓인지 | 첫 실물 `applied` 승격 |
| [[ADR-H066]] | **부분 소멸**(파일 수 초과는 exit 9 가 아니라 정보 행) — `precheck` exit 9 빈도와 그 런이 소스 파일로 넘었는가(`test_files_excluded`) | 클론의 첫 5런 |
| [[ADR-H067]] | **대상 소멸** — `risk_undeclared` 이벤트 빈도와 그 런의 05·07 지적 — 게이트 승격 여부 | 첫 5런 |
| [[ADR-H068]] | **대상 소멸** — worktree 에서 커밋한 세션이 `worktrees[].commits` 로 잡혔나, 들여다보기만 한 세션의 칸이 비었나 | 클론의 첫 `/log` 3회 |
| [[ADR-H069]] | **대상 소멸** — `attribution_rules` 가 실제로 채워지는가 · `attribution_unparsed` 가 한 번이라도 잡히는가 | 클론의 다음 5런 |
| [[ADR-H075]] | 예측 8개와 되돌림 조건 넷 — 표·기준선·채점 규칙의 정본은 [PILOT-LOG](PILOT-LOG.md) 「첫 실물런 전 예측」 | 첫 실물런 |
| [[ADR-H076]] | (a) 05 수리 런의 full 1회 wall — `gate_stages` 의 phase 05 · selector full `sec` (b) `trace_repair` 소모 — 08 카운터 (c) 영수증 거부 — `receipt_stale` 이벤트 수(`receipt` 별) | 첫 실물런 |
| [[ADR-H077]] | 08 「자진신고 모델」이 `claude-opus-5-5` 인가(리뷰어만 — 작성자는 blind spot) · 예측 1·4·5 · 사람이 보는 사용량 | 첫 실물런 |
| [[ADR-H078]] | 06 `waiting_human(approval)` 이 0 인가 · 자동 승인으로 나간 PR 을 사람이 닫은 일이 있는가 | 첫 실물런 |
| [[ADR-H079]] | `01_plan.md` 가 스펙의 AC 마다 절을 대응시켰나 · 07 escaped 중 AC 미충족 건수 | 스펙 요청 첫 런 |

**정정 (2026-09-23 · [[ADR-H076]])** — 위 「전부 기록 배선이 있다」는 덜어내기 뒤 참이 아니다. `대상 소멸` 행은 볼 것이 없고, `부분 소멸` 행은 괄호 밖만 잰다.
...
**번호 없이 산문으로 남긴 잔여는 잊힌다** (2026-09-21 · [[ADR-H073]]). [[ADR-H070]] 이 백로그 21 을 닫으며
"읽는 코드가 없는 것은 열린 채로 둔다" 고만 적고 번호를 주지 않았고, 그래서 실행 순서표에 오르지 못했으며
파동 6 이 「결정 대기가 비었다」고 셀 때도 세어지지 않았다. **미룬 것은 여기서 번호를 받는다** — ADR 본문의
"열린 채로 둔다" 한 문장은 기록이지 대기열이 아니다.
...
**추기 (2026-09-21 · [[ADR-H072]] 결정 3)**: 이 결정이 연 사다리는 이제 **순서가 고정이 아니다** —
실패 프레임이 전부 테스트 파일이면 맨 앞이 테스트 역할로 바뀐다. 그래서 배정을
`ladder[len(assigned)]` 대신 **미시도 집합의 첫 역할**로 골라야 한다. 위치로 고르면 뒤집힌
사다리가 한 역할을 통째로 건너뛰어 이 ADR 이 막으려던 M33 의 모양이 그대로 돌아온다.
...
**추기 (2026-09-21, [[ADR-H073]])**: 이 결정이 `loop` 어휘에만 걸려 있었다.
`submit_checks[].id`(24종) · `produces[].schema`(10종) · `review.profile_caps` 의
폴백이 **같은 병을 그대로 앓고 있었고**, 여기 적힌 "폴백이 곧 새 하드코딩" 과
"변이 테스트여야 문다" 가 그때도 그대로 답이었다. [[ADR-H073]] 이 셋에 이 원칙을
적용했다 — 다만 `schema` 는 가리킬 대상 자체가 없어 **살리지 않고 걷어냈다**.
그 조사에서 07 의 `source_quote_substring` 은 **선언과 산문만 있고 구현이 아예
없던 것**이 드러났다. 이 결정의 "지우지 않고 읽는다" 가 항상 답은 아니다 —
**읽을 것이 실재하는가를 먼저 묻는다.**

관련: [[ADR-H023]] · [[ADR-H073]](같은 원칙을 남은 선언들에 적용)
...
**추기 (2026-09-21 · [[ADR-H072]] 결정 2)**: `triage_miss` 의 벌칙이 「07 을 `medium` 으로
강제한다」에서 「**생략되지 않은** 런의 effort 를 올린다」로 바뀌었다. 클론 4런 중 1런에서
이 벌칙이 외부 `disabled` · 05 수렴인 런의 07 을 불러 findings 0 에 모델 호출 1회를 태웠다 —
빈손이 **보장된** 호출이다. 예측이 빗나간 사실은 gap 으로 그대로 남는다.
...
**추기 (2026-09-21, [[ADR-H074]] — 계측 정정)**: `_instruction_slots` 가 셋을 잘못 재고
있었다. ⑴ `instruction_file` **하나만** 세어 `rules_dir` 직속 `*.md` 가 빠졌다 — 워커가
**전원 매번 읽고 증명까지** 하는 파일들이라, 규칙을 그쪽으로 옮기면 **예산이 비는 착시**가
성립했다. ⑵ 정규식이 번호 목록을 안 세어 이 리포 `CLAUDE.md` 의 「작업 원칙」 **넷이 통째로**
빠졌다(13 → **17**). ⑶ 초과 gap 이 클론 4/4런 같은 값이라 **런 내용과 무관한 상수**였고,
[[ADR-H074]] 가 그것을 **관측으로 내리고 어휘째 걷어냈다.** **예산의 취지와 값은 그대로다** —
바뀐 것은 재는 법뿐이고, 값 12 의 결정은 미구현 백로그 34 다.

관련: [[ADR-H033]] · [[ADR-H034]](축은 `rule_key`) · [[ADR-H049]] · [[ADR-H051]](skip 에는 사유) ·
[[ADR-H052]] · [[ADR-H055]] · [[ADR-H074]](계측 정정)
...
**추기 (2026-09-21 · [[ADR-H072]] 결정 2)**: 결정 1 이 적은 사슬에서 `triage_miss` → medium 이
빠졌다. 그 벌칙은 이제 사슬 **밖**에서, 생략되지 않은 런에만 effort 를 올린다. 남는 순서:
`review05 != ok` → medium / 켜 놓은 봇 결손 → low / `docs` → skipped / `fix` → skipped(0건이면 low) /
`small` + reviewed → skipped / 외부 Major → low / `findings_total == 0` → low / 그 밖 → skipped.
...
### ADR-H070: 클론 4런이 연 결함 셋 — 델타 재리뷰 · 02 인용 대조 · 규칙 집합

**날짜**: 2026-09-20 · **상태**: 채택됨 · **구현 상태**: 구현됨 (2026-09-20 — `cli._judge_05` 의
델타 계산 위치와 `cli._review_render` 의 좁히기, `verdict.check_vocabulary` 와 02 의 호출,
`config.project.rules_exclude` 와 `cli._rules_excluded`)

**맥락**: 미구현 백로그 **20 · 21 · 23** 이다. 셋 다 클론 4런이 연 것이고 **미룬 결정이 아니라 결함**이다.
넷 중 형식 교정 왕복 4회가 전부 이 셋에서 나왔다 — 02 `format_reject` 2 · 03 `format_reject` 2,
**넷 다 모델의 실수가 아니었다**. ROADMAP §7-4 가 「왕복을 세기 전에 왕복을 만드는 쪽을 고쳐야 계수가
뜻을 갖는다」고 적은 자리다. 셋을 한 파동으로 묶은 이유는 셋 다 코드가 작고 **다음 런 하나로 고쳐졌는지
기계가 말하기** 때문이다.

**결정**:

1. **델타는 에스컬레이션보다 앞에서 정한다** (백로그 20). 05 판정의 `escalate` 분기가
   `rounds_planned[round+1]` 을 쓰기 **전에** return 해서 재개된 라운드가 `_planned_for_round` 의
   폴백(전원)을 받았다. 두 줄을 `if exceeded:` 앞으로 올린다. 사람이 「이대로 진행한다」를 골라
   돌아와도 수리 대상은 같으므로 델타도 같다.
2. **봉투도 그 라운드의 계획만 이름 짓는다** (백로그 20, 같은 결함의 나머지 절반). `_review_render` 는
   `routing` 전원을 나열하고 `_planned_for_round` 를 보지 않았다. 상태만 고치면 패킷이 넷을 부르고
   `_planned_guard` 가 셋을 exit 8 로 되돌린다 — **봉투와 검사가 같은 것을 봐야 한다**(M38).
   라우팅에 없는 코드가 계획에 오르면(`next` 가 여러 번 불려 집합이 줄어든 경우) 좁히지 않는다.
3. **02 의 인용 대조는 플랜 원문이고, 프롬프트를 그쪽에 맞춘다** (백로그 21). 페이즈 파일은 리뷰어
   자신의 `.raw.md` 를 요구했고 `_record_02` 는 `01_plan.md` 와 대조했다. **검사기 쪽이 맞다** —
   02 의 `produces` 에 raw 가 없어 raw 대조는 기계가 할 수 없고, primary 가 MCP 도구라 그 프롬프트를
   우리가 못 고치는데 그 도구는 플랜 텍스트를 받아 리뷰하므로 quote 가 플랜에서 나온다.
   같은 이유로 **「헤딩 개수와 findings 개수가 같아야 한다」 약속을 걷어낸다** — 받지 않는 파일을
   검사한다고 적으면 선언만 있고 코드가 안 읽는 것이 된다.
   `.claude/agents/plan-reviewer.md` 는 01 과 공유라 통째로 바꾸지 않고 **02 분기 한 절**을 더한다.
4. **어휘 검사는 떼어내 02 와 05 가 같이 쓴다** (백로그 21 곁가지). `verdict.check_review` 를 02 가
   통째로 부를 수는 없다 — `raw_text` 를 필수로 받아 quote 와 헤딩 개수를 대조하므로 `01_plan.md` 를
   넘기면 헤딩 0개라 findings 가 하나만 있어도 전 제출이 exit 8 이다. `check_vocabulary` 로
   `reviewer`·`mode`·`primary_error`·`severity` 만 분리한다.
5. **반려의 대상 집합을 줄인다 — 스냅샷을 쓰지 않는다** (백로그 23). `config.project.rules_exclude` 를
   더하고 기본값에 `docs/PIPELINE-LOG.md` 를 둔다. `rules_read` 기대 집합과 `_instruction_destination`
   이 **이 목록만** 공유한다 — 두 판정의 나머지는 의도적으로 다르다(하나는 존재로, 하나는 경로로 가른다).

**트레이드오프**:

- 결정 5 는 **처음에 「런 init 스냅샷으로 기대 해시를 고정한다」로 설계했다가 뒤집은 것이다.** 반려는
  「워커가 신고한 해시 ≠ 기대값」이고 워커 해시는 **워커가 파일을 읽은 시점**에 계산된다. 기대값을 init
  으로 당기면 반려 창이 `[워커읽기, 제출]` 에서 `[init, 워커읽기]` = 00~03 전체로 **늘어난다.**
  패킷 시점(`_store_dispatch`)으로 내리면 창은 짧아지지만, 이번엔 **규칙이 진짜 바뀐 뒤 낸 제출이
  통과하게 되어** 백로그 23 이 적은 닫힘 조건(「규칙이 진짜 바뀐 경우에는 여전히 반려」)을 약화시킨다.
  스냅샷은 창을 닫지 않고 옮길 뿐이므로 **넣지 않는다.**
- 그래서 이 수정은 「규칙이 진짜 바뀌었다」와 「다른 런이 문서를 건드렸다」를 **여전히 구분하지 못한다.**
  가르는 수단(누가 썼는가)이 코드에 없다. 이번에 갈라내는 것은 **하네스 자신의 쓰기** 하나뿐이고,
  그것이 실측 최악의 사례(`/log` 한 번이 모든 역할의 증명을 무효로)를 덮는다. 병렬 런의 머지가 규칙
  문서를 바꾼 경우는 규칙이 실제로 바뀐 것이므로 반려가 맞다.
- 결정 3 은 **02 가 리뷰어 원문을 기계로 검사하지 않게 된다**는 뜻이다. 「JSON 은 그럴듯한데 원문에는
  없는 지적」을 잡는 손잡이가 02 에서는 플랜 인용 하나로 줄어든다. 되살리려면 `produces` 에 `.raw.md` 를
  더하는 별건이고, 그때는 `lint-phases` 의 `produces_key` 유일성까지 본다.
- 결정 2 는 사람이 「좁게 보강하고 진행」류를 골라 **일부러** 전원을 다시 부르고 싶은 경우를 막는다.
  지금 에스컬레이션 메뉴에 그 선택지가 없고(백로그 30) 상태에 그 의사를 적을 칸도 없다.
  **추기 (2026-09-20 · [[ADR-H071]] 결정 6)**: 메뉴에는 생겼다. **상태에 적을 칸은 여전히 없다** —
  답은 `resume --answer-file` 의 자유 텍스트로만 남고, 그것을 읽어 재리뷰 범위를 넓히는 코드는 없다.

**재검토 시점**: 클론의 다음 런 하나. 02 `check_fail` 0회 · 03 `format_reject` 0회 ·
`round_reviewers["3"].planned == 1` 셋을 본다. 그 런이 돌면 ROADMAP §7-4 의 리뷰어 호출 고정비를
처음으로 뜻있게 셀 수 있다. 03 `format_reject` 가 그래도 나면 실측 2런의 사유가 `/log` 가 아니라
병렬 런 머지였다는 뜻이고, 그때는 「누가 썼는가」를 가를 수단부터 설계한다.

곁가지로 **02 프론트매터의 `"schema": "verdict"` 와 `submit_checks[].id` 를 읽는 코드가 없다** —
같은 「선언만 있고 코드가 안 읽는」 모양이지만 이번 범위 밖으로 둔다. `adopted` 항목이 아예 없는
finding 이 기본 채택으로 처리되는 것(`_has_adoption` 이 blocking 에만 걸린다)도 동작을 바꾸지 않고
페이즈 산문만 맞췄다.

관련: [[ADR-H055]](`rules_read` 를 연 결정) · [[ADR-H045]](02 가 유일한 xv 호출처) ·
[[ADR-H046]](지시 시점 지문) · [[ADR-H048]](`review_repair` 예산과 에스컬레이션)

---

### ADR-H071: 클론 4런이 연 마찰 셋 — 관례 파일 면제 · precheck 재질문 · 에스컬레이션 메뉴

**날짜**: 2026-09-20 · **상태**: 채택됨 · **구현 상태**: 구현됨 (2026-09-20 —
`contract.is_convention_file` 과 어댑터 `convention_globs`, `precheck.policy_fingerprint` ·
`override_covers` 와 `cli._apply_policy_override`, `cli.REVIEW_ESCALATION_OPTIONS`)

**맥락**: 미구현 백로그 **24 · 26 · 30** 이다. [[ADR-H070]] 이 닫은 셋과 달리 **결함이 아니라
마찰**이고, 셋 다 「사람이나 모델이 한 번 더 일하게 만드는 자리」다. 실측은 클론 4런이다 —
24: `contract-trace` findings 6건 중 5건이 `out_of_contract` major 에 **전부 deferred**.
26: 사람 대기 8.5분 = 4런 전체 대기 28.1분의 **30%**, 3런이 05·06 양쪽에서 exit 9.
30: 에스컬레이션 2런 **모두** 사람이 「기타」를 골랐다.

**결정**:

1. **관례 파일 면제는 진입점 맵 밖으로도 걸린다** (백로그 24). [[ADR-H049]] 의 면제는 작동하지만
   `is_entrypoint_file()` 이 참일 때만 걸리고 `nextjs-ts` 의 진입점 맵은 API route 하나뿐이라
   화면 파일의 관례 export 가 계속 major 였다. 어댑터에 `entrypoint_resolver.convention_globs` 를
   더하고 코어는 `contract.is_convention_file` 로 읽기만 한다.
2. **`is_entrypoint_file` 의 뜻은 넓히지 않는다** (백로그 24, 같은 결정의 나머지 절반). 화면 파일을
   진입점 맵에 넣으면 `missing_entrypoint` 가 **계약에 없는 화면을 진입점으로 요구**한다.
   면제와 실재 확인은 다른 질문이므로 술어를 둘로 둔다. `implied_exports` 에는
   `metadata`·`generateMetadata`·`generateStaticParams`·`viewport` 를 더한다 — 이름은 어댑터에만 있다.
3. **사람이 고른 precheck 판단을 지문과 함께 적는다** (백로그 26). exit 9 는 상태를 잠그지도 카운터를
   쓰지도 않아 **사람이 「그대로 간다」를 고른 사실이 어디에도 남지 않았다.** `precheck --ack-policy` 가
   그것을 `precheck.policy_override` 와 `policy_acked` 이벤트로 못박고, 06 이 같은 지문이면 다시 묻지
   않는다. §E13 의 「재개마다 다시 검사한다」는 그대로다 — **바뀌는 것은 검사 여부가 아니라 질문 여부다.**
4. **지문은 실패 사유 집합과 값이다** (백로그 26, 설계 선택). 소유 범위 파일의 내용 해시
   (`state.fingerprint`, [[ADR-H046]] 와 06 승인이 쓰는 것)를 쓰지 않는다 — 05 와 06 사이에는 수리가
   들어가 거의 매번 달라지고, 그러면 닫힘 조건(「런당 1회 이하」)이 성립하지 않는다. 사람이 판단한 것은
   코드의 바이트가 아니라 **무엇이 얼마나 넘었는가**다. 값이 커지거나 새 사유가 붙으면 그것은 사람이
   본 적 없는 범위이므로 다시 묻고, 작아진 것은 덮는다.
5. **넘어간 것은 통과가 아니다** (백로그 26). `precheck_policy_override` 를 gap 으로 남기고
   **`NON_DEMOTING_GAPS` 에 넣지 않는다** — 예산을 사람 허락으로 넘은 것은 관측 결손이 아니라 이 런의
   사실이고, 등급이 그것을 말해야 한다 ([[ADR-H027]]). [[ADR-H069]] 가 `adapter_unverified` 를 비강등으로
   내린 것과 **반대 방향**인 이유가 여기다.
6. **에스컬레이션 메뉴에 가장 자주 나온 답을 넣는다** (백로그 30). 05 수리 상한 초과의 선택지를
   `cli.REVIEW_ESCALATION_OPTIONS` 상수로 올리고 「좁게 보강하고 진행한다」를 더한다. 분기하는 코드는
   없고 사람이 읽는 문자열이라 **상수로 올린 것 자체가 검사 지점**이다 — 아니면 「메뉴에 있다」를 기계가
   말할 방법이 없다.

**트레이드오프**:

- 결정 1·2 는 **타입 전용 export**(`type`·`interface`·`enum`)를 덮지 않는다. 클론 4런의
  `out_of_contract` 5건 중 **3건이 이쪽**인데, 면제할 것인지 계약 `## 데이터 형태` 가 내부 타입까지
  열거할 것인지는 **결정이 앞선다**(백로그 24 본문). 여기서 정하지 않고 백로그 24 를 그 몫만큼 열어 둔다.
  **추기 (2026-09-21 · [[ADR-H072]] 결정 4)**: 닫혔다 — `type`·`interface` 는 면제하고 `enum` 은
  하지 않는다. 셋을 한 묶음으로 부른 이 문장이 틀렸다: `export enum` 은 런타임 객체를 내보낸다.
- 결정 3 은 **새 명령 손잡이를 하나 늘린다.** `resume --ack` 로 대신할 수 없었다 — 그것은 `escalated`
  상태를 전제하는데 정책 exit 9 는 런을 잠그지 않는다. 잠그게 바꾸는 쪽이 손잡이는 줄지만 exit 9 가
  exit 10 과 같아져 실패 3분류가 흐려진다.
- 결정 4 는 **같은 파일 수·줄 수 안에서 내용이 완전히 달라진 변경을 덮는다.** 사람이 본 것은 범위이지
  내용이 아니므로 의도한 것이지만, 「12파일을 승인했더니 다른 12파일이 왔다」를 이 지문은 못 가른다.
  가르려면 결정 4 가 기각한 내용 해시가 필요하고 그러면 재질문이 돌아온다.
- 결정 6 은 [[ADR-H070]] 트레이드오프가 적은 구멍(「사람이 일부러 전원을 다시 부르고 싶은 경우」)을
  **메뉴에만** 연다. 그 의사를 상태에 적는 칸은 여전히 없다 — 답은 `resume --answer-file` 의 자유
  텍스트로만 남는다.

**재검토 시점**: 클론의 다음 런 하나. 24 는 `contract-trace` 의 `out_of_contract` 가 프레임워크 관례
export 를 **0건** 잡는가, 30 은 에스컬레이션에서 사람이 **「기타」를 고르지 않는가**로 본다 — 둘 다 이
리포의 테스트로는 닫히지 않고 런이 판정한다. 26 만 테스트로 닫힌다(`waiting_human
{"reason":"precheck_policy"}` 런당 1회 이하). 26 이 닫힌 뒤 대기 시간이 줄었는지는 08 의
`human_wait_sec` 이 말한다 — 줄지 않으면 대기의 주범이 precheck 가 아니라 06 승인이라는 뜻이다.

관련: [[ADR-H049]](관례 면제를 연 결정) · [[ADR-H031]]·[[ADR-H038]](스택 지식은 어댑터에) ·
[[ADR-H027]](면제는 통과가 아니다) · [[ADR-H052]](exit 9 와 `waiting_human`) ·
[[ADR-H069]](비강등 gap 의 반대 사례) · [[ADR-H070]](같은 4런이 연 결함 셋)

---

### ADR-H072: 결정이 앞섰던 넷 — 등급 신호 · 07 빈손 호출 · 귀속 기본값 · 타입 전용 export

**날짜**: 2026-09-21 · **상태**: 채택됨 · **구현 상태**: 구현됨 (2026-09-21 —
`self-python` 어댑터의 `stages.{compile,build,e2e}.not_applicable`,
`review07.decide` 의 사슬 밖 effort 승격, `attribution._frames_all_tests` 와
`resolve_ambiguous(adapter=)` · `dispatch(adapter=)` 의 미시도 집합 배정,
`trace_contract.TYPE_ONLY_KEYWORDS` 와 `_DEFAULT_PUBLIC` 의 `kw` 그룹)

**맥락**: 미구현 백로그 **22 · 24(잔여) · 25 · 27** 이다. [[ADR-H070]](결함 셋)·
[[ADR-H071]](마찰 셋)과 달리 넷 다 **사람이 값이나 방향을 골라야 진행되던 것**이고,
실측은 같은 클론 4런이다. 표본 대기 쪽(28 · 29)은 클론에 새 런이 없어 지금 못 연다 —
막힌 것은 코드가 아니라 런이다.

**탐색이 맥락을 하나 고쳤다.** 백로그 22 본문은 gap **셋**이 등급을 깎는다고 적었는데,
`stage_na:docs` 와 `instruction_slot_over_budget` 은 **관측 시점에도 이미 비강등**이었다
(`report.NON_DEMOTING_GAPS`, 각각 [[ADR-H047]]·[[ADR-H056]]). 클론이 옛 코드를 쓴 것도
아니다 — `instruction_slot_over_budget` 의 등재는 2026-09-18 이고 4런은 09-19~20 이다.
실제 강등 원인은 `stage_absent:e2e` **하나**였다.

**결정**:

1. **구조적 부재만 `stage_na` 로 올리고, 미룬 부재는 등급을 깎게 둔다** (백로그 22).
   `self-python` 의 `compile`·`build`·`e2e` 에 `not_applicable` 을 선언한다 — 셋 다 어댑터의
   `_note` 가 이미 "파이썬에 컴파일 단계가 없다"·"빌드 산출물이 없다"·"E2E 개념이 없다" 고
   적고 있었다. **산문이 말하던 것을 기계가 말하게 하는 것**이고 [[ADR-H047]] 추기가 `docs` 에
   한 것과 같다. `lint`·`check` 는 **건드리지 않는다.**
   **이 결정은 「등급이 매 런 같다」를 닫지 않는다 — 문제를 다시 정의한다.** `self-python` 은
   `cmd:null` 이 다섯이고 04 는 여덟 스테이지를 다 도므로 e2e 하나를 올려도 `stage_absent` 가
   넷 남는다. 재라벨링으로 닫을 수 있는 문제가 아니었다. **등급이 매 런 같은 것은 등급 산정의
   결함이 아니라 어댑터가 아직 비었다는 정확한 신호다.** 바뀌는 것은 등급의 변동성이 아니라
   **gap 이 뜻을 갖는가**이고, 실제 닫힘은 어댑터를 채우는 쪽(백로그 17 · lint·check 도입)이다.
2. **트리아지 오판정의 벌칙은 생략을 이기지 못한다** (백로그 25). `review07.decide()` 의
   `triage_miss` 분기를 if/elif 사슬에서 빼고, 사슬이 끝난 뒤 **돌게 된 런에만** `low` 를
   `medium` 으로 올린다. 외부 봇이 `disabled` 이고 05 가 수렴한 런에서 07 은 **빈손이
   보장**돼 있는데(설계상 그렇다), 벌칙이 사슬 맨 앞에 있어 그런 런까지 한 번 더 불렀다 —
   클론 4런 중 1런이 findings 0 에 모델 호출 1회를 태웠다.
   **사슬 안에서 순서만 바꾸지 않는다.** `decide()` 의 docstring 이 경고하듯 분기 재배치는
   고치는 것이 아니라 **구멍을 옮기는 것**이다(G-5). 사슬 **밖**에 두면 그 위험이 없다 —
   `gaps` 계산은 사슬보다 앞에서 이미 끝난다. `audit` 블록 **앞**에 둔다: 뒤면 감사 런의
   `high` 를 `medium` 으로 내려 [[ADR-H061]] 이 막으려던 과소측정이 된다.
3. **프레임이 전부 테스트 파일이면 테스트 역할이 사다리 맨 앞이다** (백로그 27).
   `resolve_ambiguous` 가 `adapter` 를 받아 `is_test_file` 로 묻는다. **`ambiguous` 는 예외가
   아니라 기본값**이고([[ADR-H069]] 의 19런 전수조사가 같은 뿌리를 확인했다 — 규칙 4종 판정
   0/4), 스텁 픽스처가 원인인 실패를 `primary_role` 에 먼저 보내면 구현이 스텁에 맞추려
   **계약에 없는 특수 분기를 프로덕션에 넣는다.** 클론 1런이 실제로 그랬고 사람이 되돌렸다 —
   오배정의 대가가 「라운드 하나」가 아니라 프로덕션 코드 오염이다.
4. **배정은 위치가 아니라 미시도 집합에서 고른다** (백로그 27, 결정 3 의 나머지 절반).
   `ladder[len(assigned)]` 는 사다리 순서가 sig 마다 **고정**일 때만 옳다. 결정 3 이 그 전제를
   깬다 — 순서가 그 실패의 프레임에 달렸고 `signature()` 는 프레임을 해시에 넣지 않아 같은
   sig 가 다른 프레임으로 재발한다. 뒤집힌 사다리에 위치로 배정하면 r1 `test` · r2 **또**
   `test` · r3 `contract` 가 되어 `impl` 이 한 번도 안 시도된 채 계약 결함이 된다 —
   [[ADR-H023]]·M33 이 막으려던 「두 역할 중 한쪽만 시도해 보고 끝」 그대로다. 지금 코드에서는
   두 방식이 같은 답을 내므로 **이 교체 자체는 동작을 바꾸지 않고 결정 3 을 안전하게 만든다.**
5. **`out_of_contract` 는 타입 전용 export 를 보지 않는다** (백로그 24 잔여). `_DEFAULT_PUBLIC`
   에 `kw` 명명 그룹을 더하고 `type`·`interface` 를 `TYPE_ONLY_KEYWORDS` 로 뺀다. 계약은 런타임
   심볼을 보고, 탈출구가 「01 이 내부 타입까지 `## 데이터 형태` 에 열거한다」뿐이면 계약이 타입
   선언서가 된다. 실측: 4런의 `out_of_contract` 6건 중 **3건이 타입 별칭, 전부 major · 전부
   deferred**.
6. **`enum` 은 면제하지 않는다** (백로그 24 잔여, 결정 5 의 경계). TypeScript 의 `export enum` 은
   **런타임 객체를 내보내고** 다른 모듈이 값으로 import 한다. [[ADR-H071]] 트레이드오프가 셋을 한
   묶음으로 부른 것이 그 자리에서 틀렸다 — 「계약은 런타임 심볼을 본다」는 사유로는 `enum` 을 뺄 수
   없다. 표본도 0 이다(3건 전부 타입 별칭).

**트레이드오프**:

- **결정 1 은 `check` 를 올리지 않는다.** 그 `_note` 는 "의존성이 없으므로 취약점 게이트가 잴
  것이 없다" 인데, 이것은 **구조가 아니라 현재 상태**다 — 의존성이 생기면 잴 것이 생긴다.
  경계 사례라 여기 적어 둔다. `lint` 는 더 분명하다: "린터를 들이지 않는다" 는 선택이지 부재가
  아니다. 둘을 올리면 「스킵은 통과가 아니다」가 흐려진다.
- **결정 1 은 `nextjs-ts` 를 비대칭으로 둔다.** 거기서 e2e 는 구조적 부재가 아니라 러너
  미설치이고(백로그 17), 그 `_note` 는 도입 계획을 담은 살아 있는 문서다. 그래서 **클론의
  등급은 이 PR 로 바뀌지 않는다.** 바뀌지 않는 것이 옳다 — 그 gap 은 진짜 결손을 말하고 있다.
- **결정 2 는 「외부가 꺼져 있으면 07 을 통째로 건너뛴다」를 버렸다.** 그쪽이 더 싸지만
  `escaped_05` 표본이 영영 0 이 되고, 그러면 생략 정책 자신의 근거가 사라진다 — [[ADR-H043]] 의
  재검토 조건이 07 을 아예 없애는 것을 막은 것과 같은 이유다.
- **결정 3 은 표본 1런으로 기본값을 뒤집는다.** 근거는 빈도가 아니라 **대가의 종류**다 —
  프로덕션 코드 오염은 라운드 하나로 환산되지 않고, 사람이 되돌리지 않았으면 계약에 없는
  분기가 남았다. 이 판단이 틀렸다면 `owner_reason` 에 「프레임이 전부 테스트 파일이다」가 뜬
  런에서 테스트 역할이 헛돌고 `impl` 로 넘어가는 라운드가 늘어난다.
- **결정 4 는 지금 아무 동작도 바꾸지 않는다.** 되돌아보면 위치 기반 배정은 `dispatch` 의
  롤백(`assigned[-1].pop()`)과 대칭이라는 점에서 읽기 쉬웠다. 미시도 집합도 그 pop 과 대칭이지만,
  「몇 번째냐」가 아니라 「무엇을 안 해봤냐」를 묻게 되어 `idx` 가 `carry_contract` 판정에만 남는다.
- **결정 5·6 은 `enum` 과 `const enum` 을 안 가른다.** `const enum` 은 런타임 값을 남기지 않지만
  정규식으로 둘을 가르면 어댑터가 덮은 정규식과의 합이 복잡해진다. 표본이 0 이라 지금 가르지
  않는다.
- **결정 5 는 어댑터가 정규식을 덮으면 면제를 잃는다.** `attribution.public_symbol_regex` 에는
  `kw` 그룹이 없을 수 있어 `groupdict()` 로 읽고, 없으면 면제가 안 걸린다 — 어댑터가 덮었으면
  면제도 그 어댑터의 몫이다. 조용히 터지지 않는 것이 요점이고, 그 사실을 상수 주석이 적는다.

**재검토 시점**: 클론의 다음 런 하나. 25 는 `triage_miss` 가 난 런에서 07 이 `skipped` 인가,
27 은 `owner_reason` 에 「프레임이 전부 테스트 파일이다」가 뜨고 그 런에 프로덕션 오염이
재발하지 않는가, 24 는 `contract-trace` 의 `out_of_contract` 에 **타입 별칭 0건**인가로 본다.
**22 는 런이 판정하지 않는다** — 재라벨링이라 이 리포의 테스트로 닫힌다. 대신 다음 런의
`gaps` 가 `stage_absent:lint`·`:check` 를 그대로 달고 오는지를 보고, 달고 온다면 그것이
백로그 17 과 어댑터 충실의 몫임을 [[ADR-H039]] 대로 「재봤나 아직인가」로만 적는다.

관련: [[ADR-H047]](`stage_na` 를 연 결정) · [[ADR-H056]](슬롯 예산 gap) ·
[[ADR-H044]](`triage_miss` 의 벌칙을 만든 결정) · [[ADR-H043]]·[[ADR-H059]](07 생략 조건) ·
[[ADR-H061]](감사 런의 effort) · [[ADR-H023]](사다리와 정체 감지) ·
[[ADR-H069]](`ambiguous` 가 기본값인 같은 뿌리) · [[ADR-H049]]·[[ADR-H071]](관례 export 면제) ·
[[ADR-H070]](같은 4런이 연 결함 셋)

---

### ADR-H073: 제출 검사 선언도 읽히거나 거부된다

**날짜**: 2026-09-21 · **상태**: 채택됨 · **구현 상태**: 구현됨 (2026-09-21 —
`cli.SUBMIT_CHECKS` · `cli._resolve_submit_impl` · `cli._lint_submit_checks` ·
`cli.EXIT_CODES` · `cli.PRODUCES_KEYS`, `cli.EXTERNAL_RAW` 와
`cli._external_quote_errors`, `review.CAP_LANES` 와 `_cap` 의 폴백 제거)

**맥락**: [[ADR-H070]] 이 백로그 21 을 닫으며 **번호를 주지 않고** 남긴 한 문장이다 —
*"프론트매터의 `"schema"` 와 `submit_checks[].id` 를 읽는 코드가 없는 것은 열린 채로
둔다."* 번호가 없어 실행 순서표에 오르지 못했고, 파동 6 이 「결정 대기가 비었다」고
적을 때도 세어지지 않았다.

**이 결함에는 이미 이름이 있다** — [[ADR-H025]] 「선언은 읽히거나 거부되고, 무시되지
않는다」(M36). 그 맥락 문단이 지금 상황과 글자 그대로 같다: *"상한이 전부 `scripts/`
파이썬 grep 0건이었다. 코드는 하드코딩된 값을 썼고, **지금 동작이 선언값과 우연히
일치해서** 다섯 런 동안 아무도 눈치채지 못했다."* **ADR-H073 은 새 원칙이 아니라
[[ADR-H025]] 를 빠져나간 선언들에 그것을 적용하는 것이다.**

**전수 조사가 전제를 둘 고쳤다.**
1. **`submit_checks[].id` 는 10종이 아니라 24종**이고(00:4 · 01:9 · 02:3 · 03:5 ·
   06:3 · 07:3, 선언 인스턴스 27), **20종이 `scripts/` 파이썬 grep 0건**이었다.
   나머지 4종도 우연한 동명이인이다(`_dispatched_roles`·`_tests_required` 는 렌더
   헬퍼다). `test_pipeline.py` 15,099행에 `submit_checks` 문자열이 **0건**이었다.
2. **`produces[].schema` 는 `submit_checks` 보다 나쁘다.** 10종을 11곳에서 선언하는데
   읽는 코드가 0인 것은 같고, **그 이름의 스키마 아티팩트 자체가 없다** — 리포에
   실재하는 것은 `config.schema.json`·`adapter.schema.json` 둘뿐이다.

**그리고 하나는 구현이 아예 없었다.** `07-pr-review.md` 가 `source_quote_substring`
을 선언하고 산문으로 *"`quote` 는 외부 리뷰 원문의 부분문자열이어야 한다. 05 와 같은
검사다"* 라고 약속하는데, `_record_07` 과 `review07.py` 에 `quote` 를 보는 줄이 **한
줄도 없었다.** 리포 전체의 quote 대조는 `verdict.check_plan`(01 플랜) ·
`verdict.check_review`(01·05 리뷰어) · `_record_02`(02 플랜) **셋뿐**이다. 07 의
findings 는 **외부 봇·사람**에서 오는, 파이프라인에서 가장 덜 신뢰되는 입력이다.

**결정**:

1. **어휘와 구현 자리를 한 자리에 두고, 그 포인터를 기계가 해석한다.**
   `cli.SUBMIT_CHECKS` 가 id → `{exit, impl, why}` 를 쥐고 `impl` 은 `"모듈:이름"`
   튜플이다. `lint-phases` 가 `_resolve_submit_impl` 로 **해석한다** — 호출하지 않는다.
   **주석이 아니라 기계가 대조하는 사실이라야** 레지스트리가 기계의 소재에 대해
   거짓말할 수 없다. `LOOP_ON_EXCEED` 와 같은 가드 주석을 그대로 붙였다.

2. **선언 → 레지스트리는 FAIL, 레지스트리 → 선언은 WARN 이다.** M36 이 금지한 것은
   **어휘가 기계를 앞서는** 한 방향이다. 기계를 먼저 만들고 선언을 나중에 다는 것은
   정당한 순서이고, `impl` 해석이 통과한 이상 그 기계는 실재한다 — 여기서 막으면 그
   순서가 lint 에 걸린다. WARN 의 전례는 `agent_model` · FUTURE `on_success` 다.

3. **`on_fail` 은 레지스트리가 쥔 종료 코드와 대조된다.** 이것이 없으면 lint 는
   철자 검사기일 뿐이다. 어휘의 정본은 team-spec §2.3 의 표이고 `EXIT_CODES` 는 그것을
   코드로 내린 것이다. **실제 선언값은 `4`·`8`·`9`·`10` 넷뿐인데 어휘를 그 넷으로
   좁히지 않았다** — 표본 없이 정책을 만드는 것이 되고([[ADR-H007]]), 넓은 어휘로도
   오타는 잡힌다.

4. **레지스트리의 `exit` 가 두 번째 하드코딩이 되는 것은 lint 가 아니라 테스트가
   막는다.** `SUBMIT_CHECKS["critical_zero"]["exit"] = 4` 를 손으로 적는 순간 그 값은
   `_record_02` 와 독립된 두 번째 출처이고, `_record_02` 가 8 로 바뀌어도 lint 는
   초록이다 — **M36 이 한 층 위로 이사할 뿐이다.**
   `test_the_registry_exit_is_the_exit_a_real_run_returns` 가 실제 `run_record` 의
   exit 를 **리터럴이 아니라 레지스트리 값과** 비교해 그 고리를 잇는다.
   [[ADR-H025]] 가 *"'읽는지' 만 보는 단언은 되돌려도 초록이다"* 로 적은 함정이다.

5. **`produces[].schema` 는 살리지 않고 걷어낸다.** 처방이 1 과 반대인 이유는
   가리킬 대상이 다르기 때문이다 — `submit_checks` 는 기계가 있고 배선이 없었지만
   `schema` 는 **가리킬 것 자체가 없다.** JSON Schema 열 개를 새로 짓는 것은 재지 않은
   제약을 상수로 굳히는 것이라([[ADR-H007]]·[[ADR-H039]]) 하지 않았다. 대신
   `PRODUCES_KEYS` 로 키 집합을 닫아 **근거 없는 어휘가 되돌아오지 못하게** 했다 —
   `FRONT_KEYS` 가 최상위에 하는 일과 대칭이다.

6. **07 의 quote 검사를 실제로 만든다 — 다만 `external` 출처만.** `review07
   --external` 이 받은 파일을 `07_external.raw.json` 으로 런 디렉터리에 남기고
   `record` 가 대조한다. **`produces` 에 넣지 않았다**: 워커가 내는 것이 아니라
   실행기가 받은 것이고, 넣으면 진입 검사가 워커에게 요구한다([[ADR-H070]] 이 02 에서
   배운 것). **`code-review` 와 `human` 은 대조하지 않는다** — 전자의 원문은 내장
   리뷰어의 출력이라 저장되지 않고 후자는 수리 대상이 아니라 보고 대상이다.
   **건초더미가 없는 것을 검사한 척하지 않는다.** 07 산문을 그 범위로 좁혔다.

7. **봇 원문이 없는 런의 `source: "external"` 은 「없어서 통과」가 아니라 거부다.**
   밖이 말했다는 주장인데 밖의 기록이 없으면 대조 불가능한 주장이다 — 스킵을 통과로
   세지 않는 규율이 여기에도 걸린다.

8. **`review.profile_caps` 의 폴백을 없앤다** (백로그 28 곁가지). `DEFAULT_CAPS =
   {"fix":1,"small":1,"normal":3}` 이 `config.json` 의 `{"docs":1,"fix":1,"small":2,
   "normal":4}` 와 어긋나 있었고 `docs` 키는 아예 없었다 — **config 가 이겨서
   무해했기 때문에** 아무도 눈치채지 못했다. [[ADR-H025]] 의 *"`or` 폴백을 두지
   않는다 — 폴백이 곧 새 하드코딩이다"* 를 그대로 적용했다. **값을 복사하는 안은
   버렸다** — 두 출처를 다시 만드는 것이라 결함의 재생산이다. 빠진 선언은
   `review.validate` 가 잡고, 그러면 `lint-phases` 와 `doctor` 양쪽에 공짜로
   배선된다([[ADR-H063]] 의 전례). `config.schema.json` 이 네 레인을 `required` 로
   건다.

**트레이드오프 — 이 레지스트리가 증명하지 않는 것**:

- **「그 선언이 그 런에서 실제로 돌았다」는 여전히 기계가 말하지 못한다.** 증명하는
  것은 「그 검사의 구현 자리가 아직 있다」뿐이다. 진짜 디스패치는 24개 검사의
  시그니처가 전부 다르고(`(payload, request_text, config)` · `(root, config, claims)`
  · `(root, s, ctx, adapter)` …) `_record_03` 은 **검사 순서가 비용 오름차순 설계**라,
  디스패치 루프가 그 순서를 평평하게 만든다. 그리고 디스패치 표가 있어도 「표가
  완전하다」는 아무도 증명 못 한다 — 표 밖 인라인 검사가 여전히 가능하다. 답은
  **런타임 영수증**(`trace_contract` 의 `checks_run` 과 같은 모양)이고 **미구현
  백로그 31** 로 번호를 준다. [[ADR-H070]] 이 번호 없이 산문으로 남겨서 잊힌 것을
  반복하지 않기 위해서다.
- **`impl` 포인터는 거칠다.** 인라인 검사는 그것을 품은 기록자를 가리키므로
  `source_quote_substring → cli._record_02` 는 "이 함수 안에 있다" 이지 "이 함수가
  그 검사다" 가 아니다. 주석에 적었다.
- **`submit_checks[].from`·`and`·`except`·`claims` 는 여전히 읽는 코드가 없다.**
  `"config.roles[].when_contract_section"` 같은 값은 산문 포인터다 — 닫으려면 dotted
  config 경로 해석기가 필요하고 별 기능이다. 다만 `${...}` 플레이스홀더는
  `_lint_placeholders` 가 이미 해석한다.
- **05 와 08 은 `submit_checks` 를 선언하지 않는다.** 05 는 M46 의 2층
  (`review.check` / `ledger.append`)이 따로 있어서다. **여기서 선언을 만들지
  않았다** — 없는 기계를 어휘로 예고하지 않는 것이 이 ADR 자신의 규칙이다.
  **미구현 백로그 32** 로 남긴다.

**기존 테스트 하나를 뒤집었다.** `test_변경_요청_미해결은_exit_10` 이 봇이 꺼진
런(`external.status: "disabled"`)에서 `source: "external"` 을 주장했다 — **밖이 없는데
밖이 말했다고 한 것**이고, 대조가 없어서 통과했다. 변경 요청은 정의상 외부 리뷰가
내는 것이므로(§3.7) 봇을 켜고 그 원문을 주는 모양으로 고쳤다. 형제 테스트(같은
셋업에서 `source: "code-review"`)가 원래 맞는 모양이었다. `DEFAULT_CAPS` 를 읽던
단언 한 줄도 지웠다 — 리포 전체에서 뒤집힌 것은 이 둘뿐이다.

**재검토 시점**: **런이 판정하지 않는다.** 이 결정은 전부 `lint-phases` 와 pytest 로
닫힌다 — 어휘 밖 id 를 넣으면 exit 2, 선언을 지우면 WARN, `on_fail` 을 어긋나게 하면
exit 2, 구현 함수 이름을 바꾸면 테스트가 실패, `profile_caps` 를 지우면 멈춘다.
**단 하나 런이 판정하는 것**은 07 의 quote 대조가 실물 봇 출력에서 오탐을 내는가다 —
봇이 켜진 클론의 첫 런에서 `external` 출처 finding 의 quote 거부가 나면, 원인이
메인의 옮겨 적기인지 정규화(`normalize_ws`)의 한계인지 본다.

관련: [[ADR-H025]](같은 원칙을 `loop` 어휘에 처음 적용한 결정) ·
[[ADR-H070]](이 항목을 「열린 채로 둔다」로 남긴 결정 · 02 의 건초더미 교훈) ·
[[ADR-H007]]·[[ADR-H039]](재지 않은 것을 상수로 굳히지 않는다) ·
[[ADR-H063]](프로필 시드를 `required` 로 잠근 전례) ·
[[ADR-H035]](어휘를 데이터에서 닫고 검사로 강제하는 같은 모양)

---

### ADR-H074: 계기판은 압력이 실리는 집합을 잰다

**날짜**: 2026-09-21 · **상태**: 채택됨 · **구현 상태**: 구현됨 (2026-09-21 —
`cli._SLOT_BULLET` 의 번호 목록, `cli._instruction_slots` 가 `_rules_read_expected`
집합을 돌며 `per_file` 을 남김, `run_report` 의 `instruction_slot_over_budget` 방출
제거, `report.NON_DEMOTING_GAPS`·`GAP_DESCRIPTIONS` 에서 그 이름 삭제,
`report._slots_cell` 의 파일별 렌더)

**맥락**: 미구현 백로그 **33**. `instruction_slot_budget`([[ADR-H056]])은 **08 지시문
검토가 규칙을 흡수하는 루프의 압력계**다 — 판정 코드가 `instruction_review` 바로
**다음 줄**에 있다. 규칙을 계속 흡수하면 지시문이 무한히 자라고, 커질수록 워커가 다
읽을 확률이 떨어진다. **그 압력계가 셋을 잘못 재고 있었다.**

| 무엇 | 실측 (2026-09-21) |
|---|---|
| **집계 범위** | `instruction_file` **하나만** 셌다. `rules_dir` 직속 `*.md` 는 워커가 **전원 매번 읽고 sha256 증명까지** 하는데 한 칸도 안 세어졌다 |
| **세는 방식** | `^[-*+]\s+` — 번호 목록을 안 셌다. 이 리포 `CLAUDE.md` 의 「작업 원칙」 **4개가 통째로 빠져** 13 으로 세어졌다(실제 17) |
| **gap 판정** | 클론 **4/4런 `17/12`**, 이 템플릿 `13/12` — 런 내용과 무관한 **상수** |

**결정**:

1. **재는 집합을 `_rules_read_expected` 와 같게 한다.** 새로 짤 것이 없었다 — 그 함수가
   이미 올바른 집합을 만든다. **세는 집합이 곧 증명하는 집합**이라야 계기판이 압력을
   잰다. 아니면 **규칙을 `rules_dir` 의 다른 파일로 옮기기만 해도 gap 이 사라지는
   착시**가 성립하는데, 그건 압력을 낮추는 것이 아니라 **계기판을 끄는 것**이다.

2. **번호 목록도 한 칸이다.** 빠져 있던 넷은 파일 자신이 *"넷 다 기계가 안 잡는
   산문이고, 이 파일은 워커가 읽어야 지켜진다"* 고 적은 것들이다 — **예산에서 가장
   비싸야 할 것이 공짜였다.** 펜스·표·들여쓰기 제외는 **그대로 둔다**: 이 파일 기준
   0칸이고 의도된 것이다(명령어 예시는 규칙이 아니다).

3. **파일별 칸 수를 남긴다.** 합계만으로는 *"어느 문서가 비대해졌나"* 를 말할 수 없고,
   값을 정하려면(백로그 34) 그 분포가 먼저 필요하다. 보고서는 **파일이 둘 이상일 때만**
   적는다 — 하나면 합계와 같은 말이다.

4. **초과는 gap 이 아니라 관측이다.** 지시문 파일 크기는 런 내용과 무관해 매 런 같은
   값이 뜬다. **매 런 울리는 경보는 경보가 아니고**, gap 목록 전체를 둔감하게 만든다.
   [[ADR-H067]] 이 `risk_undeclared` 에 먼저 간 길이다 — *"게이트가 아니라 관측만"*.
   **숫자는 사라지지 않는다**: `report._slots_cell` 이 `used/budget` 을 계속 렌더한다.

5. **gap 이름을 남기지 않고 지운다.** 방출을 멈추고 `GAP_DESCRIPTIONS`·
   `NON_DEMOTING_GAPS` 에 이름만 두면 **아무도 안 내는 어휘**가 된다 — 어휘가 기계를
   앞서는 M36 의 거울상이고, [[ADR-H073]] 이 `produces[].schema` 에 대해 *"가리킬 것이
   없으면 살리지 않고 걷어낸다"* 로 답한 자리와 같다.

6. **`instruction_slot_unmeasured` 는 그대로 gap 이다.** 그건 상수가 아니라 **「못 잼」의
   신호**다 — 본문은 있는데 불릿이 0인 것은 런마다 달라질 수 있고, 0칸이 "규칙 없음"
   으로 읽히면 안 된다.

7. **값 12 는 건드리지 않는다.** 1·2 를 하면 숫자가 달라지고(13 → 17), 그 상태의 12 는
   **재본 적 없는 값**이다([[ADR-H007]]). 올릴지 규칙을 줄일지 규칙 자체를 폐기할지는
   클론 런의 파일별 분포가 정한다 — **미구현 백로그 34**.

**트레이드오프**:

- **총량은 안 줄어든다.** 이 결정은 **재는 법**만 고친다. 실제 절감은 ⑴ 규칙을 줄이거나
  ⑵ 역할별로 다른 집합을 읽게 하는 것뿐이고, 후자는 **미구현 백로그 35** 다.
- **분포가 보이는 것이 곧 답은 아니다.** `docs/` 가 빈 골격인 지금은 `CLAUDE.md` 17 ·
  나머지 0 이라 새 정보가 없다. **실제 프로젝트에서 PRD·TRD 가 채워져야** 이 숫자가
  말을 하기 시작한다.
- **gap 을 지우면 보고서를 훑는 사람이 놓칠 수 있다.** 다만 그 경보는 지금까지 **한
  번도 런별 정보를 준 적이 없고**, 숫자 자체는 `## 리뷰` 표에 남는다.

**재검토 시점**: **런이 판정하지 않는다** — 이 결정은 전부 `lint-phases` 와 pytest 로
닫힌다(선언을 바꾸면 숫자가 따라 바뀌는 변이 테스트 6개). 런이 판정하는 것은 값(백로그
34)이고, 클론의 첫 런들에서 `instruction_slots.per_file` 분포를 본다.

관련: [[ADR-H056]](이 예산을 만든 결정 — 계측만 고치고 정책은 그대로다) ·
[[ADR-H067]](게이트가 아니라 관측만 — 같은 길) ·
[[ADR-H073]](가리킬 것 없는 어휘는 걷어낸다 · `rules_read` 집합의 단일 출처) ·
[[ADR-H025]](선언은 읽히거나 거부된다 — 변이 테스트여야 문다) ·
[[ADR-H007]]·[[ADR-H039]](재지 않은 것을 상수로 굳히지 않는다) ·
[[ADR-H055]](`rules_read` 증명이 이 집합을 정한다)

---

### ADR-H075: 덜어내기 — 실물 4런 근거로 뺀 것과 남긴 것

**날짜**: 2026-09-22 · **상태**: 채택됨 · **구현 상태**: 구현됨 (2026-09-21~22 — 4웨이브:
PR #29 주변부 · PR #30 07 교체·승격 계층·원장·08 축소 · PR #31 00·02 제거·01 축소·모델 슬롯표 ·
PR #32 03 단일 작성자·04 귀속 제거·05 단일 리뷰어·계약 대조 5종·문서 마무리)

**맥락**: 「모델이 좋아질수록 하네스를 많이 만들면 오히려 못한다 — 덜어내라」는 조언을
그대로 믿지 않고 **실물로 쟀다.** 근거는 클론 리포 `banana-island-ops` 의 4런(P2~P5,
2026-09-20 · 그 리포 `docs/harness/PILOT-LOG.md` 「4런 교차 분석」)과 이 템플릿 자신의
인벤토리다. 아래 표는 마스터 플랜(리포 밖)의 근거표를 **표째** 옮긴 것이다 — 값의 정본은
클론 리포이고, 여기에는 「무엇이 판정을 냈고 무엇이 한 번도 안 냈나」가 든다 ([[ADR-H039]]).

*비용 분포 (4런 합 244분 · 지시된 모델 호출 54회)*

| 페이즈 | wall | 모델 호출 | 실물 수확 |
|---|---:|---:|---|
| 05 코드리뷰 | 24% | **31 (57%)** | major 21 중 15 가 `test` 리뷰어의 `TEST_MISSING_FAILURE_PATH`. 원장 236행 중 deferred 71%. 같은 critical 을 3인이 각자 발견. 에스컬레이션 뒤 전원 재팬아웃 r3 = 호출 8 · blocking 0 |
| 03 구현 | 23% | 2~3/런 | — |
| 04 게이트 | 16% | 수리 시 | 스텁 행 결함 1건. 단 `ambiguous → impl` 오배정으로 impl 이 계약 없는 분기를 넣었고 사람이 되돌렸다 |
| 06 PR | 12% | 0 | 사람 대기 22.5분(승인 14.0 + `precheck_policy` 8.5) |
| 01 계획 | 10% | 1/런 | — |
| 02 교차검증 | 9% | 3 | 16건 중 채택 7(44%), 편차 4/5↔0/5. P5 는 02 자신이 틀린 critical 을 냈다. P3 는 1차 관측기의 critical 2 를 코드로 반박 |
| 07 PR 리뷰 | 4% | 1(강제) | 4런 구조적 빈손 — 외부 봇 off, `clean_05` 생략 |
| 00·08 | 2% | 1/런 | — |
...
| 장치 | 근거 |
|---|---|
| 어댑터 귀속 규칙 4종 · `verified` · `verify-adapter` | 19런 실패 21건 전수 **0/4 판정**. `verified` 는 근거 없이 true 로 올라갔다가 되돌아왔다([[ADR-H069]]) |
| 규칙 승격 `promote` · `rules_changelog` · 임계 6종 | staged 13행 전부 `skipped`, `applied` 0 |
| 외부 PR 리뷰 봇 · 07 의 `source_quote_substring` | `enabled:false`. 후자는 선언만 있고 구현이 없었다([[ADR-H073]]) |
| 비용 계측 `cli.py cost` | 4런 전부 「미계측」 |
| `instruction_slot_budget` (12) | 4/4런 `used 17/12` — 런과 무관한 상수. ADR 4건([[ADR-H055]]·[[ADR-H056]]·[[ADR-H072]]·[[ADR-H074]])이 이 계기판 하나를 손봤다 |
| `risk_undeclared` | 관측만. 표본 2/5 |
| 캘리브레이션 임계 180s · `tests_ran_floor` | full 10.9s. 임계에 닿은 적 없음. floor 는 15런 동안 14 로 고정돼 감지가 꺼져 있었다 |
| 지시문 검토(`revise-claude-md`) | 매 런 「흡수 0 · 기각 0 · 바꾼 파일 0」 |

*하네스가 스스로 만든 마찰 (모델 실수가 아닌 왕복)*

| 무엇 | 횟수/4런 | 원인 |
|---|---:|---|
| `precheck` exit 9 (`files_max` 10, 실제 12~14) | 3런, 05·06 양쪽 | 3/4런이 넘는 상한은 상한이 아니라 통행료 |
| 03 `rules_read` sha256 불일치 반려 | 2 | 병렬 런이 `docs/*.md` 를 바꿨다 |
| 02 `quote` 부분문자열 반려 | 2 | 프롬프트와 검사기가 다른 문서를 가리켰다 |
| `triage_miss` → 07 강제 호출 | 1 | 벌칙이 빈손 페이즈를 불렀다 |
| `out_of_contract` major | 원장 26행/12런 전부 major | 프레임워크 관례·타입 전용 export 를 「계약 밖 심볼」로 오탐(수정 전 78/78). ADR 3건이 면제 목록을 넓혔다 |
| `untested_contract_item` major | 21행/7런 | UI 컴포넌트를 「테스트 없는 유닛」으로 — 6/6 오탐 |

*하네스가 실제로 잡은 제품 결함*

| 결함 | 잡은 장치 | 남긴 것 |
|---|---|---|
| 캐스트 누락 → 실DB 500 (테스트 전부 초록) | 05 `gen`·`data` 각각 SQL 정독 | gen 하나로 충분 — 둘 다 잡았다 |
| P4 critical | 05 `gen`·`data`·`sec` 3인 중복 | gen 하나 |
| 스텁 행 필드 누락 | 04 게이트(테스트 실행) | 게이트 유지 |
| 타입 오류(초기 런) | 03 컴파일 | 유지 |
| P3: 1차 관측기의 critical 2 를 코드를 읽어 반박 | 02 | 01 리뷰어가 리포를 읽는다(Wave 3) — 아래 「02 유보」 |
| 초기 15런: 05 통과분에서 critical 2 | 07 내장 `/code-review` | 도구는 남긴다 |
| 게이트를 통과해 밖에서 잡힌 것 3건(빌드 · 런타임 `new` 누락 · UI 표시) | 없음 | 게이트를 더 돌려야 할 이유지 덜 돌릴 이유가 아니다 |

*메타*: 이 리포의 ADR 74건 중 33건(45%)이 하네스가 하네스를 고친 것이고 마지막 5건(H070~H074)은
전부 그렇다. 승격 계층에 ADR 7건, 실물 승격 0. 클론의 PR 35건 중 하네스 동기화 7건(20%),
파이프라인 밖 머지 12건. 클론 `CLAUDE.md` 는 「규칙을 늘리는 것보다 워커가 읽었는지 게이트가 묻게
하라」고 적었고, 그 게이트(`rules_read`)가 4런에서 낸 것은 오탐 반려 2회뿐이다.

**결정**: 네 부류로 나눠 뺐다. 항목마다 「뺀 것 · 왜 · 대체」다.

*T1 — 한 번도 판정을 내지 않았다 (지워도 잃는 것이 0)*

| 뺀 것 | 왜 | 대체 |
|---|---|---|
| 규칙 승격 계층(`promote`·`ledger` 집계·임계 6종·`rules_changelog`·`deferred`·`taxonomy`)·`findings.jsonl` | staged 13행 전부 skipped, applied 0 | 런별 지적은 `_workspace/runs/*/05_review*.json` 에 그대로 |
| 귀속(`attribution.py`·어댑터 규칙 4종·`verified`·`verify-adapter`·`gate --replay`·`fixtures/gate/`) | 19런 0/4 판정. 유일한 실물 효과가 오배정 | `adapters.infra_match` 한 줄 — 환경 실패가 수리 예산을 태우지 않게. 실패 출력은 작성자에게 그대로 |
| 외부 PR 리뷰 봇 · 07 의 `source_quote_substring` | 켜진 적 없음 · 구현 없음 | 07 = `/code-review` 1회 |
| 비용 계측(`cost`) | 4런 「미계측」 | 없음 |
| 지시문 검토·`instruction_slot_budget`·`per_file` | 흡수 0 · 상수 경보 | 없음 — 지시문은 짧게 둔다 |
| 캘리브레이션(`calibrate`·`calibration.json`·`derived`·`background_threshold_sec`) | 임계에 닿은 적 없음 | 테스트 수 하한 = 직전 완주 런 × 0.9 (`harness.completed_runs`) |
| 부르지 않는 서브커맨드 `advance`·`retry`·`escalate`·`abandon`·`mask` | 호출처 0 | `st.escalate`·`resume`·`mask_text` 유지 |
| `risk_undeclared`·`reviewers[].risk`·`cross_verify.secondary` | 관측만 | 없음 |
| 세션 원장(`session_log`·훅·`/log`) | 하네스 개발용 메타 도구 | 없음 |

*T2 — 마찰이 수확보다 크다*

| 뺀 것 | 왜 | 대체 |
|---|---|---|
| 02 교차검증 페이즈 | 채택 44%, 편차 4/5↔0/5, 틀린 critical 1, wall 9% | 01 plan-reviewer 가 리포를 읽고, 메인은 코드 근거로 `false_positive` 기각(아래 「02 유보」) |
| 리뷰어 5종·라우팅·priority·`profile_caps`·`review.depth` 레인표·`merged` 모드·심각도 상승 지급 | gen 이 data·sec 의 critical 을 전부 같이 잡았다. arch 수확 minor 4/1런 | `general-reviewer` 하나 `diff+refs` + 델타 1회(같은 리뷰어). 「볼 것」에 테스트 품질 행 |
| `precheck` 예산 exit 9·`--ack-policy`·정책 지문 | exit 9 가 3/4런, 대기 8.5분 — 전부 `files_max` | 예산은 정보 행. exit 9 는 브랜치·base 둘뿐. 검사는 05·06 양쪽 그대로 |
| `rules_read` sha256 증명·`rules_dir`·`rules_exclude`·역할별 「읽을 곳」 | 반려 2/4런 전부 병렬 런의 문서 변경. 해시 일치는 읽었다는 증명이 아니다([[ADR-H055]] 자인) | 작성자 패킷의 「읽을 곳」 세 줄 |
| 01 INTENT/COVERAGE·`verdict` 검사 ≈11종·`mode`/`primary_error` | 옮겨 적기를 기계가 대조하는 의례. 잡은 결함 0 | severity 어휘 + Critical 0 판정 + 인용 부분문자열 |
| 계약 대조 5종(`out_of_contract`·`untested_contract_item`·`authz_untested`·`missing_journey_spec`·`missing_screen`)·baseline 유예·`convention_globs`·`TYPE_ONLY_KEYWORDS`·계약의 여정·화면 절 | 오탐 78/78 · 6/6 · 재본 적 없음 | 5종 남김 — `missing_impl`·`missing_entrypoint`·`missing_error_symbol`(비용 0) + `untested_entrypoint`·`untested_error_symbol`([[ADR-H058]] 실측 14/14 · 오탐 0) |
| 00 트리아지·`triage_miss`·`config.triage`·`config.models`·`small` 레인 | miss 1/4런이 빈손 호출을 강제. 등급표 전부 미검증 | `init --profile {normal\|docs\|fix}` 사용자 선언. 모델·effort 는 에이전트 프론트매터 |
| 04 `stuck_after_identical`·flip 사다리·`sig_chain`·에스컬레이션 메뉴 | 역할 분리의 부속. 2런 모두 사람이 「기타」 | `repair` 3회 초과 → 자유 서술로 사람에게 |
| 07 기계(`review07`·`audit_run`·`pr_repair`) | 4런 빈손 | 얇은 07 + `dup_05` 로 escaped 계수 |
| 08 원장 축·리뷰어별 해소표·`blind_spots`·비용 행 | 읽고 행동한 것은 서술 4절뿐 | 등급·gap·wall·호출·테스트 수·서술 4절 |

*T3 — 구조 (사용자 결정 2026-09-21)*: 03 은 **단일 작성자** `impl-writer`(옛 impl ∪ test ∪ ui 의
`owns`, `excludes` 비움) — 계약대로 테스트를 먼저 쓰고 구현하며 typecheck·lint·테스트를 직접
돌린다. 04 는 영수증 — 스테이지 전부를 한 번 더 돌려 기록하고 실패 출력을 작성자에게 되돌린다(≤3).
05 는 gen 1명 + 델타 1. 07 은 `/code-review` 1회. **「병렬 3역할에 실행 권한만 줬으면 되지
않았나」**: 병렬에서는 누구도 테스트를 구현에 대고 돌릴 수 없다 — 스텁 행 불일치·`nothing_locked`
37건은 테스트를 **코드를 못 보고 쓴** 산물이다. 그 성질을 지키는 중간안은 순차(test → impl)이고,
`roles[]` 를 배열로 남긴 것은 그리로 되돌릴 자리다.

*T4 — 문서*: `team-spec.md`(1,473줄) 삭제 — 정본은 페이즈 파일 7개 + README 표다. 살아남는 규칙
셋(P1 테스트 0개 비차단 · P3 원격 부재 exit 9 · E8 non-FF 중단)은 해당 페이즈 파일에 있다. 금지어
20단어는 테스트 상수 `BANNED_STACK_WORDS` 로. README·ROADMAP §1·§6·§7·§8·CLAUDE.md ⚠ 문단·
PILOT-LOG 골격을 남은 장치 기준으로 다시 썼다. 이 ADR 이 백로그 28·29·31·32·34·35 를
「대상 소멸」로 닫는다.

**02 유보를 뒤집는 사유**: ROADMAP §7(옛)과 [[ADR-H060]] 은 02 의 채택률을 더 본 뒤 판단하기로
유보했다. 4런이 답했다 — 02 의 값은 「다른 눈이 **코드를 읽어** 가짜 critical 을 반박」한 것
하나(P3)뿐이고, 그 기능은 페이즈 하나가 아니라 **01 리뷰어가 리포를 읽을 수 있게** 하는 것으로
흡수된다. 나머지는 편차(4/5↔0/5)와 틀린 critical(P5)이 wall 9% 를 먹었다.

**트레이드오프**:
- 단일 작성자의 숨은 비용 — (a) 테스트를 보는 눈이 gen 의 행 하나로 준다: 예측 8 이 잰다. (b) 큰
  변경(P5 급 3,332줄)은 한 컨텍스트에 구현+테스트+실행 출력이 든다: 예측 4 가 규모 정규화로 잰다.
  넘치면 요청을 나누는 것이 답이지 역할을 나누는 것이 아니다. (c) `ui_guide_checked` 자진신고가
  사라진다: 「읽을 곳」 한 줄로 대체.
- gen 단독의 사각 — data·sec·arch 가 따로 보던 관점은 gen 의 「볼 것」 표와 category 로만 남는다.
  07 `/code-review` 의 `dup_05=false` 건수가 그 사각을 센다(예측 7).
- 07 표본 — 초기 15런 escaped 16 은 리뷰어 4인 체제의 값이라 1인 체제의 기준선이 아니다.
- `rules_read` 없이 「읽었는지」는 다시 자진신고다 — 대신 지시문을 짧게 두고 03 패킷이 읽을 곳을
  세 줄로 준다.
- 귀속 없이 04 의 실패 출력은 요약 없이 작성자에게 간다 — 브리프는 끝 60줄·4,000자다.

**재검토 시점**: 첫 실물런(클론 동기화 뒤) **전에** 아래 예측을 PILOT-LOG 에 옮겨 적고, 런 뒤
대조한다. 기준선은 P2~P5 의 `state.json`·`events.jsonl`·`precheck.budget` 에서 런 전에 뽑는다.

| # | 예측 | 기준선(P2~P5) |
|---|---|---|
| 1 | 런 wall ≤ 40분 | 중앙 61.5분 (48.1 · 39.4 · 74.9 · 81.6) |
| 2 | 지시된 모델 호출 ≤ 8 | 중앙 13.5 |
| 3 | `format_reject` 0 — 남는 생산자는 01·05 의 `check_review` 뿐 | 4런 합 4 (출처였던 02·`rules_read` 는 소멸) |
| 4 | 03+04 분 ÷ 변경 줄 수(`precheck.budget.lines`) ≤ P2~P5 중앙값 × 1.3 | P5 = 35.4분/3,332줄 ≈ 1.06분/100줄. P2~P4 는 런 전에 채운다 |
| 5 | 04 수리 ≤ 1회 | P5 2회 |
| 6 | gen 이 Critical 을 낸다면 그 종류(계약 대조 · SQL 정독)를 적는다 | P4·P5 는 gen 이 전부 잡았다 |
| 7 | 07 `/code-review` 의 `dup_05=false` Major+ 건수 | 초기 15런 escaped 16 (major 11 · minor 23 · critical 0) |
| 8 | gen 이 내는 `TEST_MISSING_FAILURE_PATH` 류 findings 수 | 4런 합 49 (test 리뷰어 47 포함) |

되돌림 조건:
- 예측 4 가 기준 × 1.3 을 넘으면 `roles[]` 에 `test` 를 다시 넣는다(순차 test → impl) — `roles[]`
  를 배열로 남긴 이유다.
...
- 예측 5 가 빗나가거나(04 수리 > 1회) 작성자의 논리 오류로 gen 이 Critical 을 내면 2런에서
  ...
  (사용자 결정 2026-09-22 — 작성자는 `sonnet · high` 로 시작한다).
- 예측 7 이 초기 15런 기준선을 넘으면 리뷰어를 늘리지 말고 gen 의 「볼 것」 표에 그 관점의 행을
  더한다 — 그래도 새면 `reviewers[]` 에 둘째를 넣는다(`planned` 가 배열 순서로 전원을 부른다).

관련: [[ADR-H039]](남의 실측을 상속하지 않는다) · [[ADR-H058]](남긴 대조 둘의 실측) ·
[[ADR-H060]](02 유보 — 이 ADR 이 뒤집는다) · [[ADR-H069]](`verified` 의 되돌림) ·
[[ADR-H073]](선언만 있던 검사) · [[ADR-H074]](마지막 계기판 수리 — 이 ADR 이 그 계기판째 지웠다)

---

### ADR-H076: 덜어내기 사후 검증 — 실행기 결함과 집행 지점

**날짜**: 2026-09-22 · **상태**: 채택됨 · **구현 상태**: 구현됨 (2026-09-22 — PR 1: 실행기 결함 A1~A10 + 훅 · 2026-09-23 — PR 2: 선언·잔재 · PR 3: 문서 · fix · fix 2 · PR 4: 비용·시간 · PR 4 잔여. 전부 아래 추기)
...
코드·페이즈 파일·문서를 파일:줄로 대조하니 네 부류가 나왔다.

*A — 실행기 동작 결함 (첫 실물런을 막는 것, PR 1 에서 고쳤다)*

| # | 무엇 | 왜 결함인가 |
|---|---|---|
| A1 | 05 수리 봉투·계약 대조 봉투가 `gate --phase 04 --stage scoped` 를 냈다 | 페이즈 파일·feature.md 는 `05 --stage loop`([[ADR-H046]])인데 워커는 봉투를 따른다 → scoped 만 돌아 타입 에러가 PR 로 새는 결함이 그대로 재현 |
| A2 | 04 게이트 지문 `s["fingerprint"]` 가 write-only | 「재게이트 없이 넘어가면 06 이 막는다」·「재게이트를 잊을 수 없다」가 기계가 집행하지 않는 산문. 05 수리 뒤 재게이트 없이 approve→pr 통과 |
| A3 | `gate --phase 05` 를 `--stage` 없이 치면 리뷰 우회 | 05 체인이 돌고 `04_gate_report.json` 을 덮어쓴 뒤 05 를 passed 로 올려 06 으로 |
| A4 | 05 수리 뒤 전체 회귀가 안 돈다 | `--stage loop` 는 compile·scoped 만. PR 본문 「전체 회귀 실행됨」이 수리 전 코드의 수를 증언. `06-pr.md` 의 「백그라운드 회귀」 행은 Wave 1 이 지운 잔재 |
| A5 | 위험 명령 차단 훅이 동작하지 않음 | `$CLAUDE_TOOL_INPUT`·exit 1 을 전제 — 훅 입력은 stdin JSON 뿐이고 차단은 exit 2 |
| A6 | 봉투가 지워진 `retry` 를 안내 | Wave 1 이 지운 서브커맨드 |
| A7 | `--failed` 가드가 2회차에 `_r{n}` 파일명을 봄 | 페이즈 파일은 접미사 없는 이름 하나 → 제출이 있어도 허위 신고 통과 |
| A8 | contract-trace 선수리 루프에 상한 없음 | 파이프라인에서 유일한 천장 없는 루프. 반복마다 작성자 호출 1 + 재게이트 1 |
| A9 | 01 라운드 상한 `or 5` 폴백 | 같은 파일의 `_loop_max` 는 선언 없으면 exit 2 — M36 이 이름한 그 폴백 |
| A10 | 변경 집합이 비ASCII 경로·rename 을 잘못 읽음 | `-z` 없이 따옴표만 벗김 → `src/한글.ts` 가 8진 이스케이프 · `R old -> new` 가 한 경로. 지문은 `-z` 라 둘이 다른 파일 집합 |

*B — 읽히지 않는 선언·잔재 ([[ADR-H025]] 위반, PR 2)*: 페이즈 프론트매터의 미읽음 키(`parallel`·`kind`·`raw/json`·`one_round_allowed_when`·`rerun_failed_once`·`assert_tests_ran`·05 `review`·`docs`·`owner`·`approval`) · 01 의 `converge`/`loop` 상한 중복 · config·adapter 스키마의 죽은 키 · 죽은 코드·import · 지워진 `execute.py` 서술 · 영구 스킵 테스트 2건 · 봉투↔페이즈 파일↔feature.md 가 갈라진 자리 · gen 리뷰어의 모델·effort 가 어디에도 없음.

*C — 문서·ADR 부정합 (PR 3)*: 사실과 다른 문장 10곳(「8페이즈」 · 「훅은 셋」 · 「6분」 · doctor 가 CLAUDE.md 를 읽는다는 서술 등) · 현황 색인이 지워진 장치를 「구현」으로 · 삭제된 team-spec 앵커가 살아 있는 파일에 · PILOT-LOG 골격 불일치 · ADR-H075 예측표가 아직 PILOT-LOG 에 없음.

*B‴ — 비용·시간 (PR 4)*: 설계는 건전(모델 호출 상한 ≤12 · 05·06 앞 단계 무료 · 08 O(1)). 구현 비효율만 — `repo` 픽스처의 git 5회 spawn × 358 · config 재로드 · 전수 read · 비원자 상태 쓰기.

**결정**:

1. **A2·A4 는 기계로 집행한다** (사용자 결정 2026-09-22). 영수증은 둘이다 — `s["fingerprint"]`(loop: compile·scoped 가 이 코드에서 통과)와 `s["tests"]["fingerprint"]`(full: 전체 회귀가 이 코드에서 돌았다). `gate --stage loop` 성공이 앞을, `--stage full` 성공이 뒤를, 04 전체 게이트가 둘 다 찍는다.
2. **집행 지점은 셋** — `next`(05 진입·재진입: 게이트 안 된 코드에 리뷰어를 보내지 않는다) · `record --phase 05`(슬롯을 쓰기 전에 거부해야 재게이트 뒤 같은 제출을 다시 낼 수 있다 — record 는 멱등이 아니다) · `approve`(loop 는 exit 6, full 은 exit 3). `pr` 은 승인 지문을 대조하므로 전이적으로 덮인다. `precheck --phase 06` 은 런 없이도 도는 검사라 상태 검사를 얹지 않는다.
3. **페이즈 전이는 04 의 전체 게이트만 한다** — `gate --phase 05` 는 `--stage` 필수, 04 도 04 에 있을 때만 전체를 돈다.
4. **선수리 루프도 선언된 상한을 읽는다** — 05 `trace_loop {counter: trace_repair, max: 2}`. `loop` 를 배열로 바꾸지 않고 같은 모양의 둘째 키를 둔다(기존 `loop` 소비자를 안 건드린다). `counter_inc` 는 `used ≥ max` 에서 초과이므로 Critical 1회차는 수리, 2회차는 에스컬레이션 — `review_repair.max: 2` 와 같은 모양이다.
5. **제출 파일은 라운드와 무관하게 `05_review_{code}.json` 하나** — 원문·findings 는 record 가 슬롯에 옮기므로 덮어써도 잃지 않는다.
6. **훅은 파이썬 파일** — 훅 셸이 무엇인지 문서가 안 정하므로 상대 경로 + `python` 으로 셸 무관하게 둔다.
7. **gen 리뷰어는 에이전트 파일로**(D1) · **`fix` 레인은 유지**(D2) · 수정은 **PR 4개**로 — 첫 실물런을 막는 것(PR 1) → 원칙 위반 정리(PR 2) → 문서(PR 3) → 성능(PR 4, 실물런 뒤여도 된다). 결정 기록은 이 ADR 하나에 PR 2·3 이 추기한다.
8. **exit 7 은 쓰지 않는다** — 코드 어디에도 없다. 루프 초과는 기존대로 `st.escalate` → exit 10.

**트레이드오프**:
  - 05 수리가 있던 런은 06 전에 full 1회가 더 든다 — 그 1회가 없으면 PR 본문의 「전체 회귀 실행됨」이 거짓이다. 수리가 없던 런은 04 의 영수증이 그대로라 비용 0.
  - `next` 의 영수증 검사는 05 재진입마다 지문 계산 1회 — 소유 범위 파일 해시이고 리뷰어 호출 하나보다 훨씬 싸다.
  - trace 루프 상한 2 는 미검증 값이다(`review_repair.max: 2` 를 따랐다). 실측이 검사한다 — 선언이라 페이즈 파일에서 바꾼다.
  - 없는 영수증을 stale 로 읽으므로 이 변경 전 런은 05 에서 거부된다 — 템플릿에 실물 런이 없어 비용 0. `dispatched_fp` 의 「옛 상태 관용」과 다른 선택이다: 04 가 통과했으면 영수증은 반드시 있다.
  - `_r{n}` 이름을 버려 2회차가 1회차 `.raw.md` 를 덮어쓴다 — record 뒤에 원문을 읽는 코드는 없다(07·08 은 상태를 읽는다).
...
**추기 (2026-09-23 · fix PR 2 — fix PR 이 남긴 결함 셋)**:

*고친 것*:
1. **읽히지 않는 선언 넷을 지웠다** — `produces[].min_bytes`·`must_contain`(01 plan · 03 contract), `submit_checks[].from`·`unless`(03 `tests_required`). 크기와 절은 다음 페이즈 `requires` 가, `no_contract` 건너뛰기는 `_tests_required` 가 이미 집행했다 — 동작 변화 없음. `PRODUCES_KEYS` 에서 빼고 `SUBMIT_CHECK_KEYS = (id, on_fail)` 로 닫았다.
2. **05 가 계약 변경을 감지한다** — `next` 가 영수증 검사 뒤 계약 sha256 을 기준과 대조해 다르면 exit 10. 해시는 03·04·05 가 적기만 했다.
3. **차단 `CONTRACT_DEFECT` 는 exit 10** — 예전에는 다른 차단 지적처럼 exit 4 수리 봉투에 한 줄로 적혔다. `review_repair` 1 소모, 다음 회차 델타는 정해 둔다. 초과와 겹치면 두 사유를 한 번에 싣고, 수리 봉투를 안 내므로 그 회차의 차단 지적 목록도 사유에 싣는다.
4. **06 원격 선택지는 둘이고 base 부재를 잡는다** — ②(로컬 종료 · `PASS_WITH_GAPS`)는 고를 수단이 없었고(`pr` 에 플래그가 없고, 잠그지 않으니 `resume` 도 안 받는다) ③(중단)도 명령이 없었다. base 는 `ls-remote --exit-code` 로 원격에 직접 묻고, 없으면 `waiting_human(no_base)` · exit 9 · push 하지 않는다. doctor 의 「base 없으면 06 이 exit 9」가 이제 사실이다. `pr` 에 없는 `--resume` 안내도 바로잡았다.
5. **비밀 파일이 하나도 없으면 비강등 gap `secret_files_missing`** — `build_body` 가 부재 목록을 버렸다. `NON_DEMOTING_GAPS` 의 첫 정확 일치 항목이다.
6. **게이트 호출마다 `gate_stages` 이벤트** — `sec` 는 04 매 회차 덮어쓰이는 `04_gate_report.json` 에만 있었고 `--stage` 경로는 그 파일도 안 썼다. 이제 04 앞 회차와 05 재게이트의 스테이지 소요가 `events.jsonl` 에 남는다. `run_report` docstring 의 종료 코드(「0 / 3 / 6」 → 0 / 3 / 8 / 11)도 같은 커밋이다.

*결정*:
1. **선언은 읽게 하지 않고 지운다**(사용자 결정). 읽게 하면 01 record 가 짧은 플랜을 exit 8 로 거부해 fix PR 이 잠근 「03 진입 exit 3」이 바뀌고, 같은 검사가 두 지점에서 돈다.
2. **05 의 계약 기준은 04 전체 게이트의 계약이고, 05 에서 사람이 `resume` 하면 그 시점의 계약이다.** 처음 안(`resume` 이 기준을 비운다)은 재개 뒤 첫 `next` 가 **고치기 전** 계약을 기준으로 적어, 사람의 답에 따라 고친 다음 사이클에서 한 번 더 멈췄다(Plan 검토가 찾았다). 규칙은 「답에 따른 계약 수정은 `resume` 전에」 — 잊으면 한 번 더 멈출 뿐이다(fail-closed). lane_miss 로 계약 모드가 되면 기준을 비운다 — docs 런에 남은 옛 계약의 해시가 기준이 되는 오탐 경로다. 05 수리 작성자의 `CONTRACT_DEFECT`(03 기준 「메인이 고친다」)는 05 에서 exit 10 이라 「실패 시」에 행을 더했다.
3. **`CONTRACT_DEFECT` 는 차단 심각도만** — Minor 는 리뷰어가 차단이 아니라고 본 것이라 보고서로 간다. 카운터를 쓰는 것은 「차단 지적이 있던 회차 수 = 카운터」 불변식 때문이다(초과 경로도 소모 뒤 멈춘다).
4. **06 ② 는 구현하지 않고 뺐다**(사용자 결정) — 구현하면 06 을 PR 없이 통과시키고 07 을 건너뛰는 경로가 새로 생긴다. 원격 부재는 doctor 가 기동 전에 알린다.
5. **secret gap 은 전부 없을 때만** — 기본 설정이 `.env.local`·`.env` 둘이라 한쪽만 있는 것이 보통이고, 그때는 있는 파일의 값이 가려진다. 하나라도 없을 때 달면 매 런 소음이고 「패턴만 적용」이 거짓이 된다. 부재 목록 자체는 봉투 `data` 에 늘 싣는다.
6. **재게이트 소요는 이벤트로만**(사용자 결정) — 08 은 렌더하지 않는다. PILOT-LOG 「스테이지 실측」이 그 이벤트를 가리킨다. `round` 는 05 에서 04 의 repair+1 이라 싣지 않고 `log` 경로를 싣는다.

*트레이드오프*: 05 의 `resume` 은 계약과 무관한 에스컬레이션 뒤에도 그 시점의 계약을 받아들인다 — 조용히 받지 않게 `resume` 봉투에 적기만 했다(`contract_rebaselined`). base 확인은 `pr` 마다 `ls-remote` 1회(네트워크)이고, 판정 불가(원격 불통)는 막지 않는다 — push 실패가 말한다. doctor 는 「무료」 검사라 로컬 추적 ref 를 보므로 원격에서 지워진 base 를 PASS 로 볼 수 있다 — 06 이 잡는다. 봉투는 05 +355 · 06 +113 · 01 +11 바이트(「실패 시」 행 문구, 05 행 하나 추가).
...
**추기 (2026-09-23 · PR 4 잔여 — 번호 없던 결함 · 재겠다고 한 것의 배선 · 기준선 재셈)**:

*맥락*: PR 4 추기의 「남은 결함」 아홉이 백로그 번호를 못 받았다 — 「번호 없이 산문으로 남긴 잔여는
잊힌다」([[ADR-H073]])를 이 ADR 이 다시 어겼다. 코드와 대조하니 여덟이 사실이었고(보존 정책은 README 한
줄로 이미 닫혔다) 조사에서 둘이 더 나왔다 — `run_pr` 의 `build_context` 가 `config` 를 안 넘겨 한 번 더
읽는 것, `trace_contract._tests_for_entrypoint` 의 전수 읽기. 그리고 재검토 시점 (c) 는 **잴 배선이
없었다** — 영수증 거부는 봉투만 냈다. 첫 실물런 전에 막지 않으면 그 런이 답할 수 없다.

*고친 것*:
1. `mask.secret_values` 가 비밀 파일을 `with` 로 연다 — 핸들이 GC 에 맡겨져 ResourceWarning 이 났다.
2. `05_review.json`·`05_trace.json` 이 `cli._write_json`(원자)을 탄다 — 상태 쓰기만 원자였다.
3. `state._write_json` 이 도중에 실패하면 `.tmp` 를 지우고 원 예외를 다시 던진다(치우다 난 `OSError` 는 삼킨다). PR 4 트레이드오프 「`.tmp` 가 남는다」를 닫는다.
4. **모르는 프로브 kind 는 실패다** — 「건너뛴다」(fail-open)였고, `on_missing: warn` 이면 면제 gap `infra_skipped` 로까지 샜다. 모르는 kind 는 환경 부재가 아니라 설정 오류라 warn 분기를 타지 않는다. 스키마가 먼저 거부하므로 운영 경로는 안 바뀐다.
5. **`receipt_stale` 이벤트** — `next`·`record`·`approve` 의 영수증 거부(`receipt: loop|full`)와 `record` 의 지시 뒤 변경(`receipt: dispatch`, exit 3). 재검토 (c) 와 백로그 19 의 [[ADR-H046]] 행이 이것으로 센다. `record` 는 지시 지문 대조가 영수증 대조보다 앞이라, 실물에서 더 자주 나는 쪽은 `dispatch` 다.
6. `cmd_status` docstring 「항상 exit 0」 — 깨진 state 는 exit 1 이다(PR 4 결정 4).

*번호 준 것* (사용자 결정 — 실측 대기): 백로그 **36**(어댑터·config 재로드 셋) · **37**(`secret_values` 2회) · **38**(유닛×파일 전수 읽기 넷). 첫 실물런이 판정한다.

*수용한 것* (번호 없음 — 미룬 것이 아니라 고른 것): doctor 의 base 판정은 로컬 추적 ref 다(fix PR 2 트레이드오프). doctor 는 무료 검사이고 06 이 `ls-remote` 로 잡는다.

*예측 7·8 기준선 재셈* — 클론 `banana-island-ops` 의 원장 `findings.jsonl`(236행)과 08 보고서 15개에서
셌다. 원장은 클론 동기화가 지우므로 그 전에 셌다(이력은 그 리포의 git 에 남는다).
- **예측 8** 「4런 합 49 (test 리뷰어 47 포함)」 — 49 는 맞고 내역이 틀렸다. 49 = 리뷰어 48 +
  contract-trace 1 이고, test 리뷰어 몫은 **39** 다(47 은 test 리뷰어의 전 카테고리 합 — 백로그 28).
  예측 8 은 gen 의 산출이므로 기준은 **리뷰어 48 = 12.0/런**(P2~P5 = 15 · 5 · 14 · 14).
- **예측 7** 「초기 15런 escaped 16 (major 11 · minor 23 · critical 0)」 — **다른 두 양을 한 칸에 섞었다.**
  「16」은 [[ADR-H054]] 의 `escaped_05`(07 의 비중복 Critical+Major — 옛 `review07.escaped`)이고 critical 2 를
  포함한다. 「major 11 · minor 23 · critical 0」은 원장의 07 행 전체(중복·minor 포함, 34행 · 8런)다. 예측 7 의
  `dup_05=false` Major+ 에 맞는 것은 앞의 것이다. 클론의 파일로 재현하면 08 보고서 `escaped_05` 합 **13** /
  표본 **9런**(클론 4런 전 · 「내장 리뷰 생략, 표본 아님」 2런 제외) = **1.44/런**, critical 2(`5ef4` · `e7ff`).
  「16건/15런」은 추출 이력 쪽 집계라 이 클론의 파일로는 재현되지 않는다 — 기준은 재현되는 쪽으로 한다.
- 본문(H075 예측표)의 숫자는 고치지 않는다([[ADR-H039]] 결정 1) — 예측표의 정본은 PILOT-LOG 이고 거기를
  고쳤다. 채점 규칙(셈 출처 · 런당 정규화)도 같은 곳에 **런 전에** 적었다 — 런 뒤에 정하면 채점이 아니라 해석이다.

*검증*: 테스트 커밋 시점 10 failed(새 테스트만 빨강 · 가드 1 초록) → 전부 초록 · `lint-phases`·`doctor`·`harness.py doctor` exit 0.

---

### ADR-H077: 서브에이전트 셋을 Opus 5.5 로 — 작성자도 올린다

**날짜**: 2026-09-23 · **상태**: 채택됨 · **구현 상태**: 구현됨 (2026-09-23 — 에이전트 셋의 `model:` ·
`TestAgentFrontmatter` · README · PILOT-LOG 되돌림 조건 3)

**맥락**: 2026-09-22 Claude Opus 5.5(`claude-opus-5-5`)가 나왔다. 단가는 $4 / $20(MTok 입력 / 출력)으로
Opus 5($5 / $25)보다 싸고 Sonnet 5($2 / $10)의 2배다 — [[ADR-H061]] 이 「검사자 opus 는 sonnet 대비
2.5배」를 전제로 작성자를 sonnet 에 둔 격차가 좁아졌다. 공식 문서는 에이전트 코딩에서 Opus 5 보다 적은 토큰으로
같은 결과를 낸다고 한다(이 리포의 실측은 0). 하네스 구조상 화면 품질은 04 게이트(테스트·타입·린트)와 05
리뷰어(diff)가 보지 못한다 — 로직은 계약과 테스트가 잡지만 UI 는 작성자가 유일한 방어선이다. 어제의 사용자 결정
(「작성자는 `sonnet · high` 로 시작, 되돌림 조건 3 이 걸리면 opus 로 올려 2런 비교」, [[ADR-H075]])은 아직 한 런도
돌지 않았다 — 지금 바꿔도 잃는 측정값이 없다. 프론트매터 `model:` 은 전체 모델 ID 를 받지만, `opus` 별칭이 지금
어느 모델로 풀리는지는 Claude Code 문서에 없다.

**결정** (사용자 결정 2026-09-23):
1. **셋 다 `claude-opus-5-5`** — `impl-writer`(03 · 04 수리) · `plan-reviewer`(01) · `general-reviewer`(05).
   [[ADR-H061]] 결정 1 의 「작성자는 싸게」를 개정한다. 「검사자는 비싸게」는 그대로다.
2. **별칭이 아니라 전체 ID 로 고정한다** — 별칭의 해석이 문서에 없어 5.5 를 보장하는 방법은 이것뿐이다. 다음 Opus
   가 나와도 자동으로 따라가지 않는다. 그때는 이 세 줄과 `TestAgentFrontmatter` 를 함께 고친다.
3. **effort 는 셋 다 `high` 그대로** — 한 번에 변수 하나만 바꾼다. Opus 5.5 는 같은 effort 에서 Opus 5 보다 더
   생각하고(공식 문서: `medium` 이 Opus 5 `high` 이상), 기본값이 `medium` 이다. 내릴지는 첫 런을 보고 정한다.
4. **[[ADR-H075]] 되돌림 조건 3 을 반대 방향으로** — 정본인 PILOT-LOG 를 고쳤다. 사람이 런 wall(예측 1)이나
   사용량이 과하다고 판단하면 먼저 `impl-writer` 의 `effort: medium`, 그래도 과하면 `model: claude-sonnet-5` 로 내려
   두 런을 비교한다. 작성자 논리 오류 Critical(예측 6)은 기록만 한다.
5. 메인 세션(`/model`)과 07 `/code-review`(내장 스킬)는 리포 파일이 정하지 않는다 — 건드리지 않았다.

**트레이드오프**:
- 예측 4·5·8 의 기준선(P2~P5)은 sonnet 작성자 런이다 — 첫 런의 결과는 덜어내기와 모델 교체가 섞인다.
  어느 쪽의 효과인지 한 런으로는 가를 수 없다.
- 작성자와 리뷰어가 같은 모델이다. 문맥이 다른 에이전트라 [[ADR-H075]] 의 독립 관측은 유지되지만 모델의 맹점은
  겹칠 수 있다 — 예측 7(07 의 비중복 Major+)로 본다.
- 하네스는 토큰·비용을 재지 않는다([[ADR-H011]] 소멸) — 되돌림의 비용 판단은 사람 몫이다. 작성자는 `model_used`
  자진신고도 없다(blind spot).
- 「UI 에 강하다」의 실체: 공식 문서가 내세우는 강점은 에이전트 코딩과 시각 **입력**(스크린샷·차트) 판독이다.
  `impl-writer` 는 스크린샷을 보지 않는다. 프론트엔드 **생성**은 지시가 없으면 몇 가지 기본 스타일로 수렴하고
  「뻔한 AI 룩을 피하라」 같은 일반 지시는 잘 듣지 않는다 — 피할 패턴을 구체적으로 적은 `UI_GUIDE.md` 안티패턴
  목록이 모델 교체만큼 필요하다.

**재검토 시점**: 첫 실물런 — 08 「자진신고 모델」에 `claude-opus-5-5` 가 찍히는지(리뷰어 둘), 예측 1·4·5, 사람이
보는 사용량. 되돌림 조건은 PILOT-LOG 조건 3.

---

### ADR-H078: 06 승인을 설정 스위치로 — `vcs.pr_approval`

**날짜**: 2026-09-24 · **상태**: 채택됨 · **구현 상태**: 구현됨 (2026-09-24 — `run_pr` 4단계 · config·스키마 ·
`TestPr06AutoApproval` · 06-pr.md · feature.md · README)

**맥락**: 06 의 사람 승인이 정상 경로에서 **유일한 필수 중단점**이다 — 다른 exit 9(원격·base 부재, precheck 정책)는
예외 경로다. 그래서 06~08 을 무인으로 완주할 수 없다. 그런데 되돌릴 수 없는 관문인 머지는 여전히 사람 몫이고, 피처
브랜치의 PR 은 그 자체가 리뷰 요청이다. 승인 화면(`_approval_prompt`)이 보이는 것은 파일·라인 수 · gap · 예상
등급뿐이고 전부 PR 본문에 다시 실린다 — 파일 목록조차 없어 사람이 그 시점에 막을 근거가 화면에 없다.
[[ADR-H015]] 는 `--auto` 를 "사람이 미리 켜 두는 것" 으로 두었지만 **켤 자리가 없었다** — config 키도 `init`
플래그도 없고 모델은 금지다. 설계 의도가 구현되지 않은 채 죽은 경로였다.

`approve` 에는 사람 판단 말고 기계 검사가 하나 들어 있다 — 게이트 영수증(loop exit 6 · 전체 회귀 exit 3).
`pr` 은 그것을 따로 보지 않고 승인 지문으로 **전이적으로** 덮는다. 승인을 통째로 지우면 이 검사가 같이 사라진다.

**결정** (사용자 결정 2026-09-24):
1. **`vcs.pr_approval`: `"auto"` | `"user"`.** 스키마가 어휘를 지킨다 — 오타는 doctor 가 막는다.
2. **템플릿은 `"auto"` 를 명시한다. 키가 없으면 `"user"` 다** — 이미 클론한 프로젝트가 템플릿 동기화만으로 무인
   push 가 되지 않게.
3. **`auto` 가 지우는 것은 사람 대기뿐이다.** 승인이 없거나 승인 지문이 지금과 다르면 `pr` 이 영수증을 **자기
   이름으로** 먼저 보고(낡았으면 exit 6/3, push 안 함) 깨끗하면 `run_approve(auto=True)` 로 기록한다. 기록 형식
   (`mode: auto` · 지문 · `scope: push+pr` · 등급)은 사람 승인과 같다 — 08 과 종료 보고의 user/auto 구분이 그대로 산다.
4. **철회는 되살리지 않는다.** `revoked_at` 이 있으면 `auto` 여도 exit 9 다 — 사람의 "멈춰" 를 정책이 덮지 않는다.
5. **모델은 여전히 `approve --auto` 를 치지 않는다.** 켜는 자리는 `harness/config.json` 이고, 그 파일은 이미
   `/feature` 의 수정 금지 목록에 있다.

**트레이드오프**:
- 템플릿을 클론한 사람이 **무인 push + PR 생성**을 기본으로 받는다. ROADMAP §1 이 승인을 건너뛰는 순차 실행기를
  뺀 선례([[ADR-H005]] · [[ADR-H037]])와 방향이 다르다. 차이는 범위(push + PR 만 — 머지·force-push·자동 리베이스는
  여전히 금지)와 되돌리기(`"user"` 한 줄)다.
- push 는 커밋된 코드를 공개한다. 마스킹은 PR 본문만 덮는다 — 비밀 파일이 커밋되면 막을 사람 시점이 없다. 다만 지금
  승인 화면도 파일 목록을 보이지 않아 그 시점에 잡을 수단이 원래 없었다.
- Claude Code 기본 권한 모드에서는 `cli.py pr`(push)과 forge 의 PR 생성이 여전히 도구 권한 프롬프트를 거친다
  (`.claude/settings.json` 에 허용 목록 없음) — 사라지는 것은 하네스의 관문이다.
- 사람 승인 뒤 철회했다가 `approve` 로 다시 승인한 런은 `revoked_at` 이 남아 그 뒤 코드 변경에 자동 재승인이 걸리지
  않는다(exit 6 → 사람). 드문 경로라 두었다.

**재검토 시점**: 첫 실물런 — 06 의 `waiting_human(approval)` 이 0 인가, 자동 승인으로 나간 PR 을 사람이 닫은 일이
있는가. 있으면 기본값을 `user` 로 되돌리는 근거다.

---

### ADR-H079: 컴포넌트 스펙 — PRD 는 Must 목록, US 하나 = `docs/specs/` 파일 하나

**날짜**: 2026-09-24 · **상태**: 채택됨 · **구현 상태**: 구현됨 (2026-09-24 — `docs/specs/_template.md` · PRD §3 ·
feature.md 1단계 · 03·05 역할 템플릿 · impl-writer · general-reviewer · README · ROADMAP · CLAUDE.md)

**맥락**: SDD 의 계층형 스펙(마스터 → 컴포넌트)을 템플릿에 들인다. 지금은 PRD §3 에 모든 US 와 AC 가 한데 있고,
작성자는 「`docs/` 직속 .md 전부」로 PRD 전체를 읽는다(`03-implement` 「읽을 곳」). 문서를 미리 전부 채우면 첫 런의
회고가 뒤 US 를 바꿀 자리가 없고, 한 런이 읽는 AC 는 그 런의 것보다 훨씬 많다. 그런데 AC 를 PRD 에서 빼는 순간
**AC 가 런에 닿는 길이 끊긴다** — 스펙 파일을 런에 잇는 변경이 필수다.

두 가지를 교차검증했다. 04 게이트는 AC 를 모른다 — AC 커버는 plan-reviewer·gen 의 판단이지 기계 검사가 아니다
([[ADR-H075]] 가 커버리지 대조를 「잡은 결함 0」으로 지웠다). 규칙 승격 게이트도 [[ADR-H075]] 에서 지워졌다 — 회고는
사람이 하는 수동 루프다.

**결정** (사용자 결정 2026-09-24):
1. **PRD §3 은 Must 목록이다.** US 한 줄 요약 · 스펙 링크 · 상태. **AC 는 스펙에만** — 두 곳이면 갈라진다. 상태는
   다음 US 의 스펙 커밋에서 함께 고친다(`main` 직접 push 금지라 상태만 고치는 PR 을 따로 만들지 않는다).
2. **US 하나 = `docs/specs/US-xxx-<slug>.md` 하나, 차례가 온 것만 그때 쓴다.** 틀은 `docs/specs/_template.md` —
   목적·성공 기준 / 동작 / AC(Given/When/Then) / 엣지 케이스 / 제약 / 범위 밖 / 인터페이스 / 열린 질문.
3. **스펙 파일 자체를 요청으로 동결한다.** 요청이 스펙 경로 하나뿐이면 `/feature` 는 요청 파일을 새로 쓰지 않고
   그 파일을 `init --request-file` 로 준다. `run_init` 은 존재하는 파일이면 받고 바이트·sha256 을 박으므로
   **실행기 0줄**이다. 「동결 요청 = 유일한 앵커」가 그대로 참이라 `plan-reviewer.md`·`01-plan.md` 도 무변경 —
   「요청의 요구 하나하나가 플랜의 어느 절에」가 곧 「AC 하나하나가」가 된다. 런 중에 누가 `docs/specs/` 를 고쳐도
   런의 기준은 동결본이다.
4. **03·05 에 요청 원문을 잇는다.** 03 「읽을 곳」 넷째 줄 = 요청 원문(AC·엣지 케이스로 테스트 입력을 고르고,
   시그니처는 계약이 이긴다). 05 역할 템플릿에 `## 요청 원문 (경로)` — gen 의 「요청 정합」이 이미 약속한 일인데
   입력에 요청이 없던 빈틈이다. 탐색 금지는 그대로다.
5. **작업 방식의 단일 출처는 README 「빠른 시작」**이다 — 부트스트랩(클론 → 문서 → `init`·`doctor`) 한 번 뒤 US 마다
   스펙 → `feat-us-xxx-<slug>` 브랜치에 스펙 커밋 → 새 세션 `/feature <스펙 경로>` → 회고.
6. **헤드리스는 적용하지 않는다** — 고려할 수 있는 질문으로 ROADMAP §7 에 남긴다.

**트레이드오프**:
- **경로 하나일 때만 스펙이 동결된다.** 경로에 문장을 붙이면 기존 흐름대로 문장이 동결되고 스펙 본문은 런에 없다.
  추가 조건은 스펙에 적어야 한다 — 산문 규칙이다.
- 작성자의 `docs/**` 수정 금지는 산문이고 기계가 막지 않는다. `main_owned_paths` 는 doctor 의 겹침 검사에서만
  읽힌다 — 런 중 스펙이 고쳐지면 diff 에 드러날 뿐이다(런의 기준은 동결본이라 판정은 흔들리지 않는다).
- 03 「읽을 곳」이 [[ADR-H075]] 의 세 줄에서 넷이 되고, 05 입력이 는다 — 요청 원문은 한 US 크기라 작다.
- 브랜치에 먼저 커밋한 스펙 줄이 `precheck` 줄 수(`at_05.lines`)에 들어간다 — PILOT-LOG 예측 4 채점에서 뺀다.

**재검토 시점**: 스펙 요청 첫 런 — `01_plan.md` 가 스펙의 AC 마다 절을 대응시켰나, 07 escaped 중 AC 미충족 건수.
대응이 빠지거나 AC 미충족이 07 에서 나오면 plan-reviewer 의 「읽을 곳」에 AC 대응을 명시하는 근거다.
