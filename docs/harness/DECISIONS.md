...
**날짜**: 2026-09-18 · **상태**: 제안됨 · **구현 상태**: 구현됨 (이 브랜치)
...
**날짜**: 2026-09-18 · **상태**: 제안됨 · **구현 상태**: 구현됨 (2026-09-18 — 02 `loop.max` 는 1 → 2 로 뜻이 "왕복 수" 에서 "Critical 제출 상한" 으로 바뀌었다. 동작은 같다: 두 번째 Critical 에서 멈춘다)
...
**날짜**: 2026-09-18 · **상태**: 제안됨 · **구현 상태**: 결정 1·2·3 구현됨 · 결정 4·5 미결
...
**날짜**: 2026-09-18 · **상태**: 제안됨 · **구현 상태**: 구현됨 (결정 1·2 회고 브랜치 · 결정 3 은 2026-09-18 후속 — `ledger.write_deferred` · `deferred_overlap`)
...
**날짜**: 2026-09-18 · **상태**: 제안됨 · **구현 상태**: 구현됨 (2026-09-18 — 결정 4 는 `08-report.md` 의 「보고서를 기능 PR 에 싣지 마라」 를 뒤집었다: 런 기록·PILOT-LOG·deferred.md 는 기능 PR 에 실리고 닫힌 런의 `pr` 재실행이 그것을 검사한다)
...
관련: [[ADR-H007]] · [[ADR-H029]](예산의 사유는 원장이 말한다) · [[ADR-H045]](lines_max 400→1000 의 결정)

---

### ADR-H055: 워커의 규칙 읽기를 게이트가 묻는다

**날짜**: 2026-09-18 · **상태**: 제안됨 · **구현 상태**: 구현됨 (2026-09-18 — `cli._rules_read_expected` 는 `rules_dir` **직속** `.md` 만 본다: `docs/harness/**` 의 ADR·원장을 읽으라는 뜻이 아니다)
...
3. **baseline 은 검사별로 잰다** (`ledger.in_baseline_for`). 원장 전체 `distinct_runs` 하나로 재면
   파일럿 원장(12런)을 물려받은 클론에서 새 검사가 첫 런부터 `deferred` 로 들어가고, 재지 않은
   오탐률이 두 런 만에 승격·검토 입력을 채운다. 이제 "첫 3런" 은 **그 검사가 지적을 낸 런**이다
   (`warn_only` 행도 원장에 쌓이므로 센다). 기존 두 검사도 같은 함수로 옮겼다 — 이미 3런을 넘은
   원장에서는 동작이 같다.
...
지적이 난다(거짓 통과보다 거짓 지적 쪽으로 기운다 — baseline 기간이 그 비율을 잰다).
