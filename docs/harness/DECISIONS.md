...
| 24 | 코드 | `implied_exports` 면제가 `is_entrypoint_file()` 에만 걸린다 — App Router 의 `page.tsx`·`layout.tsx` 로 넓히고 `metadata`·`generateMetadata`·`generateStaticParams`·`viewport` 를 목록에 더한다. 타입 전용 export 는 별건(아래 본문) | `out_of_contract` 가 프레임워크 관례 export 를 0건 잡는다 — `page.tsx` 픽스처로 잠근다 |
| 26 | 코드 | 사람이 05 에서 고른 precheck 판단을 지문과 함께 상태에 적고, 06 이 같은 지문이면 다시 묻지 않는다 | `waiting_human {"reason":"precheck_policy"}` 가 **런당 1회 이하** |
| 30 | 코드 | 에스컬레이션 선택지에 「좁게 보강하고 진행」을 더한다. 문자열 1개 | 다음 에스컬레이션에서 사람이 「기타」를 고르지 않는다 |
...
24. **`out_of_contract` 의 면제가 진입점 파일에만 걸린다** — 클론 4런. [[ADR-H049]] 가 연 면제는
...
26. **같은 precheck 정책을 05·06 에서 두 번 묻는다** — 클론 4런. exit 9 는 상태를 잠그지도 카운터를
...
30. **에스컬레이션 선택지가 실제 답을 담지 않는다** — 클론 4런. 제시 옵션은 셋(계약 결함 의심 ·
