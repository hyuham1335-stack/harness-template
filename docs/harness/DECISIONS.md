...
| [[ADR-H061]] | 작성자는 싸게, 검사자는 비싸게 | 채택됨 | 부분 승계 | 소멸: `config.models` 등급표 ([[ADR-H075]]). 모델·effort 는 에이전트 프론트매터가 정한다(작성자 sonnet · 리뷰어 opus) |
...
6. **배선 닫힘 (2026-09-19 · [[ADR-H064]])** — **05 `review_repair` 작성자의 모델 등급** — [[ADR-H061]]. `_instruct` 호출은 00·01·03·04 넷뿐이라 05 수리 작성자는 `config.models` 밖이다. 닫힘: `state.models.instructed × counters.repair` 5런 뒤 빈도를 보고 정한다.
    **닫힘 (2026-09-23 · [[ADR-H076]] PR 4 잔여) — 대상 소멸.** 등급 **값**을 정할 `config.models` 가 [[ADR-H075]] 로 지워졌다. 작성자 모델은 `impl-writer` 프론트매터가 정하고, 올릴지는 [[ADR-H075]] 되돌림 조건 3 이 정한다.
