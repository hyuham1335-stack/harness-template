...
8(클론의 `OTHER` 관측 대기, 파동 2 에서 이동) · 7(오탐 원인 규명 → `required_tests` 1줄 + `BASELINE_CHECKS` 제외, [[ADR-H058]] 결정 7 을 뒤집으므로 새 ADR) · 11(코드 완비 — 아래 정정) · 15 · 19 표(지표 14개 **전부 기록 배선이 있다**, 원인은 런 0) · 파동 1 의 6 등급값 · **28 본체**(리뷰어별 수확 편차 — 곁가지는 [[ADR-H073]] 이 닫았다) · **29**(`nothing_locked` 기계화) · **34**(`instruction_slot_budget` 값 — **33 이 먼저다**, 세는 범위와 방식을 고치기 전 값은 무의미하다). 셋 다 2026-09-21 배치. **2026-09-23: 7·8·11·15 는 대상 소멸로 닫힘**([[ADR-H076]]).

**파동 4 — 밖 · 보류**

10(클론 리포에서 `git log --merges` 대조, 하네스에 넣어도 `cmd_status` 한 칸) · 14(`secondary` 읽는 코드 0, 공급자 미정) · 16(`.claude/settings.json` 의 `PreToolUse` matcher 는 `Bash` 뿐) · 17(아래 정정) · 18 · **32**(무엇이 「제출 검사」인가 — 결정이 앞선다) · **35**(워커별 규칙 파일 선별 — 매핑 축과 누수 측정법 둘 다 미정. 2026-09-21 배치) · **2(b)** — 02 시점에 계약 파일이 없다(03 이 처음 `produces`). "02 에 계약을 넘긴다" 는 봉투 한 줄이 아니라 계약 작성 시점을 01/02 로 옮기는 변경이고 페이즈 파일 3개 + `_record_02` 대조 대상 + `lint-phases` 의 `produces_key` 유일성까지 60~120줄급이다. **2026-09-23: 14·2(b) 는 대상 소멸로 닫힘, 17 은 정정**([[ADR-H076]]).
