...
| [[ADR-H054]] | 예산·상한 값의 15런 근거 | 근거 기록 | 해당 없음 | `files_max` 선택지는 사람 결정 대기 → 백로그 13 |
...
13. **`budget.files_max: 10`** — [[ADR-H054]]. 15런 중 6런(40%) 이 exit 9 였고 `files_max` 가 주범이다. 선택지 (a) 10 → 20 (b) 테스트 파일은 파일 수에서 제외 (c) 파일당 평균 줄 < 30 이면 files 면제.
