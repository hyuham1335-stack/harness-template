...
| [[ADR-H070]] | 클론 4런이 연 결함 셋 — 델타 재리뷰 · 02 인용 대조 · 규칙 집합 | 채택됨 | 구현 | 백로그 20 · 21 · 23 닫음. 23 의 스냅샷 안은 창을 옮길 뿐이라 버렸다 |
| [[ADR-H071]] | 클론 4런이 연 마찰 셋 — 관례 파일 면제 · precheck 재질문 · 에스컬레이션 메뉴 | 채택됨 | 구현 | 백로그 26 · 30 닫음. 24 는 타입 전용 export 만큼 열린 채다 |
| [[ADR-H072]] | 결정 대기 넷 — 등급 신호 · 07 빈손 호출 · 귀속 기본값 · 타입 전용 export | 채택됨 | 구현 | 백로그 22 · 24 · 25 · 27 닫음. 22 는 「등급 고정은 결함이 아니다」로 닫았다 |
...
8(클론의 `OTHER` 관측 대기, 파동 2 에서 이동) · 7(오탐 원인 규명 → `required_tests` 1줄 + `BASELINE_CHECKS` 제외, [[ADR-H058]] 결정 7 을 뒤집으므로 새 ADR) · 11(코드 완비 — 아래 정정) · 15 · 19 표(지표 14개 **전부 기록 배선이 있다**, 원인은 런 0) · 파동 1 의 6 등급값 · **28 본체**(리뷰어별 수확 편차 — 곁가지는 [[ADR-H073]] 이 닫았다) · **29**(`nothing_locked` 기계화) · **34**(`instruction_slot_budget` 값 — **33 이 먼저다**, 세는 범위와 방식을 고치기 전 값은 무의미하다). 셋 다 2026-09-21 배치.
...
22. **닫힘 (2026-09-21 · [[ADR-H072]] 결정 1) — 「결함이 아니라 정확한 신호」로 닫았다** —
    ...
    **런마다 같은 값은 판단 근거가 아니다.** 같은 병을 [[ADR-H069]] 가 `adapter_unverified` 에
    ...
    (`_note` 는 `stage_state` 가 세지 않는다) (c) `instruction_slot_budget` 값을 고친다(ROADMAP §7-2).
...
34. **`instruction_slot_budget` 의 값** — 표본. **백로그 33 이 먼저다.** 12 는 원본
    ...
    **틀린 숫자에 맞추는 것**이 된다. 33 뒤에 클론 N런의 `instruction_slots` **파일별 분포**를
    ...
    ROADMAP §7-2 가 같은 것을 열린 질문으로 들고 있다.
