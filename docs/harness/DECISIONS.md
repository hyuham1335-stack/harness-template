...
| # | 성격 | 규모 | 닫힘 조건 |
|---|---|---|---|
| 20 | 코드 | `cli.py` 의 05 판정에서 `rounds_planned[round+1]` 을 **escalate 의 `return` 앞으로** 옮기거나 `run_resume` 이 세운다. 1~3줄 + 회귀 테스트 1개 | 에스컬레이션 → resume → 3라운드에서 `_planned_for_round` 가 **1명**을 돌려주고, `round_reviewers["3"].planned == 1` |
| 21 | 코드 | 02 의 `quote` 대조 대상을 프롬프트와 맞춘다. 검사기를 바꾸는 쪽(raw 대조)과 프롬프트를 바꾸는 쪽(플랜 원문) 중 **프롬프트 쪽이 1줄**이다. 겸사겸사 02 가 `verdict.check_review` 를 부르지 않아 `mode`·`severity` 어휘가 무검증인 것도 같이 본다 | 02 `check_fail {"errors": …}` 가 **0회** |
| 23 | 코드 | `rules_read` 기대 해시를 **런 init 시 스냅샷**으로 고정하고 상태에 적는다. `docs/PIPELINE-LOG.md` 처럼 **하네스 자신이 쓰는 파일**은 `rules_dir` 집합에서 뺀다 | 런 도중 `docs/*.md` 가 바뀌어도 03 제출이 통과하고, **규칙이 진짜 바뀐 경우에는 여전히 반려**된다 |
...
20. **에스컬레이션 뒤 3라운드가 리뷰어 전원을 다시 부른다** — 클론 4런이 연 것. **결함이다.**
...
21. **02 의 `quote` 검사가 페이즈 프롬프트와 다른 문서를 가리킨다** — 클론 4런이 연 것. **결함이다.**
...
23. **`rules_read` 해시 대조가 병렬 런을 상정하지 않는다** — 클론 4런이 연 것. **결함이다.**
