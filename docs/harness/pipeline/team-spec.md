...
6. **`submit_checks[].id` 는 닫힌 어휘이고 단일 출처는 `cli.SUBMIT_CHECKS` 다** ([ADR-H073](../DECISIONS.md)). 각 항목은 그 검사를 실제로 내는 함수를 `"모듈:이름"` 으로 가리키고, `lint-phases` 가 그 포인터를 **`importlib` 로 해석한다**(호출하지는 않는다) — 레지스트리가 기계의 소재에 대해 거짓말할 수 없게 하기 위해서다. `on_fail` 은 §2.3 종료 코드표의 값이어야 하고 레지스트리가 쥔 값과 **대조된다**. 선언이 어휘 밖이거나 `on_fail` 이 어긋나면 **exit 2** 다. **이 선언은 [ADR-H025](../DECISIONS.md) 의 「선언은 읽히거나 거부되고, 무시되지 않는다」를 빠져나가 있었다** — `loop` 어휘와 달리 읽는 코드가 없었다.
7. **`produces[]` 의 키 집합도 닫혀 있다.** `key`·`path`·`kind`·`min_bytes`·`must_contain`·`unless`·`owner` 밖의 키는 **exit 2** 다. 한때 여기 `schema` 가 열 곳에 있었는데 **읽는 코드도, 그 이름의 스키마 아티팩트도 없었다** — 제출물 검증은 페이즈별 인라인이고 어느 함수가 하는지는 규칙 6 의 레지스트리가 가리킨다. `schema` 는 그 사실을 중복해서 **틀리게** 말하던 이름이라 걷어냈다 ([ADR-H073](../DECISIONS.md)).

**레지스트리가 증명하는 것과 증명하지 않는 것.** 증명하는 것은 **「그 검사의 구현 자리가 아직 있다」**이고, 증명하지 않는 것은 **「그 선언이 그 런에서 실제로 돌았다」**이다. 후자는 각 기록자가 자기가 돌린 id 를 영수증으로 남겨야 말할 수 있고, 그것은 미구현 백로그 31 이다. **반쪽짜리 보증을 온전한 것처럼 적지 않는 것**이 §E14 의 규율과 같다.

`lint-phases`가 잡는 것: JSON 파싱 실패 · 미해결 플레이스홀더 · `on_success` 고아·사이클 · 어댑터에 없는 스테이지 이름 · 필수 H2 섹션 누락 · 산출물 경로 240자 초과 · `taxonomy` 코드 유니크·어휘·참조 실재 · `approval != "none"` + 비대화형 조합 · **`submit_checks` 의 어휘·`on_fail` 대조·구현 포인터 해석** · **`produces[]` 의 알 수 없는 키**.
...
**이 표는 `submit_checks[].on_fail` 의 어휘이기도 하다** ([ADR-H073](../DECISIONS.md)). 페이즈가 선언한 `on_fail` 이 이 표 밖이거나 `cli.SUBMIT_CHECKS` 가 쥔 값과 다르면 `lint-phases` 가 **exit 2** 로 거부한다 — 「페이즈는 `4` 라는데 코드는 `8` 을 낸다」가 조용히 남는 것이 [ADR-H025](../DECISIONS.md) 가 이름한 드리프트다.
...
- **`source: "external"` 인 finding 의 `quote` 는 봇 페이로드 원문의 부분문자열이어야 한다** ([ADR-H073](../DECISIONS.md)). `review07` 이 받은 외부 파일을 `07_external.raw.json` 으로 런 디렉터리에 남기고, `record` 가 그 원문과 대조한다 — 없으면 **exit 8**. 01·05 가 `.raw.md` 에 대고 하는 검사와 같은 규율이다: **요약본을 낸 쪽이 인용을 지어낼 수 있으면 "밖이 이렇게 말했다" 가 대조 불가능한 주장이 된다** (불변식 8). **`code-review` 와 `human` 출처는 대조하지 않는다** — 전자의 원문은 내장 리뷰어의 출력이라 저장되지 않고, 후자는 수리 대상이 아니라 보고 대상이다. **건초더미가 없는 것을 검사한 척하지 않는다.**
...
| 07 | `external` finding 의 quote 위조 | 제출물 | exit 8, 재제출. 대조 대상은 `review07` 이 남긴 `07_external.raw.json` (ADR-H073) |
