...
| **05 코드리뷰** | 04 가 "동작하는가" 를 봤다면 05 는 "계약대로 만들었는가, 봐야 할 리뷰어가 실제로 봤는가" 를 봅니다. 비용이 들지 않는 `precheck` 부터 돌리고, 변경된 파일에 해당하는 리뷰어만 병렬로 부릅니다. 레인별 상한을 넘으면 우선순위(일반·데이터·보안·테스트·구조·문서) 뒤쪽부터 빠집니다. 수리 작성자는 봉투가 찍은 `roles` 등급 모델로 부릅니다 (ADR-H064) | 계약 대조 결과, 리뷰 결과, 승격 후보 | 재검토는 2회까지입니다. 리뷰어가 전부 실패했거나 호출된 리뷰어가 0명이면 등급이 `PASS_WITH_GAPS` 로 내려갑니다 |
