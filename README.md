...
레인 예측 → 계획 → 계약 확정 → 구현 → 게이트(테스트·린트) → 코드 리뷰 → PR 생성 → 보고

맨 앞의 레인 예측(00)이 요청을 `docs` · `fix` · `small` · `normal` 로 나누고, 문서만 바뀌는
요청은 계획 리뷰어 · 구현 역할을 건너뜁니다. `fix` 는 사람이나 모델이 고르는
버그 수리 레인으로 계획 1회차 · 리뷰어 1명으로 짧게 돕니다. 예측이 빗나가면 뒤 단계가
그것을 잡아 등급에 남깁니다.
...
| 트리아지 임계값 · 모델 등급 | 언급된 소스 경로가 3개 이하면 `small`, 요청이 1500자 이상이면 `normal`. 등급은 레인(`docs`·`fix`·`small`·`normal`)별로 **작성자는 싸게, 검사자는 비싸게** — 트리아지 `haiku`, 역할(구현·테스트, 그리고 05 의 수리 작성자)은 전 레인 `sonnet`, 플랜 리뷰어·코드 리뷰어는 `normal` 에서만 `opus`. effort 는 `.claude/agents/*.md` 프론트매터가 역할별로 정합니다 (ADR-H061). **전부 미검증 초기값**입니다 |
...
flowchart TD
    ...
    P00 --> P01 --> P03 --> P04 --> P05 --> P06 --> P07 --> P08 --> DONE
    P00 -. "docs 레인이면 01 리뷰어 · 03 역할을 건너뜀" .-> P03
