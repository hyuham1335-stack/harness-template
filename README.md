...
| 코드 리뷰(05) | 리뷰어 6종(일반·데이터·보안·테스트·구조·문서, 이 순서가 우선순위). 레인별 호출 상한 `docs` 1 · `fix` 1 · `small` 2 · `normal` 4, 리뷰 범위는 `normal` 만 `diff+refs` 이고 나머지는 `diff` (ADR-H062 · H063) |
...
- [docs/harness/pipeline/team-spec.md](docs/harness/pipeline/team-spec.md) — **8단계의 원본 명세.**
  페이즈별 상세, 종료 코드표, 실패 분류, 귀속 규칙이 전부 여기 있습니다
