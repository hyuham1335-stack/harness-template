...
맨 앞의 레인 예측(00)이 요청을 `docs` · `small` · `normal` 로 나누고, 문서만 바뀌는
요청은 계획 리뷰어 · 교차검증 · 구현 역할을 건너뜁니다. 예측이 빗나가면 뒤 단계가
그것을 잡아 등급에 남깁니다.
...
```mermaid
flowchart TD
    P00["00 레인 예측<br/>요청의 경로·길이만 보고 docs·small·normal 판정"]
...
|---|---|---|---|
| **00 레인 예측** | 요청 원문에 언급된 경로와 글자 수만 보고 레인을 예측합니다. 문서 경로만 있으면 `docs`, 소스 경로가 적으면 `small`, 그 밖은 `normal` 입니다. 기계가 못 정하면 저가 모델 한 번이 예측하고, 그래도 모르면 사람에게 묻습니다 | `00_triage.json` | 기계가 확정하면 모델 호출 없이 바로 01 로 갑니다. 예측은 03(계약 유닛 수)·05(변경 파일)가 검증하고, 빗나가면 `PASS_WITH_GAPS` 로 남깁니다 |
...
| `init --feature <이름> --request-file <경로> [--profile docs\|small\|normal]` | 런을 만들고, 요청 파일을 한 글자도 바꾸지 않은 채로 동결합니다. `--profile` 은 사용자가 이미 레인을 말했을 때만 줍니다. 그 값은 뒤 단계의 재판정에 밀리지 않습니다 |
