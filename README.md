...
| **06 PR** | 승인을 받아 기록으로 남기고, 계약 문서를 삭제한 뒤 원격에 push 하고, PR 본문을 만듭니다. 실제 PR 생성은 메인 세션이 GitHub 도구로 합니다. `harness/config.json` 의 `vcs.pr_approval` 이 `auto`(템플릿 기본값)면 게이트 영수증을 확인한 뒤 승인을 스스로 기록하고, 키가 없거나 `user` 면 사람에게 묻습니다 (ADR-H078) | PR 본문과 PR 생성 요청서 | 승인이 없거나 원격·base 브랜치가 없으면 exit 9 로 정상 종료하고 사람을 기다립니다. 승인 후 코드가 바뀌면 다시 승인받습니다. 원격과 어긋나면 강제로 덮어쓰지 않고 멈춥니다 |
...
| `approve --phase 06 [--auto] [--revoke]` | 승인을 기록으로 남깁니다. 그 시점의 작업 트리 해시와 등급을 함께 적습니다. `vcs.pr_approval: auto` 면 `pr` 이 이 명령을 대신 부릅니다 |
