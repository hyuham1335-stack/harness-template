...
| 코드 리뷰(05) | 리뷰어 6종(일반·데이터·보안·테스트·구조·문서, 이 순서가 우선순위). 레인별 호출 상한 `docs` 1 · `fix` 1 · `normal` 4, 리뷰 범위는 `normal` 만 `diff+refs` 이고 나머지는 `diff` (ADR-H062 · H063) |
| 수정 금지 경로 | `harness/**` `docs/**` `scripts/**` `.claude/**` 등 |
| 모델 등급 | 등급은 레인(`docs`·`fix`·`normal`)별로 **작성자는 싸게, 검사자는 비싸게** — 역할(구현·테스트, 그리고 05 의 수리 작성자)은 전 레인 `sonnet`, 플랜 리뷰어·코드 리뷰어는 `normal` 에서만 `opus`. effort 는 `.claude/agents/*.md` 프론트매터가 역할별로 정합니다 (ADR-H061). **전부 미검증 초기값**입니다 |
...
| 경로 | 무엇 |
|---|---|
| `config.json` | 이 프로젝트의 설정 하나입니다. 어댑터, 역할과 소유 경계, 계약 문서의 절 제목, 리뷰어 호출 조건과 우선순위, 런당 예산(테스트를 뺀 파일 수·줄 수·모델 호출 수), 브랜치 규칙, 슬롯별 모델 등급(`models`)이 들어갑니다 |
...
| 경로 | 무엇 |
|---|---|
| `commands/feature.md` | `/feature` 명령입니다. `doctor` 로 열고, 사용자의 요청을 한 글자도 바꾸지 않고 동결한 다음, 종료 코드에 따라 다음 행동을 정합니다. 레인은 사용자가 말했을 때만 `--profile` 로 주고, 봉투가 찍은 `model:` 을 그대로 씁니다 |
