...
| 모델 · effort | `.claude/agents/*.md` 프론트매터가 역할별로 정합니다 — 작성자(구현·테스트·화면)는 `sonnet`, 계획 검토자는 `opus` (ADR-H061). 봉투는 모델을 지시하지 않습니다. **미검증 초기값**입니다 |
...
| **05 코드리뷰** | 04 가 "동작하는가" 를 봤다면 05 는 "계약대로 만들었는가, 봐야 할 리뷰어가 실제로 봤는가" 를 봅니다. 비용이 들지 않는 `precheck` 부터 돌리고, 변경된 파일에 해당하는 리뷰어만 병렬로 부릅니다. 레인별 상한을 넘으면 우선순위(일반·데이터·보안·테스트·구조·문서) 뒤쪽부터 빠집니다. 수리 작성자는 04 와 같은 역할 에이전트입니다 (ADR-H064) | 계약 대조 결과, 리뷰 결과, 승격 후보 | 재검토는 2회까지입니다. 리뷰어가 전부 실패했거나 호출된 리뷰어가 0명이면 등급이 `PASS_WITH_GAPS` 로 내려갑니다 |
...
| 경로 | 무엇 |
|---|---|
| `config.json` | 이 프로젝트의 설정 하나입니다. 어댑터, 역할과 소유 경계, 계약 문서의 절 제목, 리뷰어 호출 조건과 우선순위, 런당 예산(테스트를 뺀 파일 수·줄 수·모델 호출 수), 브랜치 규칙이 들어갑니다 |
