...
것은 어댑터가 선언한 러너와 `git` 뿐이라는 기존 경계를 06 에서도 지킨다. 옛 명세가 06 의
...
**계약 파일 삭제는 push 직전이다.** 04 의 선택자와 05 의 대조가 계약을 계속
읽으므로 시점을 여기까지 미룬다 (ADR-H019 — 지우는 것은 계약 파일뿐이고 그것도
06 에서 한 번이다).
"""
...
def push(root, config, branch):
    ...
    if ok:
        ...
        # push 는 이미 됐을 수도, 안 됐을 수도 있다 (06-pr.md 「실패 시」).
...
def _contract_text(root, state, config):
    """계약 원문. **살아 있는 파일이 없으면 스냅샷을 읽는다**.

    06 push 가 성공하면 계약 파일을 지운다. 07 수리 뒤 재승인하고 `pr` 을 다시
    돌리는 것은 06 페이즈 파일이 선언한 정상 경로인데, 그때 원본이
...
def _check_mark(v):
    """체크리스트 마크. **확인 불가는 체크하지 않는다** (옛 명세 템플릿 매핑표)."""
    return "x" if v else " "
