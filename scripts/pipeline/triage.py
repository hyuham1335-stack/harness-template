...
레인(`docs` · `fix` · `small` · `normal`)을 정하고, 그 값이 01 의 라운드 상한 · 02 의
...
# 내지 않고 "미확정" 으로 모델에 넘긴다. `fix` 도 기계 판정이 내지 않는다
# (ADR-H053) — "버그 수정" 은 언어 키워드라 여기 원칙에 어긋나고, 사람의
# `init --profile fix` · `unclear` 응답 · 모델 제출 세 경로로만 들어온다.
PROFILES = ("docs", "fix", "small", "normal")
VOCAB = PROFILES + ("unclear",)

# 상향·하향의 순위. 재판정이 이 순위에서 **올라가면** 앞 페이즈가 양보를 적용한
# 채 지나간 것이라 miss 다. 내려가면 관측을 더 한 것이라 비용만 더 쓴 것이다.
RANK = {"docs": 0, "fix": 1, "small": 2, "normal": 3}
