...
# (ADR-H029). P6 이 정확히 그랬다 — 교착으로 예산을 태워 **수리를 한 번도
...
def demote(s, grade, gap=None):
    ...
    보고서가 "무엇을 건너뛰었는가"를 적을 수 없다 (08-report.md 가 나열을 요구한다).
    """
...
def count_instructions(s, phase, keys):
    ...
    **제출을 세지 않고 지시를 세는 이유** (ADR-H020): 제출 기준은 두 방향으로
...
def fingerprint(root, config, now=None):
    ...
    - **커밋이 지문을 바꾸지 않는다.** 예전에는 `HEAD + 미커밋 파일 해시` 라
      ...
      무엇을 테스트했는가" 이고 그것은 커밋 여부가 아니라 내용이다 (ADR-H016).
    ...
    - scope_globs_sha1 은 roles[].owns 가 런 도중 바뀐 것을 잡는다.
    """
    root = Path(root)
