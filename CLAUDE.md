...
| 문서 | 다루는 내용 |
|------|------------|
| `/docs/PRD.md` | Core Input, 성공·가드레일 지표, 유저 스토리 Must 목록(AC 는 컴포넌트 스펙에), 기능·데이터 요구사항(MoSCoW), UX·엣지 케이스, GTM·이벤트 로그, 리스크 |
...
`docs/specs/` 컴포넌트 스펙은 차례가 온 US 만 쓰고, 런은 요청이 가리킨 **하나만** 읽는다 — 틀과 쓰는 법은 `docs/specs/_template.md`.
