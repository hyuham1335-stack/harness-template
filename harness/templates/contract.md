# 계약: {기능명}

> 계약은 **메인 에이전트 단독 소유**다. 워커도 리뷰어도 고치지 않는다.
> 결함을 발견하면 고치지 말고 `CONTRACT_DEFECT`로 보고한다.
>
> 아래 절 제목은 `harness/config.json`의 `contract.sections`와 **글자 그대로 일치**해야 한다.
> 한쪽만 고치면 계약 추적이 조용히 아무것도 못 찾고 통과한다 — `doctor`가 그 불일치를 사전에 거부한다.

## 범위

- 프로파일: small | normal
- 대상 경로(역할별): impl → `{glob}` / test → `{glob}`
- 신규 / 기존 수정

## 스키마·데이터 변경

마이그레이션 파일명과 변경 내용. 없으면 "없음".

## 외부 경계

테스트가 모킹할 대상. 시그니처와 반환 형태를 적는다.

- `SearchClient.search(query: string) -> SearchResult | null`

## 데이터 형태

계약 추적이 이 절을 파싱한다 (M57). 백틱 안의 이름 중 **타입(PascalCase)과
상수(UPPER_SNAKE)** 를 계약이 이름 붙인 것으로 세고, 그 이름은
`out_of_contract` 의 "계약에 없는 심볼" 판정에서 빠진다. 필드명·내장·경로는
형태가 달라 세어지지 않으므로 산문에 자유롭게 쓴다.

- `CreateFooInput { name: string(≤255), amount: number(≥0) }`
- `FooDetail { id, name, ... }`

## 유닛

계약 추적과 스코프 테스트 선택이 이 절을 파싱한다. 컨테이너명과 심볼명을 함께 적는다 — 심볼명만으로는 흔한 이름이 다른 파일에서 거짓 통과한다.

**유닛 하나에 최상위 불릿 하나다.** 들여쓴 줄은 그 유닛의 서술이고 유닛으로 세지 않는다. 서술에 백틱을 쓰는 것은 자유다.

- `lib/match.ts · matchTitle(a: string, b: string): number`
  - 정상: 0~1 유사도 반환 / 부수효과: 없음
  - 예외: 빈 문자열이면 `0`

## 진입점

어댑터의 `entrypoint_resolver`가 실재를 확인한다. 진입점 개념이 없는 스택이면 "없음".
인가가 걸린 진입점은 백틱 밖 끝에 `[역할]` 태그를 단다 — 그 진입점의 테스트에 거부 경로가 있는지 검사된다.

- `POST /api/analyze` → 200
- `GET /api/mood/questions` → 200
- `DELETE /api/mood/questions/{id}` [admin] → 204

## 오류 어휘

응답에 실제로 나가는 상수. 코드에 실재하는지 검사된다.

- `ANALYZE_TIMEOUT` (200, 부분 성공)
- `INVALID_IMAGE` (400)

## 여정

없음.

<!--
어댑터에 e2e 스테이지가 있고, 이 런으로 PRD 유저 스토리 하나의 AC 가 전부 충족될 때만 여정 1개. 그 밖에는 "없음".
형식은 유닛과 같다: 최상위 불릿에 백틱으로 `스펙 파일 · 여정 슬러그`, 그 아래 들여쓴 불릿에
진입점 순서를 「진입점」 절의 표기 그대로 `→` 로 잇는다 (예: POST /api/x → GET /api/x/[id]).
어댑터에 e2e 스테이지가 없으면 03 이 디스패치 전에 거부한다.
-->

## 화면

없음.

<!--
화면 컴포넌트를 새로 만들거나 고치는 런에서만 적는다. 그 밖에는 "없음".
항목이 있을 때만 03 이 ui 역할을 부른다 — 없으면 ui 는 미호출이다 (ADR-H057).
형식은 유닛과 같다: 최상위 불릿에 백틱으로 `컨테이너 · 컴포넌트` (예: components/upload/Picker.tsx · Picker),
들여쓴 줄은 그 화면의 동작 서술이다. ui 역할은 UI_GUIDE 를 따르고, 화면의 단위테스트는 두지 않는다.
-->

## 인가

필요한 규칙과 그것을 어디에 추가하는가(미들웨어·가드·설정).

## 계약 밖 (에이전트 자율)

내부 헬퍼, 로깅, 변수명, 테스트 픽스처 구성.
