...
5. **내부 plan-reviewer 만** 반복 검토한다 (ADR-H045). **2라운드부터는 열린
   차단 지적이 있을 때만** 다시 온다 — 봉투의 `planned` 가 누구인지
...
7. **라운드를 강제하는 것은 `converge.blocking_severities`(지금은 Critical)뿐이다.**
   Major·Minor 는 기록되고 보고서로 가되 다음 라운드를 열지 않는다. 열린 Major 를
   고칠지는 작성자의 판단이고, 고쳤으면 다음 회차 제출이 `resolved_from_previous` 로 닫는다.
...
잊으면 exit 8 이고, 비운 것은 "위험 절이 없다" 는 자진신고다 — 05 가 켜진
리뷰어와 대조해 빠진 위험을 `risk_undeclared` 로 기록한다 (ADR-H067).
