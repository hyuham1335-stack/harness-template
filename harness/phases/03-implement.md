...
1. **메인이 계약 파일을 직접 쓴다.** 역할 에이전트에게 위임하지 않는다.
   ...
   **`## 유닛`·`## 진입점` 은 최상위 `- ` 불릿 하나에 항목 하나다.** `### `
   ...
   `record` 가 exit 8 로 거부한다 (ADR-H049). 관례 파일(어댑터의 진입점 맵과
   `convention_globs`)이 스택 관례로 내보내는 이름은 어댑터 `implied_exports` 가
   말하므로 유닛에 다시 적지 않는다. **타입 전용 export(`type`·`interface`)도
   계약에 적지 않아도 된다** — `out_of_contract` 가 보지 않는다 (ADR-H072).
   `enum` 은 런타임 값이라 그대로 계약의 대상이다. 진입점 경로의 파라미터는 `{id}` 로 적어도
...
`docs` 레인을 선언한 런은 **역할 에이전트를 부르지 않고 계약도 쓰지 않는다**
