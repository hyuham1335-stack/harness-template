...
- `diff` — **인라인 diff · 계약 · `05_trace.json`**. 그게 전부다
- `diff+refs` — 위 셋에 더해 **계약 `## 유닛` 이 참조하는 기존 파일의 경로**.
  diff 밖 상호작용(낙관적 잠금 · 상태 가드 · 기존 전이 함수)을 보는 것이 이
  범위의 목적이다 — FR-007 의 동시성 결함이 05 를 지나 07 에서 잡혔다. 파일
  본문을 인라인하지 말고 경로로 준다
...
| 두 리뷰어 지적이 상반 | 판단 | **계약 우선** → `rules_dir` 우선. 판정을 기록에 |
