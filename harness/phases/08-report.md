...
| 페이즈별 소요 — **벽시계 · 순 작업 · 에스컬레이션 대기 · 사람 판단 대기 · 형식 반려 수** — 카운터 넷(라운드 · 수리 · 리뷰 수리 · 계약 대조 수리), **모델 호출 수(근사)** · 자진신고 모델 | **계약이 어디서 부족했는가** — 가장 실행 가능한 산출이다 |
...
**옛 명세 미규정 하나 — 지어내지 않고 적어 둔다.** `gaps[]` 의 어휘가 열거형으로
정의돼 있지 않다. 코드의 `GAP_REASONS` 는 옛 명세 전체에서 **모은 것**이지 명세가
한자리에 준 목록이 아니다.

**exit 11 을 누가 내는가 (ADR-H016).** `on_success` 가 `done` 인 페이즈를 통과시키는
