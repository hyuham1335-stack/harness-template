...
- **`grade == INCOMPLETE` 면 08 을 돌리지 않는다** — `ESCALATION.md` 가
  보고서를 겸한다 (§E12)
- `## 리뷰` 표의 「프로파일」·「07 escaped」, `## 비용과 시간` 의 「지시된 모델
  등급」은 실행기가 조립한다 — **지시된 등급이지 실측이 아니다** (ADR-H044).
  재지 않은 것을 숫자로 적지 마라
...
| 페이즈별 소요 — **벽시계 · 순 작업 · 에스컬레이션 대기 · 사람 판단 대기 · 형식 반려 수** — 재시도, **모델 호출 수(근사)** · 지시된 등급과 자진신고 | **계약이 어디서 부족했는가** — 가장 실행 가능한 산출이다 |
